...
function contactRefToken(inquiryId) {
    const id = parseInt(inquiryId, 10);
    if (!Number.isInteger(id) || id < 1) return '';
    return `VGMF-CTC-${id}`;
}

function buildContactReplyAddress(inquiryId) {
    const parser = parserInboxAddress();
    if (parser) return parser;
    const id = parseInt(inquiryId, 10);
    if (!Number.isInteger(id) || id < 1) return null;
    return `contact-reply+${id}@${replyDomain()}`;
}
...
function parseRefFromText(text) {
    ...
    m = raw.match(/VGMF-CTC-(\d+)/i);
    if (m) {
        return { type: 'contact', inquiryId: parseInt(m[1], 10) };
    }
...
function parseInboundRecipient(addr) {
    ...
    let contactM = local.match(/^contact-reply\+(\d+)$/);
    if (contactM) {
        return { type: 'contact', inquiryId: parseInt(contactM[1], 10) };
    }
    return null;
}
