...
/** Optional Mailparser.io inbox — legacy; built-in IMAP poller is preferred (free). */
function parserInboxAddress() {
    if (String(process.env.INBOUND_IMAP_ENABLED || '1').trim() !== '0') {
        return '';
    }
...
function buildTicketReplyAddress(ticketCanonicalId) {
    ...
    if (String(process.env.INBOUND_IMAP_ENABLED || '1').trim() !== '0' && care) {
        return care;
    }
    const parser = parserInboxAddress();
    if (parser) return parser;
