...
/**
 * Find an existing user by email/phone or create a POS doctor account.
 * cb(err, { userId, userIdString, isNewUser, tempPass })
 */
function ensurePosUser(db, generateId, person, cb) {
    const fn = String(person.firstName || '').trim();
    const mn = String(person.middleName || '').trim();
    const ln = String(person.lastName || '').trim();
    const em = String(person.email || '').trim().toLowerCase();
    const ph = String(person.phone || '').trim();
    findPosUser(db, em, ph, (findErr, existingUser) => {
        if (findErr) return cb(findErr);
        if (existingUser) {
            return cb(null, {
                userId: existingUser.id,
                userIdString: existingUser.user_id_string,
                isNewUser: false,
                tempPass: null
            });
        }
        if (!fn || !ln) return cb(new Error('First and last name required'));
        ...
        const done = (id) => cb(null, { userId: id, userIdString: uidStr, isNewUser: true, tempPass });

        const onInsert = function (insErr) {
            if (!insErr) {
                const newId = this && this.lastID;
                if (newId) return done(newId);
                return db.get(`SELECT id FROM users WHERE user_id_string = ?`, [uidStr], (eId, row) => {
                    if (eId || !row) return cb(eId || new Error('User created but id not returned'));
                    done(row.id);
                });
            }
            if (/unique|duplicate/i.test(String(insErr.message || ''))) {
                return findPosUser(db, em, ph, (e2, again) => {
                    ...
                    if (again) {
                        return cb(null, {
                            userId: again.id,
                            userIdString: again.user_id_string,
                            isNewUser: false,
                            tempPass: null
                        });
                    }
...
/**
 * Find the latest registration for user+seminar or create an on-spot one
 * (status approved_pending_payment). cb(err, { registrationId, applicationNo, status, existing })
 */
function ensurePosRegistration(db, generateId, opts, cb) {
    const { userId, seminarId, actorId, source } = opts;
    db.get(
        ...
        (rErr, existingReg) => {
            ...
            if (existingReg) {
                return cb(null, {
                    ...
                    applicationNo: existingReg.application_no || null,
                    status: String(existingReg.status || '').toLowerCase(),
                    existing: true
                });
            }
            ...
            db.run(
                ...
                [
                    ...
                    appNo,
                    JSON.stringify({
                        ...(opts.formData && typeof opts.formData === 'object' ? opts.formData : {}),
                        source: source || 'pos',
                        onSpot: true,
                        registeredBy: actorId || null,
                        fname: String(opts.firstName || '').trim(),
                        mname: String(opts.middleName || '').trim(),
                        lname: String(opts.lastName || '').trim(),
...
function registerPosRoutes(app, deps) {
    ...
    app.post('/api/admin/pos/register', (req, res) => {
        requireAdminActor(req, res, (actor) => {
            ...
            // On-spot desk registrations are staff-controlled and are allowed even when online capacity is full.
            seminarCapacity.assertSeminarHasCapacity(db, sid, (capErr, capBlock) => {
                ...
                const capacityNote = capBlock ? capBlock.error || 'Seminar is full (on-spot override)' : null;
                    ...
                    (uErr, u) => {
                        ...
                        ensurePosRegistration(
                            ...
                            (rErr, reg) => {
                                ...
                                if (reg.existing && (reg.status === 'completed' || reg.status === 'checked_in')) {
                                ...
                                finishPayment(reg.registrationId, reg.applicationNo);
                            }
                        );
                    }
                );
...
module.exports = { registerPosRoutes, findPosUser, ensurePosUser, ensurePosRegistration };
