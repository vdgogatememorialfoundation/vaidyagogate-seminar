...
const AUX_TABLE_DDL = [
    ...
    {
        name: 'application_reviewer_profiles',
        sql: `CREATE TABLE IF NOT EXISTS application_reviewer_profiles (
    ...
    {
        name: 'seminar_events',
        sql: `CREATE TABLE IF NOT EXISTS seminar_events (
            id SERIAL PRIMARY KEY,
            seminar_id INTEGER NOT NULL,
            title TEXT NOT NULL,
            description TEXT,
    ...
    {
        name: 'seminar_days',
        sql: `CREATE TABLE IF NOT EXISTS seminar_days (
            id SERIAL PRIMARY KEY,
            seminar_id INTEGER NOT NULL,
            title TEXT NOT NULL,
            day_date TEXT,
    ...
    // Central WhatsApp system (campaigns, templates, logs, flows) — shared DDL from whatsapp-central.
    ...require('./whatsapp-central')
        .SCHEMA.filter((sql) => /CREATE TABLE/i.test(sql))
        .map((sql) => ({
            name: /CREATE TABLE IF NOT EXISTS (\w+)/i.exec(sql)[1],
            sql: require('./sql-convert').convertSqliteToPostgres(sql)
        }))
];
...
const CERTIFICATE_VERIFY_PG_ALTERS = [
    ...
    'ALTER TABLE seminars ADD COLUMN IF NOT EXISTS location_text TEXT',
    'ALTER TABLE seminar_volunteers ADD COLUMN IF NOT EXISTS duties TEXT'
];

const APPLICATION_REVIEW_PG_ALTERS = [
    ...
    'ALTER TABLE registrations ADD COLUMN IF NOT EXISTS review_escalated_at TIMESTAMPTZ',
...
async function ensureApplicationReviewSchema(queryWithRetry, isIgnorablePgError) {
    for (const sql of APPLICATION_REVIEW_PG_ALTERS) {
        try {
            await queryWithRetry(sql, [], 2);
        } catch (e) {
            if (!isIgnorablePgError(e)) {
                console.warn('[pg-schema] application review column:', e.message);
            }
        }
    }
}
...
async function ensureAuxiliaryTables(queryWithRetry, isIgnorablePgError) {
    ...
    await ensureApplicationReviewSchema(queryWithRetry, isIgnorablePgError);
    ...
    for (const sql of [
        ...
        'ALTER TABLE refunds ADD COLUMN IF NOT EXISTS provider_status TEXT',
        ...
        'ALTER TABLE refunds ADD COLUMN IF NOT EXISTS provider_payment_id TEXT',
        'ALTER TABLE refunds ADD COLUMN IF NOT EXISTS bank_utr TEXT',
        'CREATE INDEX IF NOT EXISTS idx_refunds_provider_id ON refunds (provider_refund_id)',
        'ALTER TABLE cancellation_requests ADD COLUMN IF NOT EXISTS action_type TEXT',
        'ALTER TABLE cancellation_requests ADD COLUMN IF NOT EXISTS initiated_by TEXT',
        'ALTER TABLE tickets ADD COLUMN IF NOT EXISTS event_id INTEGER',
        'ALTER TABLE tickets ADD COLUMN IF NOT EXISTS day_id INTEGER',
        'ALTER TABLE seminars ADD COLUMN IF NOT EXISTS day_selection_mode TEXT',
        'ALTER TABLE seminars ADD COLUMN IF NOT EXISTS event_end_date TIMESTAMPTZ',
        'ALTER TABLE seminars ADD COLUMN IF NOT EXISTS ticket_expires_at TIMESTAMPTZ',
        'ALTER TABLE seminars ADD COLUMN IF NOT EXISTS intro_video_url TEXT',
        'ALTER TABLE registrations ADD COLUMN IF NOT EXISTS ticket_expired_at TIMESTAMPTZ',
        ...
        'ALTER TABLE seminar_events ADD COLUMN IF NOT EXISTS description TEXT',
        'ALTER TABLE seminar_events ADD COLUMN IF NOT EXISTS location_text TEXT',
        'ALTER TABLE seminar_events ADD COLUMN IF NOT EXISTS location_url TEXT',
        'ALTER TABLE seminar_events ADD COLUMN IF NOT EXISTS capacity INTEGER',
        ...
        'ALTER TABLE seminars ADD COLUMN IF NOT EXISTS alumni_source_seminar_ids TEXT',
        'ALTER TABLE seminars ADD COLUMN IF NOT EXISTS alumni_notify_auto INTEGER DEFAULT 0',
        'ALTER TABLE seminars ADD COLUMN IF NOT EXISTS alumni_notify_sent_at TIMESTAMPTZ',
        `CREATE TABLE IF NOT EXISTS seminar_alumni_notification_log (
...
module.exports = {
    ...
    ensureApplicationReviewSchema,
    APPLICATION_REVIEW_PG_ALTERS,
