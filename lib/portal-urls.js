...
function resolveAccountPortal(user) {
    ...
    const ur = userRoles.normalizeUserRole(user.user_role);
    const r = userRoles.normalizeUserRole(user.role);
    if (eff === 'judge_user') {
        return {
            url: urls.judge,
            name: 'Judge portal',
    ...
    if (ur === 'co_admin') {
        return {
            ...
            blurb:
                'Co-admins sign in at the staff portal. After login, open Staff CRM for full seminar administration.'
        };
    }
    if (r === 'admin' && ur !== 'co_admin') {
        return {
            url: urls.admin,
            name: 'Admin portal',
