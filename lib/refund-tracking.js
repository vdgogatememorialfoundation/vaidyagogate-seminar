 ...
 */
function buildRefundEligibilityView(policyJson, eventDate, orderAmount, checkinContext) {
    const policy = cancelPolicy.parseCancellationPolicy(policyJson);
    const gate = cancelPolicy.evaluateDoctorCancellation(policyJson, eventDate, checkinContext);
...
function mergeCancellationIntoTimeline(timeline, cancelRow, refunds) {
    ...
    const rs = String(cancelRow.refund_status || 'none').toLowerCase();
    ...
    return {
        ...
        cancellationLivePending: livePending
    };
}

function slimCancellationTracking(cancelRow, refunds) {
    ...
    return {
        ...
        actionType: cancelRow.action_type || null,
        initiatedBy: cancelRow.initiated_by || 'doctor',
        refundOnly: isRefundOnlyRow(cancelRow),
        refundStatus: cancelRow.refund_status || 'none',
        refundStatusLabel: refundStatusLabel(cancelRow.refund_status),
        refundStatusTone: refundStatusTone(cancelRow.refund_status),
        refundAmount: cancelRow.refund_amount,
        refundPercent: cancelRow.refund_percent,
        providerRefundId: cancelRow.provider_refund_id || null,
        bankUtr: (latestRefund && latestRefund.bank_utr) || null,
        trackingSteps: buildRefundTrackingSteps(cancelRow, refunds, cancelRow.registration_status),
        refunds: (refunds || []).map((r) => ({
            ...
            bankUtr: r.bank_utr || null,
            failureReason: r.failure_reason || null
        })),
        razorpayLive:
            latestRefund && latestRefund.gateway === 'razorpay'
                ? {
                      providerRefundId: latestRefund.provider_refund_id || cancelRow.provider_refund_id,
                      bankUtr: latestRefund.bank_utr || (rzSummary && rzSummary.bankUtr) || null,
 ...
 */
function isRefundOnlyRow(cancelRow) {
    return !!cancelRow && String(cancelRow.action_type || '').toLowerCase() === 'refund_only';
}

function isAdminInitiatedRow(cancelRow) {
    return !!cancelRow && String(cancelRow.initiated_by || '').toLowerCase() === 'admin';
}
...
function buildRefundTrackingSteps(cancelRow, refunds, regStatus) {
    ...
    const refundOnly = isRefundOnlyRow(cancelRow);
    const byAdmin = isAdminInitiatedRow(cancelRow);
    const steps = [];

    steps.push({
        key: 'requested',
        title: refundOnly ? 'Refund initiated by office' : byAdmin ? 'Cancelled by office' : 'Cancellation requested',
        state: 'completed',
        at: cancelRow.requested_at || null,
        desc: cancelRow.reason
            ? String(cancelRow.reason).slice(0, 180)
            : refundOnly
              ? 'The office initiated a refund on your payment. Your registration stays active.'
    ...
    if (refundOnly) {
        steps.push({
            key: 'review',
            title: 'Registration remains active',
            state: 'completed',
            at: cancelRow.reviewed_at || null,
            desc: cancelRow.admin_notes || 'Your seat and e-ticket are unaffected by this refund.'
        });
    } else if (st === 'pending') {
        steps.push({
            key: 'review',
        ...
        steps.push({
            ...
            desc:
                (bankUtr
                    ? 'Bank UTR / RRN: ' +
                      bankUtr +
                      ' — share this with your bank if the credit is not visible yet. '
                    : '') +
...
function mapCancellationRowForClient(row, refunds, liveEligibility) {
    ...
    return {
        ...
        initiatedBy: row.initiated_by || 'doctor',
        refundOnly: isRefundOnlyRow(row),
        reason: row.reason,
    ...
    loadLatestCancellationsForRegistrations(db, ids, (e1, cancelByReg) => {
        ...
        loadRefundsForRegistrations(db, ids, (e2, refundsByReg) => {
            ...
            const out = list.map((row) => {
                ...
                // If the doctor reopened/re-registered, the registration row id can be reused.
                // In that case we must NOT keep showing the old cancellation/refund tracking on the "fresh" application.
                // Refund-only rows are shown regardless of registration status (the registration stays active).
                if (!cancelRow || (currentStatus !== 'cancelled' && !isRefundOnlyRow(cancelRow))) {
                    return { ...row, cancellationTracking: null };
                }
                const refunds = refundsByReg[row.id] || [];
                const slim = slimCancellationTracking(cancelRow, refunds);
                const mergedTimeline = mergeCancellationIntoTimeline(row.timeline, cancelRow, refunds);
                mergedTimeline.cancellationTracking = slim;
                return {
                    ...row,
                    timeline: mergedTimeline,
                    cancellationTracking: slim
                };
            });
            cb(null, out);
        });
...
function enrichAdminCancellationRow(row, refunds) {
    ...
    return {
        ...row,
        refundStatusLabel: refundStatusLabel(row.refund_status),
        bankUtr: (latestRefund && latestRefund.bank_utr) || (rzSummary && rzSummary.bankUtr) || null,
