...
function persistMulterFile(db, file, uploadsDir, cb) {
    ...
    // #region agent log
    try {
        const dbg = require('./debug-agent-log');
        dbg.agentLog(
            'file-store.js:persistMulterFile',
            'persistMulterFile entry',
            {
                useBlob: useBlobStore(),
                vercel: !!process.env.VERCEL,
                hasPath: !!(file.path && fs.existsSync(file.path)),
                bufferLen: file.buffer ? file.buffer.length : 0,
                storageKey
            },
            'H2'
        );
        ...
        return ensureSchema(db, (schemaErr) => {
            ...
            storeBlob(db, storageKey, buffer, file.mimetype, file.originalname, (storeErr) => {
                ...
                try {
                    const dbg = require('./debug-agent-log');
                    dbg.agentLog(
                        'file-store.js:storeBlob',
                        storeErr ? 'blob store failed' : 'blob store ok',
                        { storageKey, err: storeErr ? storeErr.message : null },
                        'H2'
                    );
                } catch (_) {}
                // #endregion
    ...
    try {
        ...
        try {
            const dbg = require('./debug-agent-log');
            dbg.agentLog(
                'file-store.js:diskWrite',
                'disk write failed',
                { err: e.message, uploadsDir },
                'H5'
            );
        } catch (_) {}
        // #endregion
