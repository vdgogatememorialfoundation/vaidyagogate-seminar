...
const lookupIpGeo = ipGeo.lookupIpGeo;
...
function createSession(db, opts, cb) {
    ...
    const portal =
        String(opts.channel || 'web').toLowerCase() === 'doctor_portal' || opts.userId ? 'doctor' : 'website';
    const canLive = supportDesk.isPortalLiveChatEnabled(cfg, portal) && supportDesk.isWithinBusinessHours(cfg);
