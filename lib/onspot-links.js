...
function registerOnspotRoutes(app, deps) {
    const { db, generateId, notifEngine, activityLog, seminarCapacity, listDoctorPaymentOptions } = deps;
        ...
        db.get(`SELECT * FROM onspot_links WHERE token = ?`, [token], (err, row) => {
            ...
            const proceed = (userId) => {
                seminarCapacity.assertSeminarHasCapacity(db, row.seminar_id, (capErr, capBlock) => {
                    ...
                    if (capBlock && capBlock.ok === false && !row.registration_id) {
                        return res.status(409).json({ error: capBlock.error || 'Seminar is full' });
                    }
