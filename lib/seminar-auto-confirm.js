...
function autoConfirmRegistration(db, deps, ctx, cb) {
    ...
    db.run(
        ...
        function (uErr) {
            ...
            const finish = (extra, feeAmount) => {
                cb &&
                    cb(
                        null,
                        Object.assign(
                            { ok: true, status: 'approved_pending_payment', feeAmount: feeAmount != null ? feeAmount : 0 },
                            extra || {}
                        )
                    );
            };
            ...
            seminarEvents.computeRegistrationPaymentAmount(db, rid, (payErr, pay) => {
                ...
                if (pay && pay.error) return cb && cb(new Error(pay.error));
                const defaultAmt =
                    pay && pay.amount != null
                        ? pay.amount
                        : ctx.seminarPrice != null && Number(ctx.seminarPrice) > 0
                          ? Number(ctx.seminarPrice)
                          : 1500;

                if (!getOrCreatePendingOrder) return finish(null, defaultAmt);
                ...
                getOrCreatePendingOrder(rid, defaultAmt, () => {
                    ...
                    finish({ orderCreated: true }, defaultAmt);
                });
            });
        }
    );
