/**
 * DATABASE_URL resolution and validation (Render / Neon).
 */
...
function publicDatabaseHint(code) {
    switch (code) {
        case 'DATABASE_URL_MISSING':
            return envDashboardHint() + ' Add DATABASE_URL (Neon pooler URL with ?sslmode=require), then redeploy.';
        case 'DATABASE_URL_INVALID':
        case 'DATABASE_URL_PLACEHOLDER':
            return 'In Neon: copy the pooled connection string. Paste as DATABASE_URL in Render env, then redeploy.';
