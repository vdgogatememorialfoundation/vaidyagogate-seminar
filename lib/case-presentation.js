...
const caseNotify = require('./case-presentation-notify');
...
const certificatePdf = require('./certificate-pdf-server');
...
const DEFAULT_CASE_INSTRUCTIONS = `CASE PRESENTATION COMPETITION 2026

Categories:
1. Agnikarma
2. Viddhakarma

Prize: Cash prizes up to Rs.75,000/-
Winners & selection: Selected participants receive an official selection letter regarding the cash prize. Final winners and prize amounts are declared on the seminar day (date as announced for the national seminar). Seminar registration is mandatory for all selected participants — you must complete paid seminar registration to present and receive prizes.
Eligibility: Practicing Vaidyas, P.G. Scholars, PhD scholars, Faculty members.

Submit online via the Doctor portal (Case presentation tab) — upload your case video and signed CV here. Email submission is also accepted (see below).

Submission details:
1. Submission should be in video format.
2. Video size should not be more than 50 MB.
3. This will be the final submission for the competition — any images, videos, documents, etc. should be included in this submission only.
4. Case should be presented in IMRAD format.
5. Inclusion of before and after case reports is welcome.
6. Points of Viddhakarma and Agnikarma should be precise and reason for choosing the points must be included.
7. Copied cases will be disqualified.
8. The case submitted for this competition should not be submitted elsewhere. If found otherwise, the entry will be disqualified.
9. You may also send submissions via email to vd.gogatememorialfoundation@gmail.com and care@vaidyagogate.org (portal upload is preferred).
10. Please attach your CV along with the case video.
11. CV must include the following undertaking duly signed by the participant:
    a) I agree that the judges' decision is final.
    b) I am ready to answer any questions with reference to the case.
    c) I take full responsibility of the fact that whatever information I have provided is completely true.
12. Max time for presentation: 10 + 2 minutes.
13. Judges may contact you for clarification, information and ask for presentation if need be.
14. All the rules and regulations regarding medico legal requirements to be followed.
15. Patient's or his / her parents' consent must and to be presented if asked for.
16. You can avoid Guru vandan etc. Your good case is itself 'GuruVandan'.

Last date for submission: 15th August 2026, 9:00 PM IST.
Up to 20 good cases will be awarded!

For queries, contact: WhatsApp 9373792952 / 8275764426
Email: care@vaidyagogate.org · vd.gogatememorialfoundation@gmail.com`;

const DEFAULT_CASE_PROGRAM_TITLE = 'Case Presentation Competition 2026';
/** Registration closes 15 Aug 2026 21:00 IST (stored as ISO for DB). */
const DEFAULT_CASE_REGISTRATION_END = '2026-08-15T21:00:00+05:30';
...
const DEFAULT_CASE_FORM_CONFIG = {
    ...
    fields: [
        ...
        { key: 'dob', label: 'Date of birth', type: 'date', enabled: true, required: true },
        { key: 'email', label: 'Email id', type: 'email', enabled: true, required: true },
        { key: 'phone', label: 'Phone no.', type: 'tel', enabled: true, required: true },
        { key: 'whatsapp', label: 'WhatsApp no.', type: 'tel', enabled: true, required: true },
        {
            key: 'category',
            label: 'Category (Agnikarma / Viddhakarma / Both)',
            type: 'select',
            enabled: true,
            required: true
        },
        {
            ...
            required: true,
            options: [
                { value: 'Practicing Vaidya', label: 'Practicing Vaidyas' },
                { value: 'PG Scholar', label: 'P.G. Scholars' },
                { value: 'PhD Scholar', label: 'PhD scholars' },
...
function caseFormFileFields(fields) {
    return (fields || []).filter(
        (f) =>
            f &&
            f.enabled !== false &&
            (normalizeCaseFieldType(f.type) === 'file' || f.key === 'files')
    );
}

function caseFormRequiredFileFieldCount(fields) {
    return caseFormFileFields(fields).filter((f) => f.required !== false).length;
}
...
function caseFieldActive(fields, key) {
...
function enrichCaseProgram(row, extra) {
    ...
    return {
        ...
        maxFilesPerSubmission: row.max_files_per_submission != null ? row.max_files_per_submission : 2,
    ...
    return {
        ...
        maxTotalSubmissions:
            b.maxTotalSubmissions != null && String(b.maxTotalSubmissions).trim() !== ''
                ...
                : null,
        maxFilesPerSubmission: Math.max(1, Math.min(10, parseInt(b.maxFilesPerSubmission, 10) || 2)),
        maxFileSizeMb: Math.max(1, Math.min(250, parseInt(b.maxFileSizeMb, 10) || 50)),
...
function validateCaseFormAgainstConfig(form, formConfig, enabledCategories) {
    ...
    for (const f of fields) {
        ...
        if (t === 'checkbox' || t === 'boolean' || t === 'terms') {
            if (f.required !== false && val !== '1') {
                return `Please confirm: ${f.label || f.key}`;
            }
            continue;
        }
        if (t === 'file' || f.key === 'files') continue;
        if (t === 'multiselect') {
            if (f.required !== false && (val === undefined || val === null || String(val).trim() === '')) {
        ...
        if (t === 'rating') {
            if (f.required !== false && (val === undefined || val === null || String(val).trim() === '')) {
                return `Please rate: ${f.label || f.key}`;
            }
            continue;
        }
        if (f.required !== false && (val === undefined || val === null || String(val).trim() === '')) {
        ...
        if (t === 'select' && Array.isArray(f.options) && f.options.length && val != null && String(val).trim() !== '') {
            const ok = f.options.some((o) => String(o.value != null ? o.value : o.label) === String(val));
            if (!ok) return `Invalid choice for: ${f.label || f.key}`;
        }
    }
    if (caseFieldActive(fields, 'fname') || caseFieldActive(fields, 'lname')) {
    ...
    if (caseFieldActive(fields, 'email') && form.email != null && String(form.email).trim() !== '') {
        const ev = contactValidation.validateEmail(form.email, 'Email');
        if (!ev.valid) return ev.message;
    }
    if (caseFieldActive(fields, 'phone') && form.phone != null && String(form.phone).trim() !== '') {
        const pv = contactValidation.validatePhone(form.phone, 'Phone');
        if (!pv.valid) return pv.message;
    }
    if (caseFieldActive(fields, 'whatsapp') && form.whatsapp != null && String(form.whatsapp).trim() !== '') {
    ...
    if (caseFieldActive(fields, 'category')) {
        if (!cats.includes(form.category)) {
            return 'Select a valid category for this program';
        }
    }
    const topicField = fields.find((f) => f.key === 'topic');
    if (topicField && topicField.enabled !== false && topicField.required !== false && !form.topic) {
        return 'Case topic is required';
    }
    return null;
}

function normalizeCaseFormForProgram(form, program) {
    const cfg = parseCaseFormConfig(program && program.form_config_json);
    const fields = cfg.fields || [];
    const cats = parseEnabledCategories(program && program.enabled_categories);
    const out = { ...form };
    if (!caseFieldActive(fields, 'category') && !out.category) {
        out.category = cats[0] || CASE_CATEGORIES[0];
    }
    if (!caseFieldActive(fields, 'topic') && !out.topic) {
        out.topic = 'Case presentation';
    }
    return out;
}
        ...
        db.run(
            ...
            (e0) => {
                ...
                const alters = [
                    ...
                    `ALTER TABLE case_submissions ADD COLUMN marking_deadline TEXT`,
...
function parseCaseFormBody(body) {
    ...
    const sources = [form, body];
    sources.forEach((src) => {
        if (!src || typeof src !== 'object') return;
        Object.keys(src).forEach((k) => {
            if (reserved.has(k) || Object.prototype.hasOwnProperty.call(out, k)) return;
            const v = src[k];
            if (v == null) return;
...
function registerCasePresentationRoutes(app, deps) {
    ...
    const { db, upload, generateId, fileStore, uploadsDir, notifEngine } = deps;
    
    // Store upload middleware for use in routes
    const _uploadMiddleware = upload;
    ...
    app.post('/api/case/draft', (req, res) => {
        const body = req.body || {};
        ...
        db.get(`SELECT * FROM case_programs WHERE id = ? AND IFNULL(is_active, 1) = 1`, [programId], (e0, program) => {
            ...
            const form = normalizeCaseFormForProgram(parseCaseFormBody(body), program);
            const category = form.category || '';
            const win = caseWindowState(program);
            db.get(
                `SELECT id, status, application_no FROM case_submissions WHERE user_id = ? AND case_program_id = ? AND (category = ? OR category IS NULL OR category = '') AND status IN ('draft','submitted','under_review') ORDER BY id DESC LIMIT 1`,
                [userId, programId, category],
                (eDup, row) => {
                    if (eDup) return res.status(500).json({ error: eDup.message });
                    if (row && String(row.status || '').toLowerCase() !== 'draft') {
                        return res.status(400).json({ error: 'You already submitted for this program.' });
                    }
                    const persist = (draftRow) => {
                        const appNo = (draftRow && draftRow.application_no) || generateCaseApplicationNo();
                        const formJson = JSON.stringify(form);
                        const title = form.topic || 'Case presentation (draft)';
                        const seminarId = program.seminar_id || null;
                        if (draftRow) {
                            return db.run(
                                `UPDATE case_submissions SET category = ?, title = ?, form_data = ?, application_no = COALESCE(application_no, ?), updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'draft'`,
                                [category, title, formJson, appNo, draftRow.id],
                                function (uErr) {
                        ...
                        db.run(
                            `INSERT INTO case_submissions (user_id, seminar_id, case_program_id, application_no, category, title, form_data, status, updated_at)
                             VALUES (?, ?, ?, ?, ?, ?, ?, 'draft', CURRENT_TIMESTAMP)`,
                            [userId, seminarId, programId, appNo, category, title, formJson],
                            function (insErr) {
                    ...
                    if (row) return persist(row);
                    if (win === 'closed') {
                        return res.status(400).json({ error: 'Applications are closed for this program.' });
                    }
                    if (win === 'upcoming') {
                        return res.status(400).json({ error: 'Applications are not open yet for this program.' });
                    }
                    persist(null);
                }
            );
    ...
    app.post('/api/case/submit', _uploadMiddleware.array('files', 10), (req, res) => {
        const handleSubmit = (req, res, files, r2UploadIds) => {
            const body = req.body || {};
            const userId = parseInt(body.userId, 10);
            const programId = parseInt(body.caseProgramId, 10);
            console.log('[case/submit] userId:', userId, 'programId:', programId, 'r2UploadIds:', r2UploadIds);
            if (!Number.isInteger(userId)) {
                return res.status(400).json({ error: 'userId required — please sign in again to the doctor portal' });
            }
            if (!Number.isInteger(programId)) {
            ...
            db.get(`SELECT * FROM case_programs WHERE id = ? AND IFNULL(is_active, 1) = 1`, [programId], (e0, program) => {
                if (e0) {
                    console.error('[case/submit] DB error:', e0.message);
                    return res.status(500).json({ error: e0.message });
                }
                if (!program) {
                    console.warn('[case/submit] Case program not found:', programId);
                    return res.status(404).json({ error: 'Case program not found' });
                }
                const maxFiles = Math.max(1, Math.min(10, program.max_files_per_submission || 5));
                let form = normalizeCaseFormForProgram(parseCaseFormBody(body), program);
                ...
                const formFields = parseCaseFormConfig(program.form_config_json).fields || [];
                const requiredFileCount = caseFormRequiredFileFieldCount(formFields);
                const uploadedCount = multerFiles.length + pendingIds.length;
                if (requiredFileCount > 0 && uploadedCount < requiredFileCount) {
                    return res.status(400).json({
                        error:
                            'Upload all required files (' +
                            requiredFileCount +
                            ' file field(s) required — e.g. CV and video).'
                    });
                }
                ...
                const maxMb = uploadLimits.getEffectiveMaxFileMb(program.max_file_size_mb || 50);
                ...
                let existingDraftId =
                    Number.isInteger(parseInt(body.draftSubmissionId, 10)) && parseInt(body.draftSubmissionId, 10) > 0
                        ? parseInt(body.draftSubmissionId, 10)
                        : null;
                const win = caseWindowState(program);
                if (win === 'closed') {
                    return res.status(400).json({ error: 'Applications are closed for this program.' });
                ...
                db.get(
                    `SELECT id, status FROM case_submissions WHERE user_id = ? AND case_program_id = ? AND category = ? AND status != 'priority_invited'`,
                    ...
                    (eDup, dup) => {
                        ...
                        db.get(
                            `SELECT COUNT(*) AS c FROM case_submissions WHERE user_id = ? AND case_program_id = ? AND status NOT IN ('cancelled', 'draft')`,
                            [userId, programId],
                            (eCnt, cntRow) => {
                        ...
                        const linkReg = (regId, cb) => {
                            const formJson = JSON.stringify(form);
                            const beginSubmission = (subId, appNo, insErr) => {
                                    if (insErr) return res.status(500).json({ error: insErr.message });
                                    const notifySubmitted = (cb) => {
                                        caseNotify.notifyCaseDoctor(
                                            db,
                                            notifEngine,
                                            subId,
                            ...
                            if (existingDraftId) {
                                return db.get(
                                    `SELECT application_no FROM case_submissions WHERE id = ? AND user_id = ? AND status = 'draft'`,
                                    [existingDraftId, userId],
                                    (eDr, dr) => {
                                        if (eDr || !dr) {
                                            return res.status(400).json({ error: 'Draft not found or already submitted.' });
                                        }
                                        const appNo = dr.application_no || generateCaseApplicationNo();
                                        db.run(
                                            `UPDATE case_submissions SET category = ?, title = ?, form_data = ?, registration_id = ?, status = 'submitted', updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'draft'`,
                                            [form.category, form.topic, formJson, regId, existingDraftId],
                                            function (uErr) {
                                                if (uErr) return res.status(500).json({ error: uErr.message });
                                                if (!this.changes) {
                                                    return res.status(400).json({ error: 'Draft could not be submitted.' });
                                                }
                                                beginSubmission(existingDraftId, appNo, null);
                                            }
                                        );
                                    }
                                );
                            ...
                            db.run(
                                ...
                                function (insErr) {
                                    beginSubmission(this.lastID, appNo, insErr);
                                }
                            );
                        };
        ...
        try {
            const body = req.body || {};
            console.log('[case/submit] Body keys:', Object.keys(body));
            console.log('[case/submit] uploadedFileIds raw:', body.uploadedFileIds);
            
            const rawIds = body.uploadedFileIds;
            if (rawIds) {
                if (typeof rawIds === 'string') {
                    try {
                        uploadIds = JSON.parse(rawIds);
                    } catch (e) {
                        console.log('[case/submit] JSON parse failed for:', rawIds.substring(0, 100));
                    }
                } else if (Array.isArray(rawIds)) {
                    uploadIds = rawIds;
                }
            }
            // Also extract *_uploadId fields from form data (e.g., upload_cv_uploadId, upload_video_uploadId)
            if (!uploadIds.length) {
                for (const key of Object.keys(body)) {
                    if (key.endsWith('_uploadId') && body[key]) {
                        uploadIds.push(String(body[key]));
                    }
                }
            }
        } catch (e) {
            console.log('[case/submit] Error parsing uploadIds:', e.message);
            uploadIds = [];
        }
        if (!Array.isArray(uploadIds)) uploadIds = [];
        console.log('[case/submit] Final uploadIds:', uploadIds);
        const uploadLimitsMod = require('./upload-limits');
        if (uploadIds.length) {
            return handleSubmit(req, res, [], uploadIds);
        }
        // Since multer is now middleware, req.files is already parsed
        ...
        db.all(
            ...
            (e, rows) => {
                ...
                portalTracking.attachCaseTimelines(db, mapped, (e2, enriched) => {
                    const apps = (e2 ? mapped : enriched || mapped).map((row) => {
                        const out = { ...row };
                        delete out.judge_count;
                        delete out.locked_score_count;
                        delete out.avg_score;
                        return out;
                    });
    ...
    app.get('/api/admin/case/default-form-config', (req, res) => {
        res.json({
            ...
            instructions: DEFAULT_CASE_INSTRUCTIONS,
            defaultTitle: DEFAULT_CASE_PROGRAM_TITLE,
            defaultRegistrationEnd: DEFAULT_CASE_REGISTRATION_END
        });
    });
    ...
    app.post('/api/admin/case/programs', (req, res) => {
        ...
        if (!String(body.instructions || '').trim()) {
            body.instructions = DEFAULT_CASE_INSTRUCTIONS;
        }
        const row = programBodyToRow(body);
    ...
    app.post('/api/admin/case/submissions/:id/select-winner', (req, res) => {
        const id = parseInt(req.params.id, 10);
        const finishWinner = (message) => {
            portalTracking.logCaseEvent(
                db,
                id,
                'selected',
                'Selected',
                'Marked as winner. Selection letter for cash prize; final prize on seminar day. Seminar registration mandatory.',
                () => {
                    caseNotify.notifyCaseDoctor(db, notifEngine, id, 'CASE_PRESENTATION_SELECTED', {}, () => {
        ...
        db.get(`SELECT * FROM case_submissions WHERE id = ?`, [id], (e, sub) => {
            ...
            if (!seminarId) {
                return db.run(
                    ...
                    () =>
                        finishWinner(
                            'Selected. Send cash-prize selection letter. Seminar registration mandatory if a seminar is linked later.'
                        )
                );
            }
            db.get(
                ...
                (e2, reg) => {
                    if (reg) {
                        db.run(
                            ...
                            () =>
                                finishWinner(
                                    'Selected. Cash-prize selection letter sent. Doctor is registered for the seminar. Winners declared on seminar day.'
                                )
                        );
                    } else {
                        db.run(
                            ...
                            () =>
                                finishWinner(
                                    'Selected. Cash-prize selection letter sent. Doctor must complete mandatory seminar registration (priority override can be applied).'
                                )
                        );
                    }
                }
        ...
        db.all(
            ...
            (e, rows) => {
                ...
                const out = (rows || []).map((r) => {
                    const closed = markingDeadline.isMarkingDeadlinePassed(r.marking_deadline);
                    return Object.assign({}, r, {
                        marking_deadline_display: markingDeadline.formatMarkingDeadlineDisplay(r.marking_deadline),
                        marking_closed: closed
                    });
                });
                res.json(out);
            }
                        ...
                        db.get(
                            ...
                            async (e3, score) => {
                                ...
                                res.json({
                                    ...
                                    markingDeadline: sub.marking_deadline || null,
                                    markingDeadlineDisplay: markingDeadline.formatMarkingDeadlineDisplay(sub.marking_deadline),
                                    markingClosed: markingDeadline.isMarkingDeadlinePassed(sub.marking_deadline)
                                });
                            }
                        );
                    });
    ...
    app.post('/api/judge/case/score', (req, res) => {
        ...
        db.get(
            ...
            (e0, assigned) => {
                if (e0) return res.status(500).json({ error: e0.message });
                if (!assigned) return res.status(403).json({ error: 'You are not assigned to this submission' });
                db.get(`SELECT plagiarism_zero, marking_deadline, status FROM case_submissions WHERE id = ?`, [sid], (eSub, subRow) => {
                    if (eSub) return res.status(500).json({ error: eSub.message });
                    if (subRow && subRow.plagiarism_zero) {
                        return res.status(400).json({ error: 'Submission disqualified (plagiarism/duplicate)' });
                    }
                    if (subRow && markingDeadline.isMarkingDeadlinePassed(subRow.marking_deadline)) {
                        return res.status(403).json({
                            error: 'Marking deadline has passed. You can no longer submit marks for this case.'
                        });
                    }
                        db.get(
                                    ...
                                    try {
                                        ...
                                        res.json({
                                            ...
                                            message: result.emailSent
                                                ...
                                                : result.emailSkipped && !participant.email
                                                  ? 'Message saved in portal. Participant has no email — they must check the doctor portal.'
                                                  : result.emailError
                                                    ? 'Message saved in portal, but email failed: ' + result.emailError
    ...
    app.get('/api/admin/case/marksheet', (req, res) => {
        ...
        caseMarksheet.loadMarksheetDocument(db, programId, (err, payload) => {
            ...
            if (format === 'pdf' || format === 'html') {
                ...
                return sendMarksheetDocument(res, html, 'case-marksheet-' + (programId || 'all'), format, true);
            }
            res.json(payload);
        });
    });

    function sendMarksheetDocument(res, html, baseName, format, landscape) {
        if (format !== 'pdf') {
        ...
        certificatePdf
            .htmlToPdfBuffer(html, { landscape: landscape !== false })
            .then((buf) => {
            ...
            .catch((e) => {
                console.error('[case-marksheet] PDF render failed:', e.message);
                res.setHeader('Content-Type', 'text/html; charset=utf-8');
                res.setHeader('Content-Disposition', 'inline; filename="' + baseName + '.html"');
                res.send(html);
            });
    ...
    app.get('/api/admin/case/marksheet/candidate/:submissionId', (req, res) => {
        const format = String(req.query.format || 'pdf').toLowerCase();
        caseMarksheet.loadCandidateMarksheet(db, req.params.submissionId, (err, doc) => {
            if (err) return res.status(400).json({ error: err.message });
            if (!doc) return res.status(404).json({ error: 'Submission not found' });
            if (format === 'json') return res.json(doc);
            const html = caseMarksheet.toCandidateMarksheetHtml(doc);
            const base = 'e-marksheet-' + String(doc.candidate.applicationNo || doc.candidate.submissionId).replace(/[^A-Za-z0-9_-]+/g, '');
            sendMarksheetDocument(res, html, base, format, false);
        });
    });

    app.post('/api/judge/case/submissions/:id/transfer-judge', (req, res) => {
        res.status(403).json({
            error: 'Case transfer is managed by administrators. Contact the seminar office if reassignment is needed.'
        });
    });
}
...
module.exports = {
    ...
    DEFAULT_CASE_PROGRAM_TITLE,
    DEFAULT_CASE_REGISTRATION_END,
    caseWindowState,
    enrichCaseProgram,
    parseCaseFormConfig,
    validateCaseForm,
