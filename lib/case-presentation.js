        ...
        upload.array('files', 10)(req, res, (uploadErr) => {
            ...
            db.get(`SELECT * FROM case_programs WHERE id = ? AND IFNULL(is_active, 1) = 1`, [programId], (e0, program) => {
                ...
                for (const f of files) {
                    if (f.size > uploadLimits.getEffectiveMaxFileBytes(program.max_file_size_mb || 50)) {
                        const hint =
                            process.env.VERCEL && (program.max_file_size_mb || 50) > 4
                                ? ' On free cloud hosting the limit is 4 MB per file — compress PDF/PPT or photos before upload.'
                                : '';
                        return res.status(400).json({
                            error: `Each file must be under ${maxMb} MB.${hint}`
                        });
                    }
                }
                        ...
                        const linkReg = (regId, cb) => {
                            ...
                            db.run(
                                ...
                                function (insErr) {
                                    ...
                                    if (!fileStore || !uploadsDir) return finishSubmission([]);
                                    fileStore.persistMulterFiles(db, files, uploadsDir, (pErr, paths) => {
                                        if (pErr) return res.status(500).json({ error: pErr.message });
                                        finishSubmission(paths || []);
                                    });
                                }
                            );
                        };
