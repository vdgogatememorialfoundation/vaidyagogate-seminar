    ...
    return {
        ...
        seminarId:
            b.seminarId != null && String(b.seminarId).trim() !== ''
                ...
                : null,
        registrationStart: b.registrationStart || null,
        registrationEnd: b.registrationEnd || null,
...
function caseWindowState(program) {
    const now = Date.now();
    const rs = program.registration_start ? new Date(program.registration_start).getTime() : null;
    const re = program.registration_end ? new Date(program.registration_end).getTime() : null;
...
function registerCasePresentationRoutes(app, deps) {
    ...
    app.post('/api/case/submit', (req, res) => {
        upload.array('files', 10)(req, res, (uploadErr) => {
            ...
            db.get(`SELECT * FROM case_programs WHERE id = ? AND is_active = 1`, [programId], (e0, program) => {
    ...
    app.post('/api/admin/case/programs', (req, res) => {
        ...
        db.run(
            `INSERT INTO case_programs (
                ...
                enabled_categories, form_config_json
             ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                row.title,
                row.description,
    ...
    app.delete('/api/admin/case/programs/:id', (req, res) => {
        ...
        db.get(`SELECT COUNT(*) AS c FROM case_submissions WHERE program_id = ?`, [id], (e0, row) => {
            if (e0) return res.status(500).json({ error: e0.message });
            ...
            db.all(`SELECT id FROM case_submissions WHERE program_id = ?`, [id], (e2, subs) => {
                if (e2) return res.status(500).json({ error: e2.message });
                let i = 0;
                const nextSub = () => {
    ...
    app.get('/api/judge/case/criteria', (req, res) => {
        res.json({ criteria: CASE_JUDGE_CRITERIA, totalMax: 25 });
    });
                    ...
                    (e1, sub) => {
                        ...
                        db.get(
                            ...
                            (e2, scoreRow) => {
                                ...
                                let total = 0;
                                CASE_JUDGE_CRITERIA.forEach((def) => {
                                ...
                                const finish = (err) => {
                                    ...
                                    res.json({ success: true, totalScore: total, locked: true });
                                };
    ...
    app.get('/api/admin/case/results', (req, res) => {
        ...
        let sql = `
            SELECT cs.id, cs.application_no, cs.title, cs.category, cs.status, cs.program_id,
        ...
        if (Number.isInteger(programId) && programId > 0) {
            sql += ` AND cs.program_id = ?`;
            params.push(programId);
        }
        sql += ` ORDER BY (avg_score IS NULL), avg_score DESC, cs.id ASC`;
        db.all(sql, params, (e, rows) => {
            if (e) return res.status(500).json({ error: e.message });
            res.json({ criteria: CASE_JUDGE_CRITERIA, results: rows || [] });
        });
    });
}
