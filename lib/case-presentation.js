...
const DEFAULT_CASE_FORM_CONFIG = {
    ...
    fields: [
        ...
        { key: 'category', label: 'Category (Agnikarma / Viddhakarma)', type: 'select', enabled: true, required: true },
        { key: 'topic', label: 'Case topic / title', type: 'text', enabled: true, required: true },
        {
            key: 'files',
            label: 'Upload case video (max 50 MB) + signed CV with undertaking (a–c in rules)',
            type: 'file',
            enabled: true,
            required: true
        },
        {
            key: 'agree_terms',
            label: 'I accept the competition rules and confirm my information is accurate',
            type: 'terms',
            enabled: true,
            required: true,
            termsText:
                'I agree that the judges\' decision is final; I am ready to answer questions about my case; and I take full responsibility that the information provided is true.'
        }
    ]
};
        ...
        const handleSubmit = (req, res, files, r2UploadIds) => {
            ...
            db.get(`SELECT * FROM case_programs WHERE id = ? AND IFNULL(is_active, 1) = 1`, [programId], (e0, program) => {
                ...
                const fileFieldRequired = (parseCaseFormConfig(program.form_config_json).fields || []).find(
                    (f) => f.key === 'files' && f.enabled !== false && f.required !== false
                );
                if (fileFieldRequired && !multerFiles.length && !pendingIds.length) {
