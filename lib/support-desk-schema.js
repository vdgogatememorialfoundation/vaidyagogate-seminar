...
function ensureSupportDeskSchema(db, cb) {
    ...
    const ticketAlters = [
        ...
        ['required_authority_level', 'INTEGER DEFAULT 1'],
        ['escalated_at', ts],
    ...
    const profileAlters = [['authority_level', 'INTEGER DEFAULT 1']];

    const sessionAlters = [
        ['linked_ticket_id', 'TEXT'],
        ['no_reply_escalated_at', ts],
        ['contact_form_submitted_at', ts],
        ['chat_ref', 'TEXT'],
        ['visitor_ip', 'TEXT'],
        ['visitor_location', 'TEXT'],
        ['visitor_geo_json', 'TEXT'],
    ...
    const backfillLiveChatRefs = () => {
        db.all(
            `SELECT id FROM support_live_sessions WHERE chat_ref IS NULL OR TRIM(chat_ref) = ''`,
            [],
            (e, rows) => {
                if (e || !rows || !rows.length) return runDeptAlters(0);
                let i = 0;
                const next = () => {
                    if (i >= rows.length) return runDeptAlters(0);
                    const row = rows[i++];
                    const ref = String(Math.floor(Math.random() * 1e12)).padStart(12, '0');
                    db.run(
                        `UPDATE support_live_sessions SET chat_ref = ? WHERE id = ? AND (chat_ref IS NULL OR TRIM(chat_ref) = '')`,
                        [ref, row.id],
                        (uErr) => {
                            if (uErr && /unique|duplicate/i.test(String(uErr.message))) return next();
                            next();
                        }
                    );
                };
    ...
    const runSessionAlters = (j) => {
        if (j >= sessionAlters.length) return backfillLiveChatRefs();
    ...
    const runDeptAlters = (j) => {
        if (j >= deptAlters.length) return runProfileAlters(0);
    ...
    const runProfileAlters = (j) => {
        if (j >= profileAlters.length) return seedDepartments();
        const [name, type] = profileAlters[j];
        const sql = pg
            ? `ALTER TABLE support_agent_profiles ADD COLUMN IF NOT EXISTS ${name} ${type}`
            : `ALTER TABLE support_agent_profiles ADD COLUMN ${name} ${type}`;
        db.run(sql, (e) => {
            ignoreSchemaErr(e);
            runProfileAlters(j + 1);
        });
    };
    ...
    const seedDepartments = () => {
        const defaults = [
            ...
            ['authority', 'Higher authority (cancellation, refunds, certificates)', 'admin', 5]
        ];
        let d = 0;
        const nextDept = () => {
