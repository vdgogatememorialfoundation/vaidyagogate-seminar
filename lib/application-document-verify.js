...
function verifySeminarApplication(db, registrationId, body, deps, cb) {
    ...
    db.get(
        ...
        (e, row) => {
            ...
            let notifyEvent;
            if (decision === 'approve') {
                const eventPages = require('./event-pages');
                newStatus = eventPages.statusAfterApproval(row);
            ...
            db.run(
                ...
                (e2) => {
                    ...
                    if (newStatus === 'approved_pending_payment' && getOrCreatePendingOrder) {
                        const eventPages = require('./event-pages');
                        getOrCreatePendingOrder(rid, eventPages.paymentAmountForSeminar(row), () => {});
                    }
                    if (newStatus === 'completed' && deps.ensureParticipantTicketForRegistration) {
                        deps.ensureParticipantTicketForRegistration(
                            rid,
                            {
                                createOrderIfMissing: true,
                                promotePendingToSuccess: true,
                                amount: require('./event-pages').paymentAmountForSeminar(row)
                            },
                            () => {}
                        );
                    }
                    cb(null, {
                        ...
                        message:
                            decision === 'approve'
                                ? newStatus === 'completed'
                                    ? 'Application approved. E-ticket issued (no payment required).'
