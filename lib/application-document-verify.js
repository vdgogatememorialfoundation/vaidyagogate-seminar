...
function verifySeminarApplication(db, registrationId, body, deps, cb) {
    ...
    if (!['approve', 'reject_documents', 'reject_application'].includes(decision)) {
        return cb(null, { ok: false, error: 'decision must be approve, reject_documents, or reject_application' });
    }
    ...
    db.get(
        ...
        (e, row) => {
            ...
            if (!['submitted', 'pending_approval'].includes(st)) {
                return cb(null, {
                    ok: false,
                    error:
