...
function needsAdvancedQualDocs(formData) {
    ...
    return q === 'PG' || q === 'Practicing Vaidya' || q === 'Practitioner' || q === 'Faculty';
}
...
function verifySeminarApplication(db, registrationId, body, deps, cb) {
    ...
    const feeType = String((body && body.feeType) || 'regular').toLowerCase();
    const feeAmountRaw = body && body.feeAmount != null ? Number(body.feeAmount) : NaN;
    const actingAdminId = body && body.actingAdminId != null ? parseInt(body.actingAdminId, 10) : null;
    const actingStaffId = body && body.actingStaffId != null ? parseInt(body.actingStaffId, 10) : null;
    const actingReviewerId =
        actingStaffId && Number.isInteger(actingStaffId) && actingStaffId > 0
            ? actingStaffId
            : actingAdminId && Number.isInteger(actingAdminId) && actingAdminId > 0
              ? actingAdminId
              : null;
    const { portalTracking, notifEngine, getOrCreatePendingOrder, volunteerTicketFlow, volunteerTicketDeps } = deps;
    ...
    db.get(
        `SELECT r.*, u.first_name, u.last_name, u.email, s.title AS seminar_title, s.price AS seminar_price
        ...
        (e, row) => {
            ...
            const review = {
                ...
                decision,
                fee_type: decision === 'approve' ? (feeType === 'volunteer' ? 'volunteer' : 'regular') : null,
                fee_amount:
                    decision === 'approve' && feeType !== 'volunteer' && Number.isFinite(feeAmountRaw) && feeAmountRaw >= 0
                        ? feeAmountRaw
                        : decision === 'approve' && feeType === 'volunteer'
                          ? 0
                          : null
            };
            ...
            let newStatus;
            let notifyEvent;
            const approveAsVolunteer = decision === 'approve' && feeType === 'volunteer';
            if (decision === 'approve') {
                newStatus = approveAsVolunteer ? String(row.status || 'submitted').toLowerCase() : 'approved_pending_payment';
            ...
            const rejectFromApproved =
                decision === 'reject_application' ||
                decision === 'reject_documents' ||
                decision === 'request_documents';
            const postApprovalDocReviewStatuses = ['approved_pending_payment', 'completed', 'e_ticket_issued'];
            if (
                rejectFromApproved &&
                (st === 'approved_pending_payment' ||
                    (postApprovalDocReviewStatuses.includes(st) &&
                        decision !== 'reject_application'))
            ) {
                // Allow document requests/rejection after approval (manual or auto), including post-payment.
            } else if (!['submitted', 'waitlisted', 'pending_approval', 'revision_required', 'documents_requested'].includes(st)) {
                return cb(null, {
                    ok: false,
                    error:
            ...
            if (
                !rejectFromApproved &&
                ['cancelled', 'checked_in', 'completed', 'e_ticket_issued', 'certificate_issued'].includes(st)
            ) {
                return cb(null, {
                    ok: false,
            ...
            db.run(
                ...
                (e2) => {
                    if (e2) return cb(e2);
                    appEscalation.clearReviewEscalationOnDecision(db, rid, () => {
                    const prevStatus = st;
                    ...
                    function finishApprove(message, extra) {
                        cb(null, Object.assign({ ok: true, status: newStatus, message }, extra || {}));
                    }

                    if (approveAsVolunteer) {
                        if (!volunteerTicketFlow || !volunteerTicketFlow.assignVolunteerFromAdmin) {
                            return finishApprove(
                                'Application approved as volunteer, but volunteer module is not configured on the server.'
                            );
                        }
                        return volunteerTicketFlow.assignVolunteerFromAdmin(
                            db,
                            {
                                seminarId: row.seminar_id,
                                userId: row.user_id,
                                notes: 'Approved as volunteer from application review (₹0)',
                                duties: body && body.volunteerDuties ? String(body.volunteerDuties).trim() : null,
                            ...
                            (vErr, vResult) => {
                                ...
                                if (vResult && vResult.ticketIssued) {
                                    return finishApprove(
                                        'Approved as volunteer (₹0). Free e-ticket and certificates have been issued.',
                                        { ticketId: vResult.ticketId, feeType: 'volunteer' }
                                    );
                                }
                                return finishApprove(
                                    'Approved as volunteer (₹0). Doctor is assigned — free ticket issues automatically when registration is complete.',
                                    { feeType: 'volunteer' }
                                );
                            }
                        );
                    ...
                    if (newStatus === 'approved_pending_payment' && getOrCreatePendingOrder) {
                        seminarEvents.computeRegistrationPaymentAmount(db, rid, (payErr, pay) => {
                            if (payErr) return cb(payErr);
                            if (pay && pay.error) return cb(null, { ok: false, error: pay.error });
                            const defaultAmt =
                                pay && pay.amount != null
                                    ? pay.amount
                                    : row.seminar_price != null && Number(row.seminar_price) > 0
                                      ? Number(row.seminar_price)
                                      : 1500;
                            const amt =
                                Number.isFinite(feeAmountRaw) && feeAmountRaw >= 0 ? feeAmountRaw : defaultAmt;
                            getOrCreatePendingOrder(rid, amt, () => {
                                ...
                                finishApprove(
                                    'Application approved. Doctor can proceed to payment (₹' + amt + ').',
                                    { feeType: 'regular', feeAmount: amt, paymentBreakdown: pay && pay.breakdown }
                                );
                            });
                        });
                        return;
            ...
            const requiredLevel = appAuthority.clampLevel(row.review_required_level);
            if (actingReviewerId) {
                appAuthority.checkApplicationAuthorityByUserId(
                    db,
                    actingReviewerId,
                    row,
            ...
            if (requiredLevel > 1) {
                return cb(null, {
                    ...
                    error:
                        ...
                        appAuthority.requiredLevelLabel(requiredLevel) +
                        '. Escalate to higher authority or sign in as an authorized reviewer.'
                });
            }
            continueVerify();
        }
