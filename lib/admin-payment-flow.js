...
/** Doctor portal: DQR + enabled gateways (no cash, no mock). DQR first when available. */
function buildDoctorPaymentMethods(rows, upiCfg) {
    const filtered = buildAdminPaymentMethods(rows, upiCfg).filter(
        (m) => m.id !== 'cash' && m.id !== 'mock' && m.available !== false
    );
    const dqr = filtered.find((m) => m.id === 'dqr');
    const rest = filtered.filter((m) => m.id !== 'dqr');
    return dqr ? [dqr, ...rest] : filtered;
}
...
function initiateAdminPayment(db, deps, params, cb) {
    ...
    approveRegistrationForPayment(db, registrationId, portalTracking, notifEngine, (e0, ctx) => {
        ...
        const { reg } = ctx;
        const amount = resolvePaymentAmount(reg, { amount: ctx.amount, discountAmount });
