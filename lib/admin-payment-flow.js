...
function approveRegistrationForPayment(db, registrationId, portalTracking, notifEngine, cb) {
    db.get(
        ...
        (err, reg) => {
            ...
            const withAmount = (amount, breakdown) => {
                const done = () => cb(null, { reg, amount, paymentBreakdown: breakdown });
            ...
            // A pending order is authoritative for the payable amount: the admin may
            // have set a custom fee at approval time. Never recompute over it.
            db.get(
                `SELECT amount FROM orders WHERE registration_id = ? AND status = 'pending' ORDER BY id DESC LIMIT 1`,
                [registrationId],
                (ePend, pendingOrder) => {
                    if (ePend) return cb(ePend);
                    if (pendingOrder && pendingOrder.amount != null) {
                        return withAmount(Number(pendingOrder.amount) || 0, null);
                    }
                    ...
                    seminarEvents.computeRegistrationPaymentAmount(db, registrationId, (payErr, pay) => {
                        ...
                        withAmount(amount, pay && pay.breakdown);
                    });
                }
            );
        }
