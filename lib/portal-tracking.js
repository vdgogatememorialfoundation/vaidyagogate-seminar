...
const SEMINAR_TRACK_STEPS = [
    ...
    { key: 'certificate', title: 'E-certificate issued', icon: 'fa-certificate' },
...
function buildSeminarTimeline(registration, logs, extra) {
    ...
    const steps = SEMINAR_TRACK_STEPS.map((def) => {
        ...
        if (def.key === 'submitted') {
            ...
            if (rank >= 6 || status === 'certificate_issued') {
                state = 'completed';
                at = (log && log.created_at) || null;
            } else if (rank >= 5) {
                state = 'active';
            ...
            if (rank >= 7 || status === 'checked_in' || scannedAt) {
                state = 'completed';
                at = (log && log.created_at) || scannedAt || null;
                if (scannedAt && !log) {
                    desc = 'Checked in at the venue (QR scanned at entry).';
                }
...
function attachRegistrationTimelines(db, rows, cb) {
    ...
    out.forEach((row, idx) => {
        ...
        db.get(
            ...
            (eT, tRow) => {
                db.get(
                    `SELECT t.scan_time, t.is_scanned FROM tickets t
                     JOIN orders o ON o.id = t.order_id
                     WHERE o.registration_id = ? AND t.is_scanned = 1
                     ORDER BY t.scan_time DESC LIMIT 1`,
                    [row.id],
                    (eSc, scanRow) => {
                        fetchRegistrationLogs(db, row.id, (eL, logs) => {
                            const checkedInAt =
                                scanRow && (scanRow.is_scanned === 1 || scanRow.is_scanned === true)
                                    ? scanRow.scan_time
                                    : null;
