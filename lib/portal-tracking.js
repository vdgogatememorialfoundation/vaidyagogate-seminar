...
const SEMINAR_TRACK_STEPS = [
    ...
    { key: 'waitlisted', title: 'Waitlisted', icon: 'fa-clock' },
    { key: 'pending_approval', title: 'Application review', icon: 'fa-user-shield' },
...
const CASE_TRACK_STEPS = [
    ...
    { key: 'under_review', title: 'Application review', icon: 'fa-user-shield' },
    { key: 'approved_for_judging', title: 'Approved for judging', icon: 'fa-check-circle' },
    { key: 'judging', title: 'Judging in progress', icon: 'fa-gavel' },
...
function buildSeminarTimeline(registration, logs, extra) {
    ...
    const wasEverWaitlisted = status === 'waitlisted' || !!(logByKey.waitlisted && logByKey.waitlisted.created_at);
    const steps = SEMINAR_TRACK_STEPS.filter((def) => {
        if (def.key === 'waitlisted') return wasEverWaitlisted;
        return true;
    }).map((def) => {
        let state = 'pending';
        let at = null;
        ...
        } else if (def.key === 'waitlisted') {
            desc = (log && log.message) || 'Registration window closed. You are on the waitlist.';
            if (status === 'waitlisted') {
                state = 'active';
                at = (log && log.created_at) || null;
            } else if (rank >= 3) {
                state = 'completed';
                at = (log && log.created_at) || logByKey.pending_approval?.created_at || null;
                if (!desc) desc = 'Moved from waitlist to admin review.';
            }
        } else if (def.key === 'pending_approval') {
            desc = (log && log.message) || 'Your application is being reviewed.';
            if (rank >= 4) {
                state = 'completed';
                at = (log && log.created_at) || logByKey.approved_pending_payment?.created_at || null;
            } else if (rank === 3 || status === 'submitted') {
        ...
        } else if (def.key === 'checked_in' && status === 'expired') {
            state = 'cancelled';
            desc = 'E-ticket expired — the QR was not scanned at the venue before the event ended. Contact admin if you attended.';
...
function buildCaseTimeline(submission, logs) {
    ...
    const steps = CASE_TRACK_STEPS.map((def) => {
        ...
        if (def.key === 'submitted') {
            ...
            desc = log && log.message ? log.message : 'We are reviewing your application and uploaded files.';
        } else if (def.key === 'approved_for_judging') {
            if (log || ['approved_for_judging', 'judging', 'judged', 'selected'].includes(status)) {
                state = 'completed';
                at = log && log.created_at;
            }
            desc = log && log.message ? log.message : 'Approved — awaiting judging.';
        } else if (def.key === 'judging') {
            if (status === 'judging') {
                state = 'active';
                at = log && log.created_at;
                desc = 'Your case is with the judging panel.';
            } else if (['judged', 'selected'].includes(status)) {
                state = 'completed';
                at = log && log.created_at;
                desc = 'Judging complete.';
            } else if (status === 'approved_for_judging') {
                desc = 'Awaiting judging.';
            }
            if (!desc) {
                desc =
                    log && log.message
                        ...
                        db.get(
                            ...
                            (eC, certRow) => {
                                fetchRegistrationLogs(db, row.id, (eL, logs) => {
                                    ...
                                    out[idx].is_scanned = scanRow ? Number(scanRow.is_scanned) || 0 : 0;
                                    out[idx].scan_count = scanCt;
                                    out[idx].venue_checkin_complete = cancelPolicy.registrationHasVenueCheckin({
                                        status: row.status,
                                        is_scanned: scanRow && scanRow.is_scanned,
                                        scan_count: scanCt,
...
function registrationStatusToLog(status, prevStatus) {
    ...
    const map = {
        ...
        waitlisted: {
            key: 'waitlisted',
            label: 'Waitlisted',
            message: 'Registration window closed. Added to waitlist for admin review.'
        },
        pending_approval: {
        ...
        expired: { key: 'expired', label: 'E-ticket expired', message: 'Event ended without a venue scan; QR ticket is no longer valid.' }
    };
...
function caseStatusToLog(status, extra) {
    ...
    const map = {
        ...
        under_review: { key: 'under_review', label: 'Application review', message: 'We are reviewing your application.' },
        approved_for_judging: {
            key: 'approved_for_judging',
            label: 'Approved for judging',
            message: 'Approved — awaiting judging.'
        },
        judging: { key: 'judging', label: 'Judging in progress', message: extra || 'Your case is with the judging panel.' },
        judged: { key: 'judging', label: 'Judging complete', message: extra || 'Final review in progress.' },
        selected: {
            key: 'selected',
            label: 'Selected',
            message: extra || 'Selected — cash prize selection letter will follow. Winners declared on seminar day. Seminar registration is mandatory.'
        },
    ...
    if (st === 'judging' && extra) {
        return [{ key: 'judging', label: 'Judging in progress', message: extra }];
    }
    return map[st] ? [map[st]] : [];
}
