    ...
    const steps = SEMINAR_TRACK_STEPS.map((def) => {
        ...
        if (status === 'rejected' || status === 'cancelled') {
            ...
            const upcomingDesc = {
                ...
                checked_in: 'Venue check-in after you attend the seminar.'
            };
...
function attachRegistrationTimelines(db, rows, cb) {
    ...
    out.forEach((row, idx) => {
        ...
        db.get(
            `SELECT 1 FROM tickets t JOIN orders o ON o.id = t.order_id WHERE o.registration_id = ? AND o.status = 'success' LIMIT 1`,
            [row.id],
            (eT, tRow) => {
            fetchRegistrationLogs(db, row.id, (eL, logs) => {
                out[idx].timeline = buildSeminarTimeline(row, logs || [], {
                    hasTicket: regRank >= 4 && !!tRow
                });
