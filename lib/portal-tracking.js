...
function getPortalYear(db, cb) {
    db.get(`SELECT value FROM global_settings WHERE key = ?`, [PORTAL_YEAR_KEY], (e, row) => {
        if (e) return cb(e, new Date().getFullYear());
        const y = parsePortalYearValue(row && row.value);
        cb(null, y || new Date().getFullYear());
    });
}

function setPortalYear(db, upsertGlobalSetting, year, cb) {
    const y = parseInt(year, 10);
    if (!Number.isInteger(y) || y < 2000 || y > 2100) {
        return cb(new Error('Invalid portal year'));
    }
    upsertGlobalSetting(PORTAL_YEAR_KEY, JSON.stringify(y), cb);
}
...
function fetchRegistrationLogs(db, registrationId, cb) {
    db.all(
        `SELECT step_key, label, message, created_at FROM registration_status_log
         WHERE registration_id = ? ORDER BY created_at ASC, id ASC`,
        [registrationId],
        cb
    );
}
...
function fetchCaseLogs(db, submissionId, cb) {
    db.all(
        `SELECT step_key, label, message, created_at FROM case_status_log
         WHERE submission_id = ? ORDER BY created_at ASC, id ASC`,
        [submissionId],
        cb
    );
}
...
function buildSeminarTimeline(registration, logs, extra) {
    ...
    const steps = SEMINAR_TRACK_STEPS.map((def) => {
        ...
        if (def.key === 'submitted') {
            ...
            const log = logByKey.pending_approval;
            if (['pending_approval', 'submitted'].includes(status) && !logByKey.approved_pending_payment) {
                state = status === 'pending_approval' || status === 'submitted' ? 'active' : 'pending';
                if (log) {
                    state = 'completed';
                    at = log.created_at;
                }
            } else if (log || !['submitted', 'rejected', 'cancelled'].includes(status)) {
                state = 'completed';
                at = (log && log.created_at) || logByKey.approved_pending_payment?.created_at;
            }
            desc = log && log.message ? log.message : 'Admin is verifying your application.';
        } else if (def.key === 'approved_pending_payment') {
            const log = logByKey.approved_pending_payment;
            if (log) {
                state = ['completed', 'checked_in'].includes(status) ? 'completed' : 'active';
                at = log.created_at;
                desc = log.message || 'Approved. Complete payment to confirm your seat.';
            } else if (['approved_pending_payment'].includes(status)) {
                state = 'active';
                desc = 'Approved. Complete payment to confirm your seat.';
            }
        } else if (def.key === 'completed') {
            const log = logByKey.completed;
            if (log || ['completed', 'checked_in'].includes(status)) {
                state = 'completed';
                at = (log && log.created_at) || null;
                desc = log && log.message ? log.message : 'Payment received successfully.';
            }
        } else if (def.key === 'ticket') {
            const log = logByKey.ticket;
            if (log || hasTicket) {
                state = 'completed';
                at = (log && log.created_at) || null;
                desc = log && log.message ? log.message : 'Your e-ticket has been generated.';
            } else if (['completed', 'checked_in'].includes(status)) {
                state = 'active';
                desc = 'Ticket will appear after payment is confirmed.';
            }
        } else if (def.key === 'checked_in') {
            const log = logByKey.checked_in;
            if (log || status === 'checked_in') {
                state = 'completed';
                at = (log && log.created_at) || null;
                desc = log && log.message ? log.message : 'Checked in at the venue.';
            }
        }

        if (status === 'rejected' || status === 'cancelled') {
            if (def.key === 'submitted') state = 'completed';
            else if (def.key !== 'submitted') state = 'pending';
        }

        return {
            key: def.key,
            title: (logByKey[def.key] && logByKey[def.key].label) || def.title,
            icon: def.icon,
            state,
            at,
            desc
...
function buildCaseTimeline(submission, logs) {
    ...
    const steps = CASE_TRACK_STEPS.map((def) => {
        ...
        if (def.key === 'submitted') {
            ...
            if (log || ['under_review', 'approved_for_judging', 'judging', 'selected'].includes(status)) {
            ...
            if (log || ['approved_for_judging', 'judging', 'selected'].includes(status)) {
                state = 'completed';
                at = log && log.created_at;
            }
            desc = log && log.message ? log.message : 'Cleared for judge assignment.';
        } else if (def.key === 'judging') {
            if (judges > 0 || status === 'judging' || status === 'selected') {
                state = status === 'judging' && !lockedScores ? 'active' : 'completed';
                at = log && log.created_at;
            }
            desc =
                log && log.message
            ...
            if (lockedScores > 0) {
                state = 'completed';
                at = log && log.created_at;
                desc = lockedScores + ' score(s) locked in.';
...
function attachRegistrationTimelines(db, rows, cb) {
    ...
    out.forEach((row, idx) => {
        db.get(`SELECT 1 FROM tickets t JOIN orders o ON o.id = t.order_id WHERE o.registration_id = ? AND o.status = 'success' LIMIT 1`, [row.id], (eT, tRow) => {
            fetchRegistrationLogs(db, row.id, (eL, logs) => {
                out[idx].timeline = buildSeminarTimeline(row, logs || [], { hasTicket: !!tRow });
