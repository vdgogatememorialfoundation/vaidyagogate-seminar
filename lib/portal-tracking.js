...
function buildCaseTimeline(submission, logs) {
    ...
    const steps = CASE_TRACK_STEPS.map((def) => {
        ...
        if (def.key === 'submitted') {
            ...
            if (['judging', 'judged', 'selected'].includes(status)) {
                if (status === 'judged' || allScored) {
                    state = 'completed';
                    desc = 'Judging completed.';
                } else if (status === 'judging') {
                    state = 'active';
                    desc = lockedScores > 0 ? 'Judging in progress.' : 'Your case is with the judging panel.';
                } else {
                    state = 'completed';
                    desc = 'Judging completed.';
                }
            ...
            const allScored = judges > 0 && lockedScores >= judges;
            if (allScored || status === 'judged') {
                state = 'completed';
                at = log && log.created_at;
                desc = 'All judge scores have been submitted.';
            } else if (lockedScores > 0) {
                state = 'active';
                desc = 'Scores are being finalised.';
            } else {
                desc = 'Scores will appear here once judges submit.';
            }
