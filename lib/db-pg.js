...
const { ensureAuxiliaryTables, listMissingAuxTables } = require('./extended-schema-pg');
...
async function applyPostgresSchema() {
    ...
    if (!missingBefore.length) {
        await ensureAuxiliaryTables(queryWithRetry, isIgnorablePgError);
    ...
    if (!fs.existsSync(schemaPath)) {
        const left = await ensureMissingCoreTables();
        schemaBootstrapped = left.length === 0;
        if (left.length) schemaReady = null;
        return;
    }
    ...
    try {
        ...
        const stillMissing = await ensureMissingCoreTables();
        const auxMissing = await ensureAuxiliaryTables(queryWithRetry, isIgnorablePgError);
...
function ensureSchemaReady() {
    if (!schemaReady || (!schemaBootstrapped && process.env.VERCEL)) {
        schemaReady = applyPostgresSchema().then(async () => {
            const missing = await listMissingCoreTables();
            if (missing.length) {
                schemaBootstrapped = false;
                schemaReady = null;
            ...
            if (auxMissing.length) {
                console.warn('[pg-schema] auxiliary tables still missing after ensure:', auxMissing.join(', '));
            }
            try {
                await queryWithRetry('ALTER TABLE users ADD COLUMN IF NOT EXISTS last_login_at TIMESTAMPTZ', [], 1);
            } catch (colErr) {
...
async function runEnsureAuxiliaryTables() {
    return ensureAuxiliaryTables(queryWithRetry, isIgnorablePgError);
}
