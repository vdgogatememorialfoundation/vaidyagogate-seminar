...
function getPool() {
    if (!pool) {
        ...
        pool = new Pool({
            ...
            connectionTimeoutMillis: process.env.VERCEL ? 25000 : 15000,
...
async function queryWithRetry(sql, params, attempts) {
    const n = attempts || (process.env.VERCEL ? 4 : 2);
    let lastErr;
    for (let i = 0; i < n; i++) {
        try {
...
function createPgDb() {
    const api = {
        ...
        connect(callback) {
            queryWithRetry('SELECT 1', [], process.env.VERCEL ? 4 : 2)
...
module.exports = { createPgDb, getPool, ensureSchemaReady, getSchemaApplyErrors, queryWithRetry };
