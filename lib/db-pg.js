/**
 * PostgreSQL (Supabase) — sqlite3-compatible callback API for minimal app changes.
 */
...
function getPool() {
    if (!pool) {
        ...
        pool = new Pool({
            ...
            max: Math.max(2, Math.min(50, parseInt(process.env.PG_POOL_MAX, 10) || 10)),
...
async function applyPostgresSchema() {
    ...
    if (schemaUrl && /-pooler\./i.test(resolveDatabaseUrl() || '')) {
        console.log('[pg-schema] applying DDL via direct Supabase connection (non-pooler)');
    }
...
async function runEnsureBookOrdersColumns() {
    return ensureBookOrdersColumns(queryWithRetry, isIgnorablePgError);
}
...
module.exports = {
    ...
    ensureBookOrdersColumns: runEnsureBookOrdersColumns,
