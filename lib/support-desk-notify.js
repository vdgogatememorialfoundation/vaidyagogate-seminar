...
function notifyAgentTicketUserReply(db, agentId, ticketRow, messagePreview, refToken, cb) {
    if (!agentId || !ticketRow) return cb && cb(null, { skipped: true });
    const tid = ticketRow.ticket_id || ticketRow.tracking_id || '';
    const link = notifEngine.publicBaseUrl().replace(/\/$/, '') + '/support';
    const title = 'Ticket reply: ' + tid;
    const body =
        (ticketRow.subject || 'Support ticket') +
        '\n\n' +
        String(messagePreview || '').slice(0, 500) +
        (refToken ? '\n\nRef: ' + refToken : '');
    pushInbox(
        db,
        { userId: agentId, type: 'ticket_reply', title, body, link, refId: tid },
        () => {
            notifyAgentByEmail(
                db,
                agentId,
...
function notifyAuthorityAgentsTicketEscalated(db, ticketRow, requiredLevel, note, cb) {
    const supportAuthority = require('./support-authority');
    const minLevel = supportAuthority.clampLevel(requiredLevel);
    const tid = ticketRow.ticket_id || ticketRow.tracking_id || '';
    const link = notifEngine.publicBaseUrl().replace(/\/$/, '') + '/support';
    const title = 'Escalated ticket needs ' + supportAuthority.levelLabel(minLevel) + ': ' + tid;
    const body =
        (ticketRow.subject || 'Support ticket') +
        (note ? '\n\n' + note : '') +
        '\n\nRequired authority: ' +
        supportAuthority.requiredLevelLabel(minLevel);
    db.all(
