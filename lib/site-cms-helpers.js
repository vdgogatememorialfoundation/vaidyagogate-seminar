...
const DEFAULT_SITE_MENU = [
    ...
    { label: 'Certificate', section: '', href: '/verify-certificate', visible: true, order: 6 },
...
function mergeLegalPagesWebsiteMenu(menuPages, legalPages) {
    ...
    const list = normalizeLegalPages(legalPages);
    list.forEach((page) => {
        const key = legalPageMenuKey(page.id);
        if (out[key] !== false) out[key] = true;
    });
    if (list.length && out.legal !== false) out.legal = true;
    return out;
}
