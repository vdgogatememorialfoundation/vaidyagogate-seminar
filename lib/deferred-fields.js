...
const NON_DEFERRABLE_KEYS = new Set(['fname', 'lname', 'email', 'phone', 'qual']);

/** Keys whose value lives in a different form_data key (uploaded file path). */
const VALUE_KEY_ALIAS = { certificate: 'certificate_path' };

function valueKeyFor(key) {
    return VALUE_KEY_ALIAS[key] || key;
}
...
function settlePending(formData) {
    const fd = { ...parseFormData(formData) };
    const still = pendingKeys(fd).filter((k) => !hasValue(fd[valueKeyFor(k)]));
...
function pendingFieldDefs(formData, fields) {
    ...
    return pend.map((k) => {
        ...
        return {
            key: k,
            label: f.label || k,
            type: k === 'certificate' ? 'file' : f.type || 'text',
