...
function mergeRegistrationFields(globalFields, overrideFields) {
    ...
    return sanitizeRegistrationFormFields(merged);
}

function normalizeQualOptions(options) {
    const src = Array.isArray(options) && options.length ? options : DEFAULT_QUAL_OPTIONS;
    return src
        .map((o) => ({
            value: String(o.value != null ? o.value : o.label || '').trim(),
            label: String(o.label != null ? o.label : o.value || '').trim()
        }))
        .filter((o) => o.value);
}
...
function buildConfigPayload(fields, meta) {
    ...
    const out = {
        version: 1,
        fields: sanitizeRegistrationFormFields(fields || [])
    };
