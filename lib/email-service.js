/**
 * Email delivery — Sender.net HTTPS API (primary) with Zoho / generic SMTP standby.
 * Env: EMAIL_API_PROVIDER=sender, SENDER_NET_API_TOKEN, ZOHO_* / SMTP_*
 */
let nodemailer = null;
try {
...
async function sendEmail(to, subject, html, opts) {
    ...
    if (httpTransport.isHttpEmailConfigured(overrides)) {
        ...
        if (smtpCfg && nodemailer) {
            console.warn(
                '[email]',
                httpResult.provider || 'HTTP API',
                'failed — trying Zoho SMTP standby:',
                httpResult.error
            );
    ...
    if (smtpOnly.ok || !isConnectionLikeError(smtpOnly.error)) {
        return smtpOnly;
    }
