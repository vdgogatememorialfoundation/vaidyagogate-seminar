...
function collectToAddresses(body) {
    ...
    if (body['mailparser-inbox']) add(body['mailparser-inbox']);
...
function normalizeInboundPayload(raw) {
    ...
    let from = asString(
        body.from ||
            ...
            body.from_email ||
            body['email-from'] ||
            (body.headers && body.headers.from)
    );
    let subject = asString(body.subject || body.Subject || body.email_subject || body['email-subject'] || '');
    let text = asString(
        body.text ||
            body.plain ||
            body['text/plain'] ||
...
function detectProvider(body) {
    if (body['mailparser-inbox'] || body.mailparser || body['email-from'] || body.email_from) return 'mailparser';
