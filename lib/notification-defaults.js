...
function formatGatewayLabel(gw) {
    const g = String(gw || '').toLowerCase();
    if (!g) return '';
    if (g.startsWith('dqr_razorpay')) return 'Razorpay DQR (UPI scan)';
    if (g.startsWith('razorpay')) return 'Razorpay';
    if (g === 'cash') return 'Cash';
    if (g === 'mock') return 'Test / mock';
    if (g.startsWith('dqr_upi')) return 'UPI QR (manual confirm)';
    return g.replace(/_/g, ' ');
}

/** Rich HTML block injected into payment / refund / cancellation emails. */
function buildTransactionDetailsHtml(vars, eventKey) {
    ...
    add('Application no.', vars.application_no);
    add('Seminar', vars.seminar_name);
    add('Participant ID', vars.user_id_string);
    ...
    const showRefund = ['REFUND_INITIATED', 'REFUND_COMPLETED', 'REGISTRATION_CANCELLED'].includes(key);

    if (showPayment) {
        add('Order reference', vars.order_id_string || vars.payment_id);
        add('Amount paid', vars.payment_amount != null && vars.payment_amount !== '' ? '₹' + vars.payment_amount : '');
        add('Payment gateway', vars.payment_gateway_label || vars.payment_gateway);
        add('Gateway transaction ID', vars.provider_transaction_id);
        add('Paid on', vars.payment_date_formatted);
        add('E-ticket ID', vars.ticket_id);
    }
    if (showRefund) {
        add('Refund amount', vars.refund_amount != null && vars.refund_amount !== '' ? '₹' + vars.refund_amount : '');
        add(
            'Refund percentage',
            vars.refund_percent != null && vars.refund_percent !== '' ? String(vars.refund_percent) + '%' : ''
        );
        add('Refund status', vars.refund_status_label || vars.refund_status);
        add('Razorpay / gateway refund ID', vars.provider_refund_id);
        add('Bank UTR / RRN', vars.bank_utr);
        add('Refund gateway', vars.refund_gateway_label || vars.refund_gateway);
    }
    if (key === 'REGISTRATION_CANCELLED') {
        add('Your cancellation reason', vars.cancellation_reason);
        add('Office note', vars.admin_notes);
    }
    if (!rows.length) return '';
    return (
        '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:14px 16px;margin:16px 0;">' +
        '<p style="margin:0 0 10px;font-weight:700;color:#1e3a8a;">Transaction summary</p>' +
        rows.join('') +
        '</div>'
    );
}

function emailFactBlock(rows) {
    const trs = (rows || [])
        .filter((r) => r && r.value != null && String(r.value).trim())
        .map(
            (r) =>
                '<tr><td style="padding:5px 14px 5px 0;color:#64748b;vertical-align:top;white-space:nowrap;">' +
                escEmailVal(r.label) +
                '</td><td style="padding:5px 0;color:#0f172a;font-weight:600;">' +
                escEmailVal(r.value) +
                '</td></tr>'
        )
        .join('');
    if (!trs) return '';
    return (
        '<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin:18px 0;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;">' +
        '<tr><td style="padding:14px 16px;"><table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">' +
        trs +
        '</table></td></tr></table>'
    );
}
...
function wrapHtml(title, body, opts) {
    opts = opts || {};
    const headline = escEmailVal(opts.headline || title);
    const subtitle = opts.subtitle ? escEmailVal(opts.subtitle) : '';
    const preheader = escEmailVal(opts.preheader || opts.headline || title);
    const seminarLine = escEmailVal(SEMINAR);
    return (
        ...
        preheader +
        '</div>' +
        '<table role="presentation" class="vgmf-email-wrap" cellspacing="0" cellpadding="0" border="0" width="100%" style="background:#f1f5f9;padding:24px 12px;">' +
        '<tr><td align="center">' +
        '<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">' +
        '<tr><td style="background:linear-gradient(135deg,#1e3a8a 0%,#0f766e 100%);padding:22px 28px;">' +
        '<p style="margin:0;font-size:12px;letter-spacing:0.04em;text-transform:uppercase;color:#bfdbfe;">Vaidya Gogate Memorial Foundation</p>' +
        ...
        '</h1>' +
        (subtitle
            ? '<p style="margin:10px 0 0;font-size:15px;line-height:1.5;color:#64748b;">' + subtitle + '</p>'
            : '') +
        '</td></tr>' +
        '<tr><td style="padding:6px 28px 28px;background:#ffffff;font-size:15px;line-height:1.65;color:#334155;">' +
        body +
        '</td></tr>' +
        '<tr><td style="padding:18px 28px 22px;background:#f8fafc;border-top:1px solid #e2e8f0;">' +
        '<p style="margin:0;font-size:13px;line-height:1.55;color:#64748b;">Regards,<br>' +
        TEAM +
        '</p>' +
        '<p style="margin:12px 0 0;font-size:11px;line-height:1.5;color:#94a3b8;">This is an automated message from the seminar portal. The email subject is only a short label — please read the message above for full details.</p>' +
        '</td></tr></table></td></tr></table></body></html>'
    );
}

/** Ensure legacy or admin-edited fragments still get the full branded layout. */
function finalizeEmailHtml(html, subject) {
    const raw = String(html || '').trim();
    if (!raw) {
        return wrapHtml('Notification', '<p>Dear {{full_name}},</p><p>Please see the update below.</p>', {
            headline: String(subject || 'Notification').trim() || 'Notification'
        });
    }
    if (/vgmf-email-wrap/i.test(raw)) return raw;
    const title = String(subject || 'Notification').trim() || 'Notification';
    return wrapHtml(title, raw, { headline: title, preheader: title });
}

function otpEmailIntro(introLine) {
    return (
        '<p>Dear Participant,</p>' +
        '<p><strong>Your one-time verification code</strong></p>' +
        '<p>' +
        introLine +
        '</p>' +
        '<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:20px 0;">' +
        '<tr><td align="center" style="background:#ecfdf5;border:2px dashed #0f766e;border-radius:12px;padding:18px 32px;">' +
        '<p style="margin:0 0 6px;font-size:12px;color:#0f766e;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;">4-digit OTP</p>' +
        '<p style="margin:0;font-size:32px;font-weight:800;letter-spacing:8px;color:#0f172a;">{{otp_code}}</p>' +
        '</td></tr></table>' +
...
const DEFAULT_TEMPLATES = [
    ...
    {
        event_key: 'CHECKIN_OTP',
        channel: 'both',
        email_subject: 'Your check-in OTP — {{event_name}}',
        email_html: wrapHtml(
            'Check-in OTP',
            '<p>Dear {{full_name}},</p>' +
                '<p>Use this one-time code at the venue to check in for <strong>{{event_name}}</strong> ({{seminar_name}}).</p>' +
                '<p>Seminar day: <strong>{{day_title}}</strong></p>' +
                '<div style="margin:18px 0;padding:16px;border:1px solid #e2e8f0;border-radius:12px;text-align:center;background:#f8fafc;">' +
                '<div style="font-size:12px;letter-spacing:2px;color:#475569;">CHECK-IN OTP</div>' +
                '<div style="font-size:32px;font-weight:700;letter-spacing:6px;color:#0f172a;">{{otp_code}}</div>' +
                '<div style="font-size:12px;color:#64748b;margin-top:6px;">Valid for {{expires_minutes}} minutes</div>' +
                '</div>' +
                '<p>Enter this code at the venue check-in desk when requested. Do not share it with anyone.</p>' +
                '<p>If you did not request this OTP, you can ignore this email.</p>'
        ),
        whatsapp_body:
            '🔐 Check-in OTP\n' +
            'For {{event_name}} ({{seminar_name}}): your one-time code is {{otp_code}}. Valid for {{expires_minutes}} minutes.'
    },
    {
    ...
    },
    {
        ...
        channel: 'both',
        email_subject: 'Welcome — your {{portal_name}} account is ready',
        email_html: wrapHtml(
            'Welcome to the portal',
            '<p style="font-size:18px;font-weight:700;color:#0f766e;margin:0 0 12px;">Welcome, {{full_name}}!</p>' +
                '<p>Your <strong>{{portal_name}}</strong> account has been created for the ' +
                SEMINAR +
                '.</p>' +
                ...
                '<p>{{portal_welcome_blurb}}</p>' +
                emailCtaButton('{{account_portal_url}}', '{{portal_button_label}}')
        ),
        whatsapp_body:
            '🎉 Account created\nHello {{first_name}},\n' +
            'Your {{portal_name}} account is ready.\n' +
            '🆔 User ID: {{user_id_string}}\n' +
            '📧 Email: {{email}}\n' +
            '🔑 Password: {{temporary_password}}\n' +
            'Sign in: {{account_portal_url}}'
    },
    {
        ...
        email_html: wrapHtml(
            ...
            '<p>Dear {{full_name}},</p><p>Your application for the ' +
                ...
                emailFactBlock([
                    { label: 'Application ID', value: 'SEM-{{application_no}}' },
                    { label: 'Status', value: 'Submitted' }
                ]) +
                '<p>Our team will review your application shortly. You will receive another email when the status changes.</p>' +
                emailCtaButton('{{portal_login_url}}', 'Track application'),
            { subtitle: 'We received your seminar registration application.' }
        ),
        whatsapp_body:
    ...
    },
    {
        ...
        email_html: wrapHtml(
            'Payment successful',
            '<p>Dear {{full_name}},</p><p>We have successfully received your payment for <strong>{{seminar_name}}</strong>.</p>' +
                '{{transaction_details_html}}' +
                '<p>Your registration is now <strong>confirmed</strong>. Your e-ticket will be available in the doctor portal.</p>' +
                emailCtaButton('{{portal_login_url}}', 'View payment & e-ticket')
        ),
        whatsapp_body:
            '💳 Payment successful\nHello {{first_name}},\nSeminar: {{seminar_name}}\nApplication: {{application_no}}\nAmount: ₹{{payment_amount}}\nOrder: {{order_id_string}}\nGateway: {{payment_gateway_label}}\nTxn: {{provider_transaction_id}}\nTicket: {{ticket_id}}\nStatus: Paid{{whatsapp_group_line}}'
    },
    {
        ...
        email_html: wrapHtml(
            ...
            '<p>Dear {{full_name}},</p><p>Your e-ticket for the ' +
                SEMINAR +
                ' has been generated successfully.</p><p><strong>Ticket Details:</strong><br>Ticket ID: {{ticket_id}}<br>Application: {{application_no}}<br>Participant ID: {{user_id_string}}</p>' +
                '<p>Your personal printable e-ticket is <strong>attached to this email</strong>. Please carry it (or the QR on your phone) for event entry.</p>' +
                emailCtaButton('{{ticket_pdf_url}}', 'Open printable ticket online') +
    ...
    },
    {
        ...
        email_html: wrapHtml(
            ...
            '<p>Dear {{full_name}},</p><p>Congratulations! Your application for the ' +
                ...
                ' has been approved.</p>' +
                emailFactBlock([
                    { label: 'Application ID', value: '{{application_no}}' },
                    { label: 'Status', value: 'Approved' }
                ]) +
                '<p>Complete payment in the doctor portal if you have not already done so. We look forward to welcoming you.</p>' +
                emailCtaButton('{{portal_login_url}}', 'Open doctor portal'),
            { subtitle: 'Your seminar application is approved.' }
        ),
        whatsapp_body:
    ...
    },
    {
        ...
        email_html: wrapHtml(
            'Payment pending',
            '<p>Dear {{full_name}},</p><p>Your seminar application <strong>{{application_no}}</strong> has been approved. Payment of <strong>₹{{payment_amount}}</strong> is required to confirm your registration.</p>' +
                '<p>Use the button below to pay securely (sign in with your doctor portal account if prompted):</p>' +
                emailCtaButton('{{payment_link}}', 'Pay seminar fee now') +
                '<p style="font-size:0.9rem;color:#64748b;margin-top:16px;">You can also open the doctor portal and pay from <strong>Make payments</strong>.</p>' +
                emailCtaButton('{{portal_login_url}}', 'Open doctor portal')
        ),
        whatsapp_body:
            '⏳ Payment Pending\nHello {{first_name}},\nYour seminar application {{application_no}} is approved.\nAmount: ₹{{payment_amount}}\nPay here: {{payment_link}}\nComplete payment to confirm your seat.'
    },
    {
    ...
    {
        event_key: 'REGISTRATION_CANCELLED',
        channel: 'both',
        email_subject: 'Registration cancelled — {{seminar_name}} ({{application_no}})',
        email_html: wrapHtml(
            'Registration cancelled',
            '<p>Dear {{full_name}},</p>' +
                '<p>Your seminar registration has been <strong>cancelled</strong> as requested or as approved by the seminar office.</p>' +
                '{{transaction_details_html}}' +
                '<p>If a refund applies, you will receive a separate email when it is initiated and when it completes. Track live status under <strong>Refund tracking</strong> in the doctor portal.</p>' +
                '<p>Questions? Contact {{admin_contact}}.</p>' +
                emailCtaButton('{{refund_tracking_url}}', 'Track refund in portal')
        ),
        whatsapp_body:
            'Hello {{first_name}}, registration {{application_no}} for {{seminar_name}} is cancelled.\nPaid: Rs {{payment_amount}} · Order {{order_id_string}}\nRefund eligible: Rs {{refund_amount}} ({{refund_percent}}%)\nOffice: {{admin_notes}}\nTrack refund: {{refund_tracking_url}}'
    },
    {
        event_key: 'REFUND_INITIATED',
        channel: 'both',
        email_subject: 'Refund initiated — {{seminar_name}} ({{application_no}})',
        email_html: wrapHtml(
            'Refund initiated',
            '<p>Dear {{full_name}},</p>' +
                '<p>A refund of <strong>₹{{refund_amount}}</strong> has been <strong>initiated</strong> for your cancelled seminar registration.</p>' +
                '{{transaction_details_html}}' +
                '<p>It may take 5–10 business days to reflect in your bank or UPI app depending on Razorpay and your bank.</p>' +
                '<p>Contact {{admin_contact}} if you do not see the credit after this period.</p>' +
                emailCtaButton('{{refund_tracking_url}}', 'Track refund live')
        ),
        whatsapp_body:
            'Refund initiated: Rs {{refund_amount}} for {{application_no}} ({{seminar_name}}).\nOriginal payment: Rs {{payment_amount}} · Order {{order_id_string}}\nRazorpay refund ID: {{provider_refund_id}}\nBank UTR: {{bank_utr}}\nStatus: {{refund_status_label}}\nTrack: {{refund_tracking_url}}'
    },
    {
        event_key: 'REFUND_COMPLETED',
        channel: 'both',
        email_subject: 'Refund completed — {{seminar_name}} ({{application_no}})',
        email_html: wrapHtml(
            'Refund completed',
            '<p>Dear {{full_name}},</p>' +
                '<p>Your refund of <strong>₹{{refund_amount}}</strong> for application <strong>{{application_no}}</strong> has been <strong>processed successfully</strong>.</p>' +
                '{{transaction_details_html}}' +
                '<p>Please check your bank or UPI statement. If anything looks incorrect, contact {{admin_contact}}.</p>' +
                emailCtaButton('{{refund_tracking_url}}', 'View refund receipt in portal')
        ),
        whatsapp_body:
            'Refund completed: Rs {{refund_amount}} for {{application_no}} ({{seminar_name}}).\nRazorpay refund ID: {{provider_refund_id}}\nBank UTR: {{bank_utr}}\nOriginal order: {{order_id_string}}\nShare the UTR with your bank if the credit is not visible. Support: {{admin_contact}}'
    },
    {
        event_key: 'SEMINAR_ALUMNI_INVITE',
        channel: 'both',
        email_subject: 'Registration open — {{seminar_name}}',
        email_html: wrapHtml(
            'Returning participants invited',
            '<p>Dear {{full_name}},</p>' +
                '<p>Thank you for being part of our National Seminar programme{{previous_seminar_years}}.</p>' +
                '<p>Registration is now <strong>open</strong> for <strong>{{seminar_name}}</strong>.</p>' +
                emailFactBlock([
                    { label: 'Event date', value: '{{seminar_date}}' },
                    { label: 'Base registration fee', value: '₹{{payment_amount}}' },
                    { label: 'Past programmes', value: '{{previous_seminar_titles}}' }
                ]) +
                '<p>Sign in to the doctor portal to apply and complete payment.</p>' +
                emailCtaButton('{{portal_login_url}}', 'Register for this seminar'),
            { subtitle: 'Invitation for past seminar participants' }
        ),
        whatsapp_body:
            'Hello {{first_name}}, registration is open for {{seminar_name}} ({{seminar_date}}). ' +
            'Thank you for attending our past seminar{{previous_seminar_years}}. Register now: {{portal_login_url}}'
    },
    {
    ...
    },
    {
        ...
        email_html: wrapHtml(
            'Check-in confirmed',
            '<p>Dear {{full_name}},</p><p>Your check-in for <strong>{{event_name}}</strong> has been successfully completed.</p>' +
                '<p><strong>Details:</strong><br>Event: {{event_name}}<br>Participant ID: {{user_id_string}}<br>Check-In Time: {{check_in_time}}<br>Status: Checked-In</p>' +
                '<p>Thank you for joining us. We hope you have a valuable learning experience.</p>'
        ),
        whatsapp_body:
            '✅ Check-In Confirmed!\nHello {{first_name}},\nYour check-in for {{event_name}} has been successfully completed.\n' +
    ...
    },
    {
        ...
        email_html: wrapHtml(
            'Certificate ready',
            '<p>Dear {{full_name}},</p><p>Congratulations! Your E-Certificate for <strong>{{event_name}}</strong> has been generated successfully.</p>' +
                '<p><strong>Certificate Details:</strong><br>Event: {{event_name}}<br>Certificate ID: {{certificate_id}}<br>Participant ID: {{user_id_string}}</p>' +
                '<p>You can now log in to the portal and download your certificate.</p>' +
                emailCtaButton('{{certificate_url}}', 'Download certificate')
        ),
        whatsapp_body:
            '🎓 E-Certificate Generated!\nHello {{first_name}},\nYour participation certificate for {{event_name}} is ready.\n' +
    ...
    },
    {
        ...
        email_html: wrapHtml(
            ...
            '<p>Dear {{full_name}},</p><p>We received your support ticket <strong>{{ticket_id}}</strong> regarding <strong>{{ticket_subject}}</strong>.</p>' +
                '<p>{{ticket_message}}</p>' +
                '<p>Our team will reply in the doctor portal. Sign in to view and reply here:</p>' +
