...
function inferActivityFromPath(path) {
    ...
    if (p.includes('/admin')) return { kind: 'admin', stepLabel: 'Admin' };
...
function recordHeartbeat(db, req, body, cb) {
    ...
    ensureSchema(db, (schemaErr) => {
        ...
        db.get(`SELECT location_label, geo_json FROM site_visitor_sessions WHERE session_id = ?`, [sessionId], (geoLookupErr, prev) => {
            if (!geoLookupErr && prev && prev.geo_json) {
                return upsert(prev.location_label, prev.geo_json);
            }
    ...
    ensureSchema(db, (e) => {
        ...
        db.all(
            `SELECT session_id, user_id, user_label, client_ip, location_label, geo_json,
             ...
             WHERE last_seen >= ?
               AND (LOWER(IFNULL(current_path,'')) LIKE '%/doctor%'
                    OR LOWER(IFNULL(activity_kind,'')) IN ('seminar_apply','payment','browse_seminars','track_applications','doctor_portal'))
            ...
            (err, rows) => {
                ...
                const now = Date.now();
                const allSessions = (rows || []).filter(isDoctorSeminarSession).map((r) => enrichRow(r, now));
