...
const LIVE_RADAR_ACTIVE_SEC = 90;
const LIVE_RADAR_WINDOW_MIN = 3;
...
function recordHeartbeat(db, req, body, cb) {
    ...
    if (body && body.goodbye && sessionId.length >= 8) {
        return ensureSchema(db, (schemaErr) => {
            if (schemaErr) return cb && cb(schemaErr);
            db.run(`DELETE FROM site_visitor_sessions WHERE session_id = ?`, [sessionId], (delErr) => {
                if (delErr) return cb && cb(delErr);
                if (liveRadarHub.subscriberCount() > 0) {
                    getLiveRadarSnapshot(db, { minutes: LIVE_RADAR_WINDOW_MIN }, (snapErr, snap) => {
                        if (!snapErr && snap) liveRadarHub.broadcast(snap);
                    });
                }
                cb(null, { sessionId, goodbye: true });
            });
        });
    }
    ...
    ensureSchema(db, (schemaErr) => {
        ...
        const patchGeo = (locationLabel, geoJson) => {
            const label =
                locationLabel != null && String(locationLabel).trim() !== ''
                    ? String(locationLabel)
                    : null;
            const geo =
                geoJson != null && String(geoJson).trim() !== '' ? String(geoJson) : null;
            if (!label && !geo) return;
            db.run(
                `UPDATE site_visitor_sessions SET
                ...
                (patchErr) => {
                    if (patchErr) {
                        console.warn('[site-visitors] geo patch failed:', patchErr.message);
                        return;
                    }
                    if (liveRadarHub.subscriberCount() > 0) {
                        getLiveRadarSnapshot(db, { minutes: LIVE_RADAR_WINDOW_MIN }, (snapErr, snap) => {
        ...
        db.get(`SELECT location_label, geo_json FROM site_visitor_sessions WHERE session_id = ?`, [sessionId], (geoLookupErr, prev) => {
            ...
            const skipLookup = !geoLookupErr && !ipGeo.needsGeoLookup(prevGeo, clientGeo);

            finishGeo(prevLabel, prevGeo);

            if (skipLookup || (!ip && !clientGeo)) return;

            ipGeo.resolveVisitorGeo({ ip, clientGeo, prevGeo }, (geoErr, geo) => {
                if (geoErr) {
                    console.warn('[site-visitors] resolveVisitorGeo failed:', geoErr.message || geoErr);
                    return;
                }
                if (!geo) return;
                const label =
                    geo.label || ipGeo.buildGeoLabel(geo.city, geo.region, geo.country) || prevLabel || null;
                const geoJson = JSON.stringify(geo);
                const hasData =
                    geo.label || geo.city || geo.country || (geo.lat != null && geo.lon != null);
                if (!hasData) return;
                patchGeo(label, geoJson);
            });
        });
    });
}
...
function enrichRow(r, now) {
    ...
    return {
        ...
        ageSec,
        pulse,
        isActive: ageSec != null && ageSec <= LIVE_RADAR_ACTIVE_SEC,
...
function getLiveRadarSnapshot(db, opts, cb) {
    const minutes = Math.min(60, Math.max(1, parseInt(opts && opts.minutes, 10) || LIVE_RADAR_WINDOW_MIN));
