...
function registerSupportDeskAdminRoutes(app, db, assertAdminPortalActor) {
    ...
    app.get('/api/admin/support-desk/agents', adminGuard((_req, res) => {
        db.all(
            ...
            [],
            (err, rows) => res.json(err ? { error: err.message } : { agents: rows || [] })
        );
    }));
    ...
    app.get('/api/admin/support-desk/departments', adminGuard((_req, res) => {
        db.all(
            `SELECT id, slug, name, sort_order, is_active FROM support_departments ORDER BY sort_order, name`,
            [],
            (err, rows) => res.json(err ? { error: err.message } : { departments: rows || [] })
        );
    }));
}
