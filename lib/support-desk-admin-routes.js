    ...
    app.post('/api/admin/support-desk/agents/:userId', adminGuard((req, res) => {
        ...
        db.get(`SELECT user_id FROM support_agent_profiles WHERE user_id = ?`, [uid], (e, row) => {
            ...
            const run = row
                ? db.run.bind(
                      db,
                      `UPDATE support_agent_profiles SET department_id=?, max_open_tickets=?, is_available=?, live_chat_enabled=?, notes=?, authority_level=?, updated_at=CURRENT_TIMESTAMP WHERE user_id=?`,
                      params
                  )
                : db.run.bind(
                      db,
                      `INSERT INTO support_agent_profiles (department_id, max_open_tickets, is_available, live_chat_enabled, notes, authority_level, user_id) VALUES (?,?,?,?,?,?,?)`,
                      [
                          Number.isInteger(deptId) ? deptId : null,
                          maxOpen,
                          isAvail,
