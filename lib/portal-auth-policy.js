...
const STAFF_LOGIN_PORTALS = new Set(['admin', 'judge', 'scanner', 'staff', 'support']);
...
function mergeAdminEnabledPages(pages) {
    const p = pages && typeof pages === 'object' && !Array.isArray(pages) ? { ...pages } : {};
    const keys = Object.keys(p);
    if (keys.length && keys.some((k) => p[k] === true)) {
        if (!('tab-book-sales' in p)) p['tab-book-sales'] = true;
        if (!('tab-cancellation-review' in p)) p['tab-cancellation-review'] = true;
    }
    return p;
}
...
function merge(raw) {
    ...
    const pages = mergeAdminEnabledPages(
    ...
    const doctorPortalModulesRegular =
        o.doctorPortalModulesRegular &&
        typeof o.doctorPortalModulesRegular === 'object' &&
        !Array.isArray(o.doctorPortalModulesRegular)
            ? o.doctorPortalModulesRegular
            : {};
    const doctorPortalModulesVolunteer =
        o.doctorPortalModulesVolunteer &&
        typeof o.doctorPortalModulesVolunteer === 'object' &&
        !Array.isArray(o.doctorPortalModulesVolunteer)
            ? o.doctorPortalModulesVolunteer
            : {};
    return {
...
function adminTabGloballyEnabled(tabId) {
    const pages = mergeAdminEnabledPages(cache.adminEnabledPages || {});
...
/** Staff portal (/staff/login): co-admin, operations staff, book sales — not judge/scanner. */
function canUseStaffBookPortal(row) {
    const { canUseStaffPortal } = require('./staff-portal-access');
    return canUseStaffPortal(row);
}
...
function websiteMenuPageEnabled(key) {
    ...
    const anyOn = keys.some((k) => pages[k] === true);
    if (!anyOn) return true;
    if (key === 'legal' || String(key || '').indexOf('legal-') === 0) {
