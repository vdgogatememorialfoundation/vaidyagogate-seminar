...
body.cert-print-body--capture{margin:0;padding:0;background:#fff;}
body.cert-print-body--capture .cert-page{width:297mm!important;max-width:none!important;min-height:210mm!important;margin:0 auto;box-shadow:none;}
body.cert-print-body--capture .cert-inner{display:block!important;min-height:auto!important;padding:28px 48px 32px 120px;text-align:center;}
body.cert-print-body--capture .cert-org,
body.cert-print-body--capture .cert-title,
body.cert-print-body--capture .cert-of,
body.cert-print-body--capture .cert-lead,
body.cert-print-body--capture .cert-name,
body.cert-print-body--capture .cert-body,
body.cert-print-body--capture .cert-meta{display:block!important;margin-left:auto!important;margin-right:auto!important;}
body.cert-print-body--capture .cert-body{max-width:620px;margin-bottom:14px!important;}
body.cert-print-body--capture .cert-meta{margin:6px auto!important;}
body.cert-print-body--capture .cert-ids-row{display:flex!important;flex-wrap:wrap!important;align-items:flex-end!important;justify-content:space-between!important;}
body.cert-print-body--capture .cert-sigs{display:flex!important;justify-content:space-between!important;align-items:flex-end!important;}
body.cert-print-body--capture .cert-print-hint{display:none!important;}
`;
...
function renderCertificateHtml(ctx, opts) {
    opts = opts || {};
    const forCapture = !!(opts.download || opts.pdfCapture);
    const inlineCss = opts.inlineCss ? String(opts.inlineCss) : '';
    const cssBlock = inlineCss
        ? `<style>${inlineCss}${forCapture ? PDF_CAPTURE_CSS : ''}</style>`
        : forCapture
          ? `<link rel="stylesheet" href="/css/certificate-vgmf.css"><style>${PDF_CAPTURE_CSS}</style>`
...
<head>
<meta charset="UTF-8">
<meta name="viewport" content="${forCapture ? 'width=1123' : 'width=device-width, initial-scale=1'}">
...
<body class="cert-print-body${forCapture ? ' cert-print-body--capture' : ''}">
<div class="cert-page">
  ${swoosh}
  ${flame}
...
function buildCertificateDownloadHtml(db, row, certType, cb) {
    if (!isBuiltinPath(row.template_path)) {
        return cb(null, null);
    }
    loadCertificateAssets(db, row, certType, { embed: true }, (eAssets, assets) => {
        if (eAssets) return cb(eAssets);
        const html = renderCertificateHtml(assets.ctx, {
            download: true,
            pdfCapture: true,
            inlineCss: getCertificateCssText()
        });
        cb(null, html);
    });
}

function handlePdfDownloadRequest(db, req, res) {
    resolveCertificateView(db, req, (err, data) => {
        if (err) return res.status(err.status || 500).send(err.message);
        const row = data.row;
        const certType = data.certType;
        if (!isBuiltinPath(row.template_path)) {
            if (row.template_path) return res.redirect(row.template_path);
            return res.status(404).send('Certificate file not found.');
        }
        buildCertificateDownloadHtml(db, row, certType, (buildErr, html) => {
            if (buildErr) return res.status(500).send(buildErr.message || 'Could not render certificate');
            const certPdf = require('./certificate-pdf-server');
            certPdf
                .htmlToPdfBuffer(html)
                .then((buf) => {
                    const fname = safeCertFilename(row) + '.pdf';
                ...
                .catch((pdfErr) => {
                    console.error('[certificate-pdf]', pdfErr.message || pdfErr);
                    sendCertificateHtml(db, res, row, certType, { download: true, autoPrint: true });
                });
        });
    });
}
