...
function buildRenderContext(row, certType, cmsVenue, logoUrl, configOverride, renderOpts) {
    ...
    return {
        ...
        certificateId: escHtml(certificateId || '—'),
        qrImgUrl: verifyToken && qrSrc ? escHtml(qrSrc) : '',
...
<p class="cert-print-hint no-print">${opts.download ? 'Saved to your device. Open the file and use Print → Save as PDF for a PDF copy.' : 'Use your browser Print → Save as PDF for a downloadable copy.'}</p>
...
function sendCertificateHtml(db, res, row, certType, opts) {
    ...
    if (!isBuiltinPath(path)) {
        if (opts && opts.download) {
            ...
            res.setHeader('Content-Disposition', `attachment; filename="${fname}${path.match(/\.pdf$/i) ? '.pdf' : ''}"`);
        }
    ...
    loadCmsVenue(db, (eVenue, cmsVenue) => {
        loadLogoUrl(db, (eLogo, logoUrl) => {
            const renderHtml = (qrDataUrl) => {
                const ctx = buildRenderContext(row, type, cmsVenue, logoUrl, null, { qrDataUrl });
                const html = renderCertificateHtml(ctx, opts || {});
                res.setHeader('Content-Type', 'text/html; charset=utf-8');
                if (opts && opts.download) {
                    res.setHeader(
                        'Content-Disposition',
                        `attachment; filename="${safeCertFilename(row)}.html"`
                    );
                    res.setHeader('Cache-Control', 'private, no-store');
                }
                res.send(html);
            };
            if (opts && opts.download && row.verify_token) {
                return certVerify.qrDataUrlForToken(row.verify_token, (eQr, dataUrl) => {
                    renderHtml(eQr ? '' : dataUrl);
                });
            }
            renderHtml('');
        });
