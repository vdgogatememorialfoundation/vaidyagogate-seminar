...
function inboundStatus() {
    const imapPoller = require('./inbound-imap-poller');
    const imap = imapPoller.status();
    const secret = String(process.env.INBOUND_MAIL_WEBHOOK_SECRET || '').trim();
    const domain = messageReplyAddress.replyDomain();
    const care = require('./support-care-email').careReplyToEmail();
    return {
        configured: imap.configured || !!(secret && domain),
        mode: imap.configured ? 'imap' : secret ? 'webhook' : 'none',
        imap,
        webhookPath: '/api/webhooks/inbound-email',
        parserInbox: messageReplyAddress.parserInboxAddress() || null,
        replyDomain: domain,
        careMailbox: care || null,
        needsSecret: !secret && !imap.configured,
        hint: imap.configured
            ? 'Built-in IMAP polling reads ' +
              (imap.mailbox || 'your Zoho inbox') +
              ' every ~2 min. Ticket emails include [VGMF-TKT-…] — doctors reply by email; assigned agents see it in /support.'
            : !imap.imapflowInstalled
              ? 'Install imapflow on the server (npm install imapflow).'
              : 'Save Zoho app password in Admin → Integrations, set INBOUND_IMAP_USER=care@vaidyagogate.org on Render, enable IMAP in Zoho Mail settings.'
    };
}
