...
function inboundStatus() {
    ...
    const gmail = require('./gmail-inbound-poller').status();
    ...
    const webhookReady = !!secret;
    const gmailReady = gmail.enabled;
    return {
        configured: webhookReady || gmailReady,
        webhookPath: '/api/webhooks/inbound-email',
        replyDomain: domain,
        careInbox: care,
        gmailInbound: gmail,
        needsSecret: !webhookReady && !gmailReady,
        hint: gmailReady
            ? 'Gmail IMAP poller active — forward ' +
              care +
              ' to ' +
              (gmail.user || 'Gmail') +
              '. Ticket replies sync via [VGMF-TKT-…] reference in email.'
            : webhookReady
              ? 'Webhook inbound ready. Prefer Gmail IMAP (free): set GMAIL_INBOUND_USER + App Password.'
              : 'Set GMAIL_INBOUND_USER + GMAIL_INBOUND_APP_PASSWORD on Render, and forward care@ to that Gmail inbox.'
    };
}
