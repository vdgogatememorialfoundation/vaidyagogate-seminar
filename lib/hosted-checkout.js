    ...
    loadOrder((err, row) => {
        ...
        db.all(`SELECT * FROM payment_gateways WHERE name = ? AND is_active = 1`, [gateway], (eG, gwRows) => {
            ...
            if (gateway === 'zoho') {
                ...
                const sessionStatus = String(data.payment_session_status || data.status || '').toLowerCase();
                const sigOk = signingKey ? zohoPaymentsGateway.verifyReturnSignature(data, signingKey) : true;
                if (
                    sigOk &&
                    (sessionStatus === 'succeeded' ||
                        sessionStatus === 'success' ||
                        String(data.payment_status || '').toLowerCase() === 'succeeded')
                ) {
                    return complete(
                        data.payment_id || data.payments_session_id || 'ZOHO_' + txnid,
                        'Payment successful. Your e-ticket is under Participant tickets.'
                    );
                }
                if (String(data.outcome || '').toLowerCase() === 'success' || sessionStatus === 'succeeded') {
                    return tryPoll();
                }
                return finish(false, 'Payment was not completed.');
            }
