...
function ensureCaseUploadSchema(db, cb) {
    ...
    const pendingAlters = [
        'ALTER TABLE case_pending_uploads ADD COLUMN IF NOT EXISTS original_name TEXT',
        'ALTER TABLE case_pending_uploads ADD COLUMN IF NOT EXISTS mime_type TEXT',
        'ALTER TABLE case_pending_uploads ADD COLUMN IF NOT EXISTS size_bytes BIGINT'
    ];
    const fileAlters = [
        'ALTER TABLE case_files ADD COLUMN IF NOT EXISTS storage_key TEXT',
        'ALTER TABLE case_files ADD COLUMN IF NOT EXISTS mime_type TEXT',
        'ALTER TABLE case_files ADD COLUMN IF NOT EXISTS size_bytes INTEGER'
    ];
    db.run(pendingSql, [], (e0) => {
        ignoreSchemaErr(e0);
        let i = 0;
        const next = () => {
            if (i >= pendingAlters.length) {
                // Now run file alters
                let j = 0;
                const nextFile = () => {
                    if (j >= fileAlters.length) return cb && cb();
                    db.run(fileAlters[j], (e) => {
                        ignoreSchemaErr(e);
                        j++;
                        nextFile();
                    });
...
function registerCaseUploadRoutes(app, deps) {
    ...
    app.get('/api/case/uploads/config', async (req, res) => {
        try {
            ...
            const r2Ok = r2Storage.isR2Configured() ? await r2Storage.isR2Ready() : false;
    ...
    if (!r2Storage.isR2Configured()) {
        console.log('[case-upload] R2 not configured — presigned upload routes disabled; /api/case/uploads/config active');
        return;
    }
    ...
    app.post('/api/case/uploads/init', async (req, res) => {
        try {
            ...
            if (!r2Ok) {
                console.warn('[case-upload/init] R2 not ready:', r2Storage.getR2SetupHint());
            ...
            console.log('[case-upload/init] userId:', userId, 'programId:', programId, 'file:', fileName, 'size:', sizeBytes);
            ...
            db.get(`SELECT * FROM case_programs WHERE id = ? AND IFNULL(is_active, 1) = 1`, [programId], async (e0, program) => {
                if (e0) {
                    console.error('[case-upload/init] DB error:', e0.message);
                    return res.status(500).json({ error: e0.message });
                }
                if (!program) {
                    console.warn('[case-upload/init] Case program not found:', programId);
                ...
                try {
                    ...
                    console.error('[case-upload/init] R2 error:', r2Err.message);
                    return res.status(500).json({ error: 'Failed to create upload: ' + r2Err.message });
                }
                ...
                db.run(
                    ...
                    (insErr) => {
                        if (insErr) {
                            console.error('[case-upload/init] Insert error:', insErr.message);
                            return res.status(500).json({ error: insErr.message });
                        }
                        console.log('[case-upload/init] Created upload session:', uploadId, 'multipart:', useMultipart);
                        res.json({
                            uploadId,
                            storageKey,
            ...
            console.error('[case-upload/init] Unexpected error:', err);
    ...
    app.post('/api/case/uploads/complete', async (req, res) => {
        try {
            ...
            console.log('[case-upload/complete] uploadId:', uploadId, 'userId:', userId, 'parts:', parts ? parts.length : 0);
            ...
            db.get(
                ...
                async (e, row) => {
                    if (e) {
                        console.error('[case-upload/complete] DB error:', e.message);
                        return res.status(500).json({ error: e.message });
                    }
                    if (!row) {
                        console.warn('[case-upload/complete] Upload session not found for uploadId:', uploadId, 'userId:', userId);
                        return res.status(404).json({ error: 'Upload session not found' });
                    }
                    console.log('[case-upload/complete] Found upload session:', row.id, 'status:', row.status, 'multipart:', !!row.multipart_upload_id);
                    ...
                    try {
                        ...
                        console.error('[case-upload/complete] R2 complete error:', completeErr.message);
                    ...
                    db.run(
                        ...
                        (uErr) => {
                            if (uErr) {
                                console.error('[case-upload/complete] Update status error:', uErr.message);
                                return res.status(500).json({ error: uErr.message });
                            }
                            console.log('[case-upload/complete] Upload completed successfully:', uploadId);
                            res.json({
                                success: true,
                                uploadId,
                                storageKey: row.storage_key,
            ...
            console.error('[case-upload/complete] Unexpected error:', err);
            res.status(500).json({ error: err.message });
        }
    });
...
function attachPendingUploads(db, userId, programId, uploadIds, cb) {
    ...
    console.log('[case-upload/attach] userId:', userId, 'programId:', programId, 'uploadIds:', uploadIds);
    db.all(
        ...
        (e, rows) => {
            if (e) {
                console.error('[case-upload/attach] DB error:', e.message);
                return cb(e);
            }
            const found = rows || [];
            console.log('[case-upload/attach] Found', found.length, 'of', uploadIds.length, 'uploads');
            if (found.length !== uploadIds.length) {
                const missing = uploadIds.filter((id) => !found.find((r) => r.id === id));
                console.warn('[case-upload/attach] Missing uploads:', missing);
