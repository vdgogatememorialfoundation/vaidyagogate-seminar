...
function ensureSchema(db, ignoreErr, cb) {
    const steps = [
        ...
        `ALTER TABLE seminars ADD COLUMN event_end_date TEXT`,
        `ALTER TABLE seminars ADD COLUMN ticket_expires_at TEXT`,
        `ALTER TABLE registrations ADD COLUMN ticket_expired_at TEXT`,
        `ALTER TABLE tickets ADD COLUMN day_id INTEGER`,
        `ALTER TABLE seminar_days ADD COLUMN start_time TEXT`,
        `ALTER TABLE seminar_days ADD COLUMN end_time TEXT`
    ];
    let i = 0;
    const run = () => {
    ...
    db.all(`SELECT id FROM seminar_days WHERE seminar_id = ?`, [sid], (selErr, existing) => {
        ...
        list.forEach((d, idx) => {
            ...
            items.push({
                ...
                startTime: normalizeTimeText(d.start_time || d.startTime),
                endTime: normalizeTimeText(d.end_time || d.endTime),
                checkinEnabled:
        ...
        const upsertAll = () => {
            ...
            items.forEach((d) => {
                ...
                if (d.id) {
                    db.run(
                        `UPDATE seminar_days SET title = ?, day_date = ?, checkin_date = ?, start_time = ?, end_time = ?, sort_order = ?, checkin_enabled = ?, is_active = ? WHERE id = ?`,
                        [d.title, d.dayDate, d.checkinDate, d.startTime, d.endTime, d.sortOrder, d.checkinEnabled, d.isActive, d.id],
                        (updErr) => onSaved(updErr, d.id)
                    );
                } else {
                    db.run(
                        `INSERT INTO seminar_days (seminar_id, title, day_date, checkin_date, start_time, end_time, sort_order, checkin_enabled, is_active)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                        [sid, d.title, d.dayDate, d.checkinDate, d.startTime, d.endTime, d.sortOrder, d.checkinEnabled, d.isActive],
                        function (insErr) {
