        ...
        db.all(sql, params, (err, rows) => {
            ...
            refundTracking.loadRefundsForRegistrations(db, regIds, (e2, refundsByReg) => {
                ...
                const out = list.map((row) => {
                    ...
                    return {
                        ...row,
                        refundStatusLabel: refundTracking.refundStatusLabel(row.refund_status),
                        trackingSteps: refundTracking.buildRefundTrackingSteps(row, refunds, row.registration_status),
                        refunds
                    };
                });
                res.json(out);
            ...
            paymentsMod.loadRegistrationPaymentContext(db, reqRow.registration_id, (e2, ctx) => {
                ...
                paymentsMod.processOrderRefund(
                    ...
                    (rErr, rOut) => {
                        if (rErr) return res.status(500).json({ error: rErr.message });
                        if (!rOut.ok) return res.status(400).json({ error: rOut.error });
                        const newStatus = rOut.manualRequired ? 'manual_pending' : 'completed';
                        db.run(
