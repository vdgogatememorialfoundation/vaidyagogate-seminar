...
function registerPaymentsRoutes(app, deps) {
                ...
                db.get(
                    ...
                    (e2, ord) => {
                        ...
                        const shouldPoll =
                            ...
                            ord.id &&
                            (gw.startsWith('dqr_') ||
                                gw.startsWith('easebuzz') ||
                                gw.startsWith('cashfree') ||
                                gw.startsWith('paytm') ||
                                gw.startsWith('phonepe') ||
                                gw.startsWith('payu'));
                        if (shouldPoll) {
    ...
    function redirectDoctorPaymentResult(err, out, tag) {
        if (err) {
            console.error('[' + tag + '-return]', err.message);
            return res.redirect(
    ...
    app.all('/api/payments/easebuzz/return', (req, res) => {
        const payload = { ...(req.query || {}), ...(req.body || {}) };
        adminPaymentFlow.processEasebuzzReturn(db, paymentDeps, payload, (err, out) =>
            redirectDoctorPaymentResult(err, out, 'easebuzz')
        );
    });

    app.all('/api/payments/cashfree/return', (req, res) => {
        const payload = { ...(req.query || {}), ...(req.body || {}) };
        adminPaymentFlow.processCashfreeReturn(db, paymentDeps, payload, (err, out) =>
            redirectDoctorPaymentResult(err, out, 'cashfree')
        );
    });

    app.all('/api/payments/payu/return', (req, res) => {
        const payload = { ...(req.query || {}), ...(req.body || {}) };
        adminPaymentFlow.processPayuReturn(db, paymentDeps, payload, (err, out) =>
            redirectDoctorPaymentResult(err, out, 'payu')
        );
    });

    app.all('/api/payments/paytm/return', (req, res) => {
        const payload = { ...(req.query || {}), ...(req.body || {}) };
        adminPaymentFlow.processPaytmReturn(db, paymentDeps, payload, (err, out) =>
            redirectDoctorPaymentResult(err, out, 'paytm')
        );
    });

    app.all('/api/payments/phonepe/return', (req, res) => {
        const payload = { ...(req.query || {}), ...(req.body || {}) };
        adminPaymentFlow.processPhonepeReturn(db, paymentDeps, payload, (err, out) =>
            redirectDoctorPaymentResult(err, out, 'phonepe')
        );
    });
