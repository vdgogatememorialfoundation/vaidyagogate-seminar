...
const razorpayManualMatch = require('./razorpay-manual-match');
...
function registerPaymentsRoutes(app, deps) {
    ...
    const regSelectWithScan = `SELECT r.id, r.user_id, r.status, r.application_no,
                    s.title AS seminar_title, s.event_date, s.cancellation_policy_json,
                    t.is_scanned, COALESCE(t.scan_count, 0) AS scan_count
    ...
    /**
     * Admin-initiated registration action:
     *   cancel_only   -> cancel registration, invalidate tickets, no refund
     *   cancel_refund -> cancel + refund (full or custom amount)
     *   refund_only   -> refund without cancelling (registration/ticket stay active)
     * All three create a cancellation_requests row (status 'approved', action_type set)
     * so the refund/cancellation tracking appears in admin + doctor portals.
     */
    app.post('/api/admin/registrations/:id/cancel-refund', (req, res) => {
        const registrationId = parseInt(req.params.id, 10);
        const action = String((req.body && req.body.action) || '').toLowerCase();
        const adminNotes = String((req.body && req.body.adminNotes) || '').trim();
        const customRefundAmount =
            req.body && req.body.refundAmount != null && req.body.refundAmount !== ''
                ? Number(req.body.refundAmount)
                : null;
        const actingAdminId = parseInt((req.body && req.body.actingAdminId) || '', 10);
        const ACTIONS = ['cancel_only', 'cancel_refund', 'refund_only'];
        if (!registrationId || !ACTIONS.includes(action)) {
            return res.status(400).json({ error: 'action must be cancel_only, cancel_refund or refund_only' });
        }
        const wantsRefund = action !== 'cancel_only';
        const wantsCancel = action !== 'refund_only';
        ...
        const proceed = () => {
            paymentsMod.loadRegistrationPaymentContext(db, registrationId, (e2, ctx) => {
                ...
                const st = String(reg.status || '').toLowerCase();
                if (wantsCancel && ['cancelled', 'rejected'].includes(st)) {
                    return res.status(400).json({ error: 'Registration is already ' + st + '.' });
                }
                if (wantsRefund && (!order || order.status !== 'success')) {
                    return res.status(400).json({ error: 'No successful payment found to refund.' });
                }
                const orderAmt = order ? Number(order.amount) || 0 : 0;
                const already = order ? Number(order.refunded_amount) || 0 : 0;
                const maxRefundable = Math.max(0, orderAmt - already);
                let refundAmt = 0;
                if (wantsRefund) {
                    refundAmt =
                        customRefundAmount != null && !Number.isNaN(customRefundAmount) && customRefundAmount > 0
                            ? customRefundAmount
                            : maxRefundable;
                    if (refundAmt <= 0) return res.status(400).json({ error: 'Nothing left to refund on this order.' });
                    if (refundAmt > maxRefundable + 0.01) {
                        return res.status(400).json({ error: 'Max refundable is ₹' + maxRefundable });
                    }
                }
                const refundPct = orderAmt ? Math.round((refundAmt / orderAmt) * 100) : 0;
                const labels = {
                    cancel_only: 'Cancelled by admin (no refund)',
                    cancel_refund: 'Cancelled by admin with refund',
                    refund_only: 'Refund issued by admin (registration remains active)'
                };

                const finalize = (refundResult) => {
                    const refundStatus = wantsRefund
                        ? razorpayRefundSync.deriveRefundStatusFromGatewayResult(refundResult, refundAmt)
                        : 'none';
                    const insertTracking = (next) => {
                        db.run(
                            `INSERT INTO cancellation_requests
                                (registration_id, user_id, reason, status, refund_percent, refund_amount, refund_status,
                                 provider_refund_id, admin_notes, reviewed_by, reviewed_at, action_type, initiated_by)
                            ...
                            (insErr) => {
                                if (insErr && /no such table|does not exist|no column|column .* does not exist/i.test(insErr.message)) {
                                    return paymentsMod.ensurePaymentsModuleSchema(db, () => {}, () => insertTracking(next));
                                }
                                next(insErr);
                            }
                        );
                    };
                    insertTracking((insErr) => {
                        if (insErr) return res.status(500).json({ error: insErr.message });
                        const notifyVars = {
                            cancellation_reason: labels[action],
                            admin_notes: adminNotes || 'No additional note from the office.',
                            refund_amount: refundAmt > 0 ? String(refundAmt) : '0',
                            refund_percent: refundPct > 0 ? String(refundPct) : '0',
                            approval_status: wantsCancel ? 'cancelled' : reg.status
                        };
                        if (wantsCancel) {
                            notifEngine.notify(
                                db,
                                'REGISTRATION_CANCELLED',
                                { userId: reg.user_id, registrationId, seminarId: reg.seminar_id, vars: notifyVars },
                                () => {}
                            );
                        }
                        if (wantsRefund) {
                            notifEngine.notify(db, 'REFUND_INITIATED', {
                                ...
                                vars: Object.assign({}, notifyVars, {
                                    provider_refund_id: (refundResult && refundResult.providerRefundId) || '',
                                    refund_status: refundStatus,
                                    refund_status_label: refundTracking.refundStatusLabel(refundStatus),
                                    order_id_string: order && order.order_id_string,
                                    provider_transaction_id: order && order.provider_transaction_id,
                            ...
                            if (refundResult && refundResult.providerRefundId && refundStatus === 'processing') {
                                razorpayRefundSync.syncRefundByProviderId(db, refundResult.providerRefundId, () => {});
                            }
                        }
                        activityLog.logActivity(db, {
                            user_id: actingAdminId || null,
                            ...
                            resource_type: 'registration',
                            resource_id: String(registrationId),
                            meta: { application_no: reg.application_no, refundAmount: refundAmt, adminNotes }
                        });
                        res.json({
                            success: true,
                            action,
                            message: wantsCancel
                                ? wantsRefund
                                    ? 'Registration cancelled and refund of ₹' + refundAmt + ' initiated.'
                                    : 'Registration cancelled (no refund).'
                                : 'Refund of ₹' + refundAmt + ' initiated. Registration remains active.',
                            refund: refundResult || { skipped: true },
                            refundAmount: refundAmt,
                            refundPercent: refundPct,
                            refundStatus
                ...
                const doCancel = (refundResult) => {
                    if (!wantsCancel) return finalize(refundResult);
                    invalidateTicketsForRegistration(registrationId, (invErr) => {
                        if (invErr) return res.status(500).json({ error: invErr.message });
                        db.run(`UPDATE registrations SET status = 'cancelled' WHERE id = ?`, [registrationId], (cErr) => {
                            if (cErr) return res.status(500).json({ error: cErr.message });
                            ...
                            if (portalTracking && typeof portalTracking.logRegistrationEvent === 'function') {
                                portalTracking.logRegistrationEvent(
                                    db,
                                    registrationId,
                                    'cancelled',
                                    'Application cancelled',
                                    adminNotes || labels[action],
                                    () => {}
                                );
                            }
                            finalize(refundResult);
                        });
                    });
                };
                ...
                if (wantsRefund) {
                    paymentsMod.processOrderRefund(
                        db,
                        {
                            ...
                            amountRupees: refundAmt,
                            percent: refundPct,
                            reason: labels[action] + (adminNotes ? ' — ' + adminNotes : ''),
                            adminUserId: actingAdminId
                        },
                        (rErr, rOut) => {
                            if (rErr) return res.status(500).json({ error: rErr.message });
        ...
        if (assertAdminPortalActor && actingAdminId) {
    ...
    app.get('/api/admin/payments/application-link', (req, res) => {
        ...
        assertAdminPortalActor(adminUserId, (eAct, adm) => {
            if (eAct || !adm) return res.status(403).json({ error: 'Admin access required' });
            adminPaymentFlow.prepareApplicationPaymentLink(
                db,
                paymentDeps,
                { registrationId, methodId },
                (err, out) => {
                    if (err) return res.status(500).json({ error: err.message });
                    if (out && out.error) {
                        const code = out.alreadyPaid ? 409 : 400;
    ...
    app.post('/api/admin/payments/send-application-link', (req, res) => {
        const registrationId = parseInt((req.body && req.body.registrationId) || '', 10);
        const adminUserId = parseInt((req.body && req.body.actingAdminId) || '', 10);
        const methodId = String((req.body && req.body.methodId) || '').trim();
        const channel = String((req.body && req.body.channel) || 'both').toLowerCase();
        if (!registrationId || !adminUserId) {
            return res.status(400).json({ error: 'registrationId and actingAdminId are required' });
        }
        assertAdminPortalActor(adminUserId, (eAct, adm) => {
            ...
            adminPaymentFlow.prepareApplicationPaymentLink(
                ...
                (err, out) => {
                    ...
                    if (!out.applicantEmail) {
                        return res.status(400).json({
                            error: 'Applicant has no email on file. Copy the payment link and share it manually.'
                        });
                    }
                    if (!notifEngine || !notifEngine.notify) {
                    ...
                    notifEngine.notify(
                        ...
                        {
                            userId: out.userId,
                            seminarId: out.seminarId,
                            registrationId: out.registrationId,
                            vars: {
                                payment_link: out.paymentLink,
                                payment_amount: out.amount,
                                portal_login_url: out.paymentLink
                            },
                        ...
                        (nErr, nOut) => {
                            ...
                            const queued = nOut && nOut.queued;
                            const parts = [];
                            if (sendEmail) parts.push('email to ' + out.applicantEmail);
                            if (sendWhatsapp) parts.push('WhatsApp if phone on file');
                            res.json({
                                success: true,
                                queued: !!queued,
                                paymentLink: out.paymentLink,
                                applicantEmail: out.applicantEmail,
                                applicantName: out.applicantName,
                                applicationNo: out.applicationNo,
                                amount: out.amount,
                                message:
                                    'Payment link sent' +
                                    (parts.length ? ' (' + parts.join(', ') + ')' : '') +
                                    (queued ? '.' : ' (notification may be queued).')
                            });
                        }
                    );
                }
    ...
    app.get('/api/doctor/payment-link-resolve', (req, res) => {
        const registrationId = parseInt(req.query.registrationId, 10);
        const userId = parseInt(req.query.userId, 10);
        const payUser = String(req.query.payUser || '').trim();
        if (!registrationId || !userId) {
            return res.status(400).json({ error: 'registrationId and userId are required' });
        }
        db.get(
            `SELECT r.id, r.user_id, r.status, r.application_no, r.seminar_id,
                    u.user_id_string, s.title AS seminar_title, s.price
             FROM registrations r
            ...
            (err, reg) => {
                ...
                if (Number(reg.user_id) !== userId) {
                    return res.status(403).json({
                        error: 'wrong_account',
                        message:
                            'This payment link belongs to another doctor. Sign in with the email used on the application.'
                    });
                }
                if (payUser && reg.user_id_string && payUser !== String(reg.user_id_string)) {
                    return res.status(403).json({
                        error: 'wrong_account',
                        message:
                            'This payment link is for portal ID ' +
                            reg.user_id_string +
                ...
                if (st === 'completed' || st === 'checked_in') {
                    return res.json({
                        ...
                        applicationNo: reg.application_no,
                        message: 'This application is already paid. Your e-ticket is under Participant tickets.'
                    });
                }
                db.get(
                    ...
                    [registrationId],
                    (ePaid, paidOrd) => {
                        if (ePaid) return res.status(500).json({ error: ePaid.message });
                        if (paidOrd) {
                            return res.json({
                                payable: false,
                                alreadyPaid: true,
                        ...
                        if (st !== 'approved_pending_payment') {
                            ...
                            return res.json({
                                ...
                                applicationNo: reg.application_no,
                                message:
                                    'Payment is not open yet — application status: ' +
                                    label +
                                    '. Contact the seminar office if you received a payment link.'
                            });
                        }
                        ...
                        seminarEvents.computeRegistrationPaymentAmount(db, registrationId, (payErr, pay) => {
                            ...
                            if (pay && pay.error) return res.status(400).json({ error: pay.error });
                            const amount =
                                pay && pay.amount != null ? pay.amount : Number(reg.price) || 0;
                            res.json({
                                payable: true,
                                registrationId: reg.id,
                                applicationNo: reg.application_no,
    ...
    app.get('/api/admin/payments/razorpay/lookup', (req, res) => {
        const actingAdminId = parseInt((req.query && req.query.actingAdminId) || '', 10);
        if (!actingAdminId) return res.status(400).json({ error: 'actingAdminId required' });
        assertAdminPortalActor(actingAdminId, (eAct, adm) => {
            if (eAct || !adm) return res.status(403).json({ error: 'Admin access required' });
            razorpayManualMatch.lookup(db, req.query.paymentId, (err, out) => {
                if (err) return res.status(500).json({ error: err.message });
    ...
    app.get('/api/admin/payments/razorpay/search-registrations', (req, res) => {
        const actingAdminId = parseInt((req.query && req.query.actingAdminId) || '', 10);
        if (!actingAdminId) return res.status(400).json({ error: 'actingAdminId required' });
        assertAdminPortalActor(actingAdminId, (eAct, adm) => {
            if (eAct || !adm) return res.status(403).json({ error: 'Admin access required' });
            razorpayManualMatch.searchRegistrations(db, req.query.q, (err, rows) => {
                if (err) return res.status(500).json({ error: err.message });
    ...
    app.post('/api/admin/payments/razorpay/match', (req, res) => {
        const actingAdminId = parseInt((req.body && req.body.actingAdminId) || '', 10);
        if (!actingAdminId) return res.status(400).json({ error: 'actingAdminId required' });
        assertAdminPortalActor(actingAdminId, (eAct, adm) => {
            if (eAct || !adm) return res.status(403).json({ error: 'Admin access required' });
            razorpayManualMatch.match(
                db,
                { fulfillRegistrationPayment, notifEngine, notifyTicketIssued, portalTracking },
                {
