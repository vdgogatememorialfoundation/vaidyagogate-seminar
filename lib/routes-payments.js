            ...
            (err, reg) => {
                ...
                db.get(
                    ...
                    (ePaid, paidOrd) => {
                        ...
                        // A pending order amount (e.g. an admin-set custom fee) wins
                        // over the recomputed seminar/sessions total.
                        db.get(
                            `SELECT amount FROM orders WHERE registration_id = ? AND status = 'pending' ORDER BY id DESC LIMIT 1`,
                            [registrationId],
                            (ePend, pendingOrder) => {
                                if (ePend) return res.status(500).json({ error: ePend.message });
                                if (pendingOrder && pendingOrder.amount != null) {
                                    return respondPayable(Number(pendingOrder.amount) || 0);
                                }
