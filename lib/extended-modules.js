...
function ensureExtendedModulesSchema(db, ignoreErr, next) {
    db.serialize(() => {
        db.run(
            ...
            (e) => {
                ignoreErr(e);
                db.run(`ALTER TABLE registration_overrides ADD COLUMN register_until TEXT`, (eCol) => {
                    ignoreErr(eCol);
                db.run(
...
function registrationOverrideIsEnabled(val) {
    if (val === false || val === 0 || val === '0') return false;
    return true;
}

function registrationOverrideIsActive(registerUntil) {
    return seminarDt.isRegistrationOverrideOpenNow(registerUntil);
}

function isRegistrationOverrideActiveRow(row) {
    if (!row || !registrationOverrideIsEnabled(row.enabled)) return false;
    return registrationOverrideIsActive(row.register_until);
}
...
function fetchRegistrationOverrideRow(db, userId, seminarId, cb) {
    db.get(
        `SELECT enabled, register_until FROM registration_overrides
         WHERE user_id = ? AND seminar_id = ? AND COALESCE(enabled, 1) = 1`,
        [userId, seminarId],
        (e, row) => cb(e, row || null)
    );
}

function userHasActiveRegistrationOverride(db, userId, seminarId, cb) {
    fetchRegistrationOverrideRow(db, userId, seminarId, (e, row) => {
        if (e) return cb(e);
        cb(null, isRegistrationOverrideActiveRow(row));
    });
}
...
function userHasRegistrationOverride(db, userId, seminarId, cb) {
    fetchRegistrationOverrideRow(db, userId, seminarId, (e, row) => {
        if (e) return cb(e);
        if (isRegistrationOverrideActiveRow(row)) return cb(null, true);
            db.get(
                `SELECT id FROM seminar_volunteers
                 WHERE user_id = ? AND seminar_id = ? AND status IN ('pending', 'approved')
...
module.exports = {
    ...
    userHasActiveRegistrationOverride,
    fetchRegistrationOverrideRow,
    isRegistrationOverrideActiveRow,
    registrationOverrideIsEnabled,
    registrationOverrideIsActive
};
