...
function answerFromKnowledge(message, knowledge, userContext) {
    ...
    if (m.includes('live chat') || m.includes('talk to') || m.includes('agent') || m.includes('human')) {
        return 'During support hours, tap "Talk to a support agent" in this chat or sign in to the doctor portal and use the live chat button (bottom right). You will receive a chat reference like LCHAT-384729105638.';
    }
    ...
    if (m.includes('certificate') || m.includes('cmc') || m.includes('cme')) {
        return 'After venue check-in, your participation certificate is issued automatically. Open the doctor portal → Certificates to browse by year and download PDFs. Verify authenticity publicly at /verify-certificate.html using the QR code on your certificate.';
    }
    if (m.includes('verify') && m.includes('certificate')) {
        return 'Certificate verification is at /verify-certificate.html. Scan the QR on the certificate or enter application number / portal registration number, then confirm with email and WhatsApp one-time passwords.';
    }
    if (m.includes('juspay') || m.includes('cashfree')) {
        return 'Approved seminar payments can use Razorpay, Cashfree, or Juspay Hyper Checkout (when enabled by admin). Choose your option on the payment screen in the doctor portal after approval.';
    }
    if (m.includes('payment') || m.includes('receipt') || m.includes('razorpay') || m.includes('pay')) {
        return 'Pay approved seminar registrations in the doctor portal via Razorpay, Cashfree, or Juspay. Successful payments show under Payment receipts and Orders; your e-ticket QR appears under View Tickets.';
    }
