...
async function buildChatbotKnowledge(db, loadPublicSiteCms) {
    ...
    parts.push(
        'Support: raise a ticket under Support tickets in the doctor portal, use Live chat during business hours, or use the Help button on the main website. Live chat references look like LCHAT-7.'
    );
    parts.push(
...
function answerFromKnowledge(message, knowledge, userContext) {
    ...
    if (m.includes('live chat') || m.includes('talk to') || m.includes('agent') || m.includes('human')) {
        return 'During support hours, tap "Talk to a support agent" in this chat or sign in to the doctor portal and use the live chat button (bottom right). You will receive a chat reference like LCHAT-7.';
    }
