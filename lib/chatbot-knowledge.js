...
async function buildChatbotKnowledge(db, loadPublicSiteCms) {
    ...
    parts.push(
        'Certificates: after venue check-in, participation certificates are issued automatically. Download them year-by-year from the Certificates tab in the doctor portal. Public authenticity verification is at /verify-certificate (email + WhatsApp OTP).'
    );
    parts.push(
        'Payments: pay approved seminar fees via Razorpay, Cashfree, Juspay, Easebuzz, PayU, Paytm, PhonePe, or Zoho Payments in the doctor portal. Receipts appear under Payment receipts.'
    );
    parts.push(
...
const SEMINAR_REGISTRATION_STEPS =
    ...
    '1. Open the doctor portal (/doctor) and sign in — create an account if you are new.\n' +
...
function answerFromKnowledge(message, knowledge, userContext) {
    ...
    if (/(create account|new account|sign up|doctor portal login|how to login|log in)/.test(m)) {
        return {
            reply:
                'Create your doctor portal account at /doctor → Register. Use your email and mobile for OTP verification. After sign-in, open Available Seminars to apply for open events.',
            suggestLiveChat: false
        };
    }
    ...
    if (isPaymentQuestion(m)) {
        return {
            reply:
                ...
                '3. Pay using the enabled gateway (Razorpay, Cashfree, Juspay, Easebuzz, PayU, Paytm, PhonePe, or Zoho Payments).\n' +
    ...
    if (m.includes('certificate') || m.includes('cmc') || m.includes('cme')) {
        return {
            reply:
                'After venue check-in, your participation certificate is issued automatically. Open the doctor portal → Certificates to browse by year and download PDFs. Verify authenticity at /verify-certificate using the QR code on your certificate.',
            suggestLiveChat: false
        };
    }
    ...
    if (m.includes('verify') && m.includes('certificate')) {
        return {
            reply:
                'Certificate verification is at /verify-certificate. Scan the QR on the certificate or enter your application / registration number, then confirm with email and WhatsApp OTP.',
            suggestLiveChat: false
        };
    }
    ...
    if (m.includes('seminar') || m.includes('fee') || m.includes('date') || m.includes('upcoming')) {
        ...
        return {
            reply: hits.length
                ? 'Active seminars:\n' + hits.join('\n') + '\n\nTo register, sign in at /doctor → Available Seminars.'
