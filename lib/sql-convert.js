...
function convertSqliteToPostgres(sql) {
    ...
    // SQLite 0/1 vs PostgreSQL boolean (preserve table alias: cp.is_active = 1)
    const boolCols = [
        'is_active',
        'is_countdown_active',
        'public_list_enabled',
    ...
    s = s.replace(/\bCOALESCE\s*\(\s*checkin_enabled\s*,\s*0\s*\)/gi, 'COALESCE(checkin_enabled, false)');
    s = s.replace(/\bCOALESCE\s*\(\s*is_disabled\s*,\s*0\s*\)/gi, 'COALESCE(is_disabled, false)');
    s = s.replace(/\bCOALESCE\s*\(\s*is_valid\s*,\s*1\s*\)/gi, 'COALESCE(is_valid, true)');
    s = s.replace(/\bCOALESCE\s*\(\s*public_list_enabled\s*,\s*0\s*\)/gi, 'COALESCE(public_list_enabled, false)');
