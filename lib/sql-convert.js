...
function convertSqliteToPostgres(sql) {
    ...
    // SQLite 0/1 vs PostgreSQL boolean
    s = s.replace(/\bis_active\s*=\s*1\b/gi, '(is_active IS TRUE OR is_active::int = 1)');
    s = s.replace(/\bis_active\s*=\s*0\b/gi, '(is_active IS FALSE OR is_active::int = 0)');
    s = s.replace(/\bis_countdown_active\s*=\s*1\b/gi, '(is_countdown_active IS TRUE OR is_countdown_active::int = 1)');
    s = s.replace(/\bpublic_list_enabled\s*=\s*1\b/gi, '(public_list_enabled IS TRUE OR public_list_enabled::int = 1)');
