...
function convertSqliteToPostgres(sql) {
    ...
    s = s.replace(
        /\bCOALESCE\s*\(\s*se\.checkin_enabled\s*,\s*s\.checkin_enabled\s*\)/gi,
        'COALESCE(se.checkin_enabled::int, s.checkin_enabled::int)'
    );
    s = s.replace(
        /\bCOALESCE\s*\(\s*COALESCE\s*\(\s*se\.checkin_enabled\s*,\s*s\.checkin_enabled\s*\)/gi,
        'COALESCE(COALESCE(se.checkin_enabled::int, s.checkin_enabled::int)'
    );
    s = s.replace(
        /\bCOALESCE\s*\(\s*se\.cert_scans_required\s*,\s*s\.cert_scans_required\s*\)/gi,
        'COALESCE(se.cert_scans_required::int, s.cert_scans_required::int)'
    );
    s = s.replace(
        /\bCOALESCE\s*\(\s*COALESCE\s*\(\s*se\.cert_scans_required\s*,\s*s\.cert_scans_required\s*\)/gi,
        'COALESCE(COALESCE(se.cert_scans_required::int, s.cert_scans_required::int)'
    );
    for (const col of flagCols) {
        s = s.replace(
    ...
    if (hadOrIgnore && !/ON\s+CONFLICT/i.test(s)) {
        ...
        } else if (/case_judge_marking_reminder_log/i.test(s)) {
            s = s.trim().replace(/;\s*$/, '') + ' ON CONFLICT (judge_user_id, submission_id, sent_date) DO NOTHING';
    ...
    if (hadOrReplace && /venue_entry_profiles/i.test(s) && !/ON\s+CONFLICT/i.test(s)) {
        s =
            s.trim().replace(/;\s*$/, '') +
            ' ON CONFLICT (user_id) DO UPDATE SET selfie_path = EXCLUDED.selfie_path, status = EXCLUDED.status, consent_at = EXCLUDED.consent_at, enrolled_at = EXCLUDED.enrolled_at';
    }
...
const INSERT_TABLES_WITHOUT_ID =
    /\bINTO\s+(global_settings|pages|registration_reminder_log|file_blobs|venue_entry_profiles|case_judge_marking_reminder_log|site_visitor_sessions|wa_settings|wa_contacts)\b/i;

function insertReturnsId(sql) {
    if (!isInsert(sql)) return false;
    if (/RETURNING\s+/i.test(sql)) return false;
    if (/ON\s+CONFLICT/i.test(sql)) return false;
