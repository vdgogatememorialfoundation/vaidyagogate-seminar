...
function buildTicketHtml(row, qrDataUrl) {
    ...
    const isOnline =
        ...
        String(row.delivery_mode || '').toLowerCase() === 'hybrid';
...
<div class="card">
  ...
  <div class="body">
    <div class="meta">
      ...
      ${venue && !isOnline ? `<p><strong>Venue</strong><br>${venue}</p>` : ''}
      ${isOnline ? `<p><strong>Admission</strong><br>Online webinar — join only from your doctor portal. Non-transferable.</p>` : ''}
    </div>
    ${qrBlock}
  </div>
  <div class="foot">${foundation} · ${portalYear} · Personal e-ticket for ${name} · Non-transferable${isOnline ? ' · Do not share meeting links' : ''}</div>
</div>
<script>
