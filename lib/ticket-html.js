...
const { isTicketExpiredForSeminar, ticketExpiryStartsYmd } = require('./local-date');
...
function buildQrBlock(row, qrDataUrl) {
    ...
    const watermark = escapeHtml(
        row.watermark_text || (row.display_name || row.doctor_name || 'Participant') + ' · Non-transferable'
    );
    ...
    if (scanned) {
        ...
        stampHtml = '<div class="stamp stamp-expired" aria-hidden="true"><span>EXPIRED</span></div>';
    }

    const qrImg = qrDataUrl
        ? `<img class="qr-img${scanned || expired ? ' qr-used' : ''}" src="${qrDataUrl}" alt="Entry QR code" draggable="false">`
    ...
    const hint = scanned
        ...
        : expired
          ? '<p class="qr-hint warn">Valid through event day only — expired after ' +
            escapeHtml(ticketExpiryStartsYmd(row.event_date) || 'event date') +
            ' (IST). Not scanned — no certificate.</p>'
          : paid && !invalid
    ...
    return `<div class="qr-wrap">${qrImg}<div class="qr-watermark" aria-hidden="true">${watermark}</div>${stampHtml}${paidBelow}${hint}</div>`;
}
...
function buildTicketHtml(row, qrDataUrl) {
    ...
    const holder = escapeHtml(row.display_name || row.doctor_name || 'Participant');
...
.notice{background:#fef2f2;border-bottom:2px solid #dc2626;padding:10px 26px;text-align:center;}
.notice-text{color:#dc2626;font-size:.8rem;font-weight:700;margin:0;line-height:1.4;}
.notice-text svg{vertical-align:middle;margin-right:6px;}
...
.qr-watermark{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;pointer-events:none;font-size:.62rem;font-weight:800;color:rgba(15,118,110,.22);text-align:center;line-height:1.2;padding:8px;transform:rotate(-18deg);user-select:none;-webkit-user-select:none;}
...
.stamp-expired span{color:#b91c1c;border:4px solid #dc2626;background:rgba(254,242,242,.94);}
...
@media print{body{padding:0;background:#fff;-webkit-user-select:text;user-select:text}.card{box-shadow:none}.notice{display:none}}
</style>
</head>
...
<div class="card">
  <div class="notice">
    <p class="notice-text">
      ...
      DO NOT take screenshots or photos of this e-ticket — sharing is strictly prohibited
    </p>
  </div>
  <div class="head">
    <div class="brand-row">
  ...
  <div class="foot">${foundation} · ${portalYear} · Issued to ${holder} · Non-transferable e-ticket</div>
</div>
<script>
