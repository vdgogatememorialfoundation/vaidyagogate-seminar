...
const { formatSeminarSchedule, formatScanDateTime } = require('./seminar-datetime');
const { isTicketExpired, formatTicketExpiry } = require('./local-date');
...
function buildQrBlock(row, qrDataUrl) {
    ...
    const hint = scanned
        ...
        : expired
          ? '<p class="qr-hint warn">This QR expired on ' +
            escapeHtml(formatTicketExpiry(row) || 'the event end date') +
            '. Not scanned — no certificate.</p>'
          : paid && !invalid
...
function buildTicketHtml(row, qrDataUrl) {
    ...
    const expiresLine = escapeHtml(row.ticket_expires_fmt || '');
...
<div class="card">
  ...
  <div class="body">
    <div class="meta">
      ...
      ${expiresLine ? `<p><strong>QR valid until</strong><br>${expiresLine}</p>` : ''}
...
async function buildTicketHtmlFromRow(row, db) {
    ...
    if (r.event_date && !r.event_date_fmt) {
        r.event_date_fmt = formatSeminarSchedule({ event_date: r.event_date, event_end_date: r.event_end_date });
    }
    if (!r.ticket_expires_fmt) {
        r.ticket_expires_fmt = formatTicketExpiry(r);
    }
