...
const { formatSeminarDateForTicket, formatSeminarDateTime } = require('./seminar-datetime');
...
function buildQrBlock(row, qrDataUrl) {
    ...
    if (scanned) {
        ...
        stampHtml = '<div class="stamp stamp-paid" aria-hidden="true"><span>PAID</span></div>';
    }

    const qrImg = qrDataUrl
        ? `<img class="qr-img${scanned ? ' qr-used' : ''}" src="${qrDataUrl}" alt="Entry QR code">`
    ...
    return `<div class="qr-wrap">${qrImg}${stampHtml}${hint}</div>`;
}
...
<style>
body{font-family:'Segoe UI',system-ui,sans-serif;margin:0;padding:24px;background:#f0fdfa;}
...
.head{background:linear-gradient(135deg,#0f766e,#134e4a);color:#fff;padding:22px 26px;}
.head h1{margin:0;font-size:1.3rem;font-weight:800;}
.head p{margin:6px 0 0;opacity:.92;font-size:.92rem;}
...
.qr-img{width:160px;height:160px;border:1px solid #cbd5e1;border-radius:10px;background:#fff;}
...
.foot{padding:14px 26px;background:#f8fafc;font-size:.82rem;color:#64748b;border-top:1px solid #e2e8f0;}
...
<div class="card">
  ...
  <div class="foot">Vaidya Gogate Memorial Foundation · Print or save as PDF from your browser</div>
</div>
</body>
</html>`;
...
async function buildTicketHtmlFromRow(row) {
    ...
    if (r.event_date && !r.event_date_fmt) {
        r.event_date_fmt = formatSeminarDateForTicket(r.event_date);
    }
    if (r.scan_time && !r.scan_time_fmt) {
        r.scan_time_fmt = formatSeminarDateTime(r.scan_time);
    }
    let qrDataUrl = '';
    if (r.qr_code_data) {
        try {
