...
function otpRowPurpose(purpose, meta) {
    ...
    if (purpose === 'certificate_verify' && meta && meta.certId != null) {
        return `certificate_verify:${parseInt(meta.certId, 10)}`;
    }
...
function tokenPurpose(purpose, channel, meta) {
    ...
    if (purpose === 'certificate_verify' && meta && meta.certId != null) {
        return `certificate_verify:${parseInt(meta.certId, 10)}:${channel}`;
    }
...
function validateRegistrationSubmitOtpTokens(db, seminarId, { phoneToken, emailToken }, cb) {
    const pPhone = `registration_submit:${seminarId}:phone`;
    const pEmail = `registration_submit:${seminarId}:email`;
