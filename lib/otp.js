...
function issueVerificationToken(db, { purpose, channel, meta, userId, seminarId }, cb) {
    ...
    db.run(
        ...
        function (ierr) {
            ...
            cb(null, { ok: true, token, tokenId: this.lastID, demoBypass: true });
        }
    );
}
        ...
        userIdIsDemo(db, uid, (eDemo, isDemoById) => {
            ...
            if (isDemoById) {
                return issueVerificationToken(
                    db,
                    { purpose, channel, meta, userId: uid, seminarId },
                    (eTok, result) => {
            ...
            resolveDemoUserByDestination(db, { channel, destination }, (eDest, isDemoDest, destUid) => {
                ...
                return issueVerificationToken(
                    db,
                    { purpose, channel, meta, userId: destUid, seminarId },
                    (eTok2, result2) => {
...
function consumeVerificationToken(db, token, cb) {
    const th = hashCode(token);
    const now = new Date().toISOString();
    db.get(
        ...
        (err, row) => {
            ...
            db.run(`UPDATE otp_verification_tokens SET consumed = 1 WHERE id = ?`, [row.id], (e2) => {
                if (e2) return cb(e2);
                cb(null, { ok: true, purpose: row.purpose, channel: row.channel });
            });
        }
    );
}
...
function validateRegistrationOtpTokens(db, seminarId, { phoneToken, emailToken }, cb) {
    const pPhone = `registration:${seminarId}:phone`;
    const pEmail = `registration:${seminarId}:email`;
    if (!phoneToken || !emailToken) return cb(null, { ok: false, error: 'Missing OTP verification for this seminar' });
    consumeVerificationToken(db, phoneToken, (e1, r1) => {
        if (e1) return cb(e1);
        if (!r1 || !r1.ok || r1.purpose !== pPhone || r1.channel !== 'phone') {
            return cb(null, { ok: false, error: 'Invalid phone verification for this registration' });
        }
        consumeVerificationToken(db, emailToken, (e2, r2) => {
            if (e2) return cb(e2);
            if (!r2 || !r2.ok || r2.purpose !== pEmail || r2.channel !== 'email') {
                return cb(null, { ok: false, error: 'Invalid email verification for this registration' });
            }
            cb(null, { ok: true });
        });
    });
