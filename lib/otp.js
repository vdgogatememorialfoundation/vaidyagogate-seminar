...
const SIGNUP_MIN_GAP_MS = parseInt(process.env.OTP_SIGNUP_COOLDOWN_SEC || '60', 10) * 1000;
const SIGNUP_MAX_SENDS_PER_HOUR = parseInt(process.env.OTP_SIGNUP_MAX_SENDS || '4', 10);
...
function countSignupSends(db, channel, destination, cb) {
    const since = new Date(Date.now() - 3600 * 1000).toISOString();
    const dest = normalizeOtpDestination(channel, destination);
    db.get(
        `SELECT COUNT(*) AS c FROM otp_codes WHERE channel = ? AND destination = ? AND purpose = 'signup' AND created_at > ?`,
        [channel, dest, since],
        (err, row) => {
            if (err) return cb(err, 999);
            cb(null, row && row.c != null ? row.c : 0);
...
function lastSignupSendAt(db, channel, destination, cb) {
    const dest = normalizeOtpDestination(channel, destination);
    db.get(
        `SELECT created_at FROM otp_codes WHERE channel = ? AND destination = ? AND purpose = 'signup' ORDER BY id DESC LIMIT 1`,
        [channel, dest],
        (err, row) => cb(err, row && row.created_at ? row.created_at : null)
    );
}

/**
 * Gate OTP send — signup: 60s gap, max 4/hour (1 initial + 3 resends).
 * Returns { allowed, retryAfterSeconds?, resendsRemaining?, resetAt? }
 */
function evaluateSendPolicy(db, channel, destination, purpose, cb) {
    if (purpose === 'signup') {
        return lastSignupSendAt(db, channel, destination, (eLast, lastAt) => {
            if (eLast) return cb(eLast);
            if (lastAt) {
                const elapsed = Date.now() - new Date(lastAt).getTime();
                const waitMs = SIGNUP_MIN_GAP_MS - elapsed;
                if (waitMs > 0) {
                    return cb(null, {
                        allowed: false,
                        retryAfterSeconds: Math.ceil(waitMs / 1000),
                        reason: 'Wait before requesting another code.'
                    });
                }
            }
            countSignupSends(db, channel, destination, (eCnt, cnt) => {
                if (eCnt) return cb(eCnt);
                if (cnt >= SIGNUP_MAX_SENDS_PER_HOUR) {
                    ...
                    return cb(null, {
                        ...
                        resendsRemaining: 0,
                        resetAt,
                        reason: 'Maximum signup OTP attempts reached. Try again in about an hour.'
                    });
                }
                cb(null, {
                    allowed: true,
                    resendsRemaining: Math.max(0, SIGNUP_MAX_SENDS_PER_HOUR - cnt - 1),
                    cooldownSeconds: Math.ceil(SIGNUP_MIN_GAP_MS / 1000)
                });
            });
        });
    }
    countRecentSends(db, channel, destination, (cerr, cnt) => {
        if (cerr) return cb(cerr);
        if (cnt >= MAX_SENDS_PER_HOUR) {
            return cb(null, {
                allowed: false,
                retryAfterSeconds: 3600,
                reason: 'Too many OTP requests. Try again later.'
            });
        }
...
function validateSignupOtpTokens(db, { emailToken }, cb) {
    if (!emailToken) return cb(null, { ok: false, error: 'Missing email OTP verification token' });
    consumeVerificationToken(db, emailToken, (e2, r2) => {
        if (e2) return cb(e2);
