    ...
    db.all(`SELECT * FROM payment_gateways`, [], (err, rows) => {
        ...
        const pickFromDb = (id) => {
            if (id) {
                ...
                if (resolved && resolved.gateway === 'razorpay') return resolved;
            }
            const expanded = [];
            (rows || []).forEach((row) => {
                expanded.push(...paymentGatewayOptions.expandGatewayRow(row));
            });
            if (id) return expanded.find((o) => o.id === id) || null;
            return expanded.find((o) => o.gateway === 'razorpay') || null;
        };

        const fromDb = pickFromDb(optId || 'razorpay:test') || pickFromDb('razorpay:live');
        if (fromDb) {
            return cb(null, { ...fromDb, source: 'db' });
        }
        ...
        if (requestedMode === 'live' && env.mode !== 'live') return cb(null, null);
        if (requestedMode === 'test' && env.mode !== 'test') return cb(null, null);
