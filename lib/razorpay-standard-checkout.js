...
function resolveRazorpayForCheckout(db, paymentOptionId, cb) {
    ...
    const optId = String(paymentOptionId || '').trim() || (env ? `razorpay:${env.mode}` : 'razorpay:test');

    if (env) {
        const envMode = env.mode;
        const requestedMode = optId.includes(':') ? optId.split(':')[1] : envMode;
        if (requestedMode === 'live' && envMode !== 'live') {
            return cb(null, null);
        }
        if (requestedMode === 'test' && envMode !== 'test') {
            return cb(null, null);
        }
        return cb(null, {
            id: optId,
            gateway: 'razorpay',
            mode: envMode,
            label: envMode === 'live' ? 'Razorpay' : 'Razorpay (Test)',
    ...
    db.all(`SELECT * FROM payment_gateways`, [], (err, rows) => {
        ...
        const resolved = paymentGatewayOptions.resolvePaymentOption(optId, rows);
        if (!resolved || resolved.gateway !== 'razorpay') {
            return cb(null, null);
        }
        cb(null, { ...resolved, source: 'db' });
    });
}
...
function syncRazorpayEnvToDb(db, cb) {
    ...
    db.get(`SELECT config, is_active FROM payment_gateways WHERE name = ?`, ['razorpay'], (e, row) => {
        ...
        const config = paymentGatewayOptions.migrateLegacyRazorpay(existing);
        if (env.mode === 'test') {
            config.test = {
                enabled: true,
                key_id: env.key_id,
        ...
        db.run(
            ...
            ['razorpay', 1, JSON.stringify(config)],
...
function handleCreateOrder(req, res, deps) {
    ...
    loadRegistrationForPayment(db, registrationId, userId, (eReg, ctx) => {
        ...
        resolveRazorpayForCheckout(db, paymentOption, (eGw, resolved) => {
            ...
            getOrCreatePendingOrder(registrationId, amount, (oErr, orderRow) => {
                ...
                adminPaymentFlow.createRazorpayOrder(
                    rz,
                                    ...
                                    db.run(
                                        ...
                                        () => {
                                            adminPaymentFlow.notifyAfterRegistrationPaid(
                                                ...
                                                {
                                                    invoice_url:
                                                        notifEngine.publicBaseUrl() + '/doctor.html#tab-orders'
                                                },
                                                portalTracking
                                            );
                                            res.json({
...
function handleVerifyPayment(req, res, deps) {
    ...
    function verifyAndFulfill() {
        resolveRazorpayForCheckout(db, paymentOption, (eGw, activeGateway) => {
            if (eGw) return res.status(500).json({ success: false, error: eGw.message });
            if (!activeGateway) {
