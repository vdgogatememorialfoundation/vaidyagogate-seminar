...
function formatJudgeFromDisplay(judge) {
    return require('./staff-display-name').formatStaffFromDisplay(judge, 'Judge');
}

function formatJudgeShortName(judge) {
    return require('./staff-display-name').formatStaffPersonName(judge) === 'Support team'
        ? 'Judge'
        : require('./staff-display-name').formatStaffPersonName(judge);
}

function orgFromEmail() {
    const rt = integrationSettings.getRuntimeIntegrations();
    const smtpCfg = integrationSettings.getMailConfig();
    if (smtpCfg && smtpCfg.from) return smtpCfg.from;
    try {
        const httpCfg = require('./email-http-transport').getHttpEmailConfig();
        if (httpCfg && httpCfg.from) return httpCfg.from;
    } catch (_) {}
    return (
        integrationSettings.normalizeEmail(rt.zoho_from) ||
        integrationSettings.normalizeEmail(process.env.ZOHO_FROM) ||
        integrationSettings.normalizeEmail(process.env.MAIL_FROM) ||
        integrationSettings.normalizeEmail(process.env.ADMIN_CONTACT_EMAIL) ||
...
function publicDoctorPortalUrl() {
    ...
    return String(base).replace(/\/$/, '') + '/doctor';
}
...
function parseParticipantFromSubmission(sub, userRow) {
    ...
    return {
        ...
        email: (userRow.email || fd.email || '').trim(),
...
function notifyParticipantByEmail(judge, participant, subject, bodyText, submissionId) {
    ...
    if (!to) return Promise.resolve({ ok: false, skipped: true, error: 'Participant has no email on file' });
    if (!isOutboundEmailReady()) {
        return Promise.resolve({ ok: false, error: 'Email not configured on server' });
    }
    const fromEmail = orgFromEmail();
    const display = sanitizedJudgeFromDisplay(judge);
    ...
    return emailSvc.sendEmail(to, subjWithRef, html, {
        ...
        fromDisplay: display,
        fromEmail,
        toName: participant.fullName || 'Participant',
...
function notifyJudgeByEmail(judge, participant, subject, bodyText, applicationNo, submissionId) {
    ...
    if (!to) return Promise.resolve({ ok: false, skipped: true, error: 'Judge has no email on file' });
    if (!isOutboundEmailReady()) {
        return Promise.resolve({ ok: false, error: 'Email not configured on server' });
    }
    const fromEmail = orgFromEmail();
...
function ensureEmailIntegrationsLoaded(db) {
    return new Promise((resolve, reject) => {
        integrationSettings.ensureIntegrationSettingsLoaded(db, (e) => (e ? reject(e) : resolve()));
    });
}

function sanitizedJudgeFromDisplay(judge) {
    const http = require('./email-http-transport');
    return http.sanitizeEmailDisplayName(formatJudgeFromDisplay(judge));
}

function logJudgeEmailNotification(db, row, result) {
    const err = result && !result.ok ? [result.error, result.provider && 'provider=' + result.provider, result.hint].filter(Boolean).join(' | ') : null;
    notifEngine.logNotification(db, {
        event_key: row.event_key || 'CASE_MESSAGE_FROM_JUDGE',
        channel: 'email',
        destination: row.to_address,
        user_id: row.user_id || null,
        status: result && result.ok ? 'sent' : result && result.skipped ? 'skipped' : 'failed',
        subject: row.subject,
        body_preview: (row.body_preview || '').slice(0, 500),
        error: err
    });
}

function notifyCaseMessageViaEngine(db, userId, eventKey, vars, emailExtras, cb) {
    if (typeof emailExtras === 'function') {
        cb = emailExtras;
        emailExtras = null;
    }
    if (!userId) return cb && cb(null, { skipped: true });
    notifEngine.notify(db, eventKey, { userId, vars, immediate: true, emailExtras: emailExtras || undefined }, cb);
}

async function sendJudgeMessage(db, opts) {
    await ensureEmailIntegrationsLoaded(db);
    ...
    if (!opts.viaEmail) {
        const subRow = await new Promise((resolve, reject) => {
            db.get(`SELECT user_id FROM case_submissions WHERE id = ?`, [submissionId], (e, row) =>
                e ? reject(e) : resolve(row)
            );
        });
        participantUserId = subRow && subRow.user_id;

        const emailExtras = {
            fromDisplay: sanitizedJudgeFromDisplay(judge),
            replyTo: messageReplyAddress.buildCaseReplyAddress(submissionId, judge.id) || undefined
        };
        ...
        logCommunication(
            db,
            {
                ...
                from_display: emailExtras.fromDisplay
            },
            emailResult.ok ? 'sent' : emailResult.error || 'skipped',
            emailResult.error
        );

        logJudgeEmailNotification(
            db,
            {
        ...
        if (!emailResult.ok && participantUserId && participant.email) {
            await new Promise((resolve) => {
                notifyCaseMessageViaEngine(
                    db,
                    participantUserId,
    ...
    return {
        ...
        emailError: emailResult && !emailResult.ok ? emailResult.error || 'Email not sent' : null,
    ...
    if (!opts.viaEmail) {
        ...
        if (emailResult.ok) {
            logCommunication(db, {
                ...
                body_preview: bodyText,
                to_address: judge.email,
                from_display: participant.fullName + ' (email reply)'
            }, 'sent');
        }
    }
