...
function getWebhookSecret(db, cb) {
    ...
    paymentsMod.loadGatewayCredentials(db, 'razorpay', (err, opt) => {
...
function applyRefundStatusUpdate(db, opts, cb) {
    ...
    db.get(
        ...
        (err, row) => {
            ...
            db.run(
                `UPDATE refunds SET status = ?, provider_status = ?, failure_reason = ?, raw_response = COALESCE(?, raw_response), updated_at = ? WHERE id = ?`,
                [
                    internalStatus,
                    providerStatus,
                    failureReason,
