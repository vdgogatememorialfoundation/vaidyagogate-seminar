...
function ensureCertificateVerifySchema(db, ignoreErr, next) {
    ...
    const ticketAlters = pg
        ? [
              ...
              `ALTER TABLE user_certificates ADD COLUMN IF NOT EXISTS certificate_id_string TEXT`,
              `ALTER TABLE volunteer_certificates ADD COLUMN IF NOT EXISTS certificate_id_string TEXT`
          ]
        : [
              ...
              `ALTER TABLE user_certificates ADD COLUMN certificate_id_string TEXT`,
              `ALTER TABLE volunteer_certificates ADD COLUMN certificate_id_string TEXT`
          ];
...
/** True while the seminar's scheduled certificate release date/time is still in the future. */
function scheduledCertReleasePending(sem) {
    const goLive = parseGoLiveAt(sem && sem.certificate_verify_go_live_at);
    return goLive != null && Date.now() < goLive;
}

/**
 * Certificates are "live" for a seminar once the admin has made them public:
 * verification enabled and (manual override / seminar ended / go-live reached),
 * or the scheduled release time has passed. Until then scans only record eligibility --
 * no certificate email is dispatched.
 */
function certificateReleaseLive(sem) {
    if (!sem) return false;
    const goLive = parseGoLiveAt(sem.certificate_verify_go_live_at);
    if (goLive != null && Date.now() >= goLive) return true;
    return isPublicCertVerifyLive(sem);
}
...
function doctorCertificateViewState(row) {
    ...
    if (!countdown && scheduledCertReleasePending(sem)) {
        countdown = { at: parseGoLiveAt(sem.certificate_verify_go_live_at), label: 'Certificate available in' };
    }
...
/** Exactly 12 numeric digits for display / certificate ID. */
function normalizeCertId12(raw) {
...
function generateCertId12() {
    let id = '';
    for (let i = 0; i < 12; i++) id += Math.floor(Math.random() * 10).toString();
    return id;
}

function certIdStringUnique(db, candidate, cb) {
    const c = normalizeCertId12(candidate);
    if (!c) return cb(null, false);
    db.get(
        `SELECT 1 AS ok FROM user_certificates WHERE certificate_id_string = ?
         UNION SELECT 1 FROM volunteer_certificates WHERE certificate_id_string = ? LIMIT 1`,
        [c, c],
        (e, row) => {
            if (e) return cb(e);
            cb(null, !row);
...
function ensureCertificateIdString(db, certKind, certId, applicationNo, cb) {
    const id = parseInt(certId, 10);
    if (!Number.isInteger(id) || id < 1) return cb(new Error('Invalid certificate id'));
    const table = certKind === 'volunteer' ? 'volunteer_certificates' : 'user_certificates';
    db.get(`SELECT certificate_id_string FROM ${table} WHERE id = ?`, [id], (e, row) => {
        if (e) return cb(e);
        if (!row) return cb(new Error('Certificate not found'));
        const existing = normalizeCertId12(row.certificate_id_string);
        if (existing) return cb(null, existing);
        const tryAssign = (left) => {
            if (left <= 0) return cb(new Error('Could not assign unique certificate ID'));
            const next = generateCertId12();
            certIdStringUnique(db, next, (e2, unique) => {
                if (e2) return cb(e2);
                if (!unique) return tryAssign(left - 1);
                const assigned = normalizeCertId12(next);
                db.run(
                    `UPDATE ${table} SET certificate_id_string = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
                    [assigned, id],
                    (e3) => {
                        if (e3) return cb(e3);
                        cb(null, assigned);
...
function qrDataUrlForToken(token, cb) {
    const QRCode = require('qrcode');
    const url = publicVerifyUrl(token);
    QRCode.toDataURL(url, { width: 220, margin: 1, errorCorrectionLevel: 'M' }, (err, dataUrl) => {
...
function publicVerifyUrl(token) {
    ...
    return `${base}/verify-certificate?t=${encodeURIComponent(token)}`;
}
...
function notifyCertificateAvailable(db, seminarId, row, certKind, cb) {
    ...
    ensureCertVerifyToken(db, row.cert_id, certKind, (e3, tok) => {
        if (e3) return cb(e3);
        ensureCertificateIdString(db, certKind, row.cert_id, chk.applicationNo, (e4, certId12) => {
            if (e4) return cb(e4);
            ...
            notifEngine.notify(
                ...
                'CERTIFICATE_AVAILABLE',
                {
                    ...
                    immediate: true,
                    vars: {
                        certificate_id: certId12,
                        certificate_url: viewUrl,
                        verify_url: verifyUrl,
                        application_no: chk.applicationNo,
                        prn_no: chk.prn,
                        event_name:
                            String(row.event_name || row.scan_event_title || row.sub_event_title || row.seminar_title || '').trim() ||
                            undefined
                    }
                },
                (nErr) => {
...
function finalizeParticipantCertificateAfterScan(db, ticketId, deps, cb) {
    const { notifEngine, portalTracking, promoteRegistrationToCertificateIssued, scanCount } = deps || {};
    const tid = parseInt(ticketId, 10);
    if (!Number.isInteger(tid) || tid < 1) return cb && cb(null, { skipped: true });

    const q = `
        SELECT t.id AS ticket_id, IFNULL(t.scan_count, 0) AS scan_count, t.user_id, IFNULL(t.event_id, 0) AS event_id,
               ...
               s.event_date,
               IFNULL(s.certificate_verify_enabled, 0) AS certificate_verify_enabled,
               IFNULL(s.certificate_verify_manual, 0) AS certificate_verify_manual,
               se.title AS sub_event_title,
               CASE WHEN se.title IS NOT NULL AND trim(se.title) != '' THEN se.title ELSE s.title END AS scan_event_title,
    ...
    db.get(q, [tid], (err, row) => {
        ...
        if (scanCount != null && Number.isFinite(Number(scanCount))) {
            row.scan_count = Number(scanCount);
        }
        ...
        // Never dispatch the certificate email on scan alone -- hold until the admin
        // makes certificates live for this seminar (verification settings / go-live).
        const scheduledRelease = !certificateReleaseLive(row);

        const eventName =
            String(row.scan_event_title || row.sub_event_title || row.seminar_title || '').trim() || 'National Seminar';
            ...
            (eTpl, templateId) => {
                ...
                db.get(
                    ...
                    (eFind, existing) => {
                        const finishCert = (certDbId) => {
                            db.get(
                                ...
                                (e2, certRow) => {
                                    ...
                                    const afterPromote = () => {
                                        if (String(certRow.cert_dispatched_at || '').trim()) {
                                            return cb(null, {
                                                issued: true,
                                                certId: certRow.cert_id,
                                                notified: false,
                        ...
                        const enabledVal = scheduledRelease ? 0 : 1;
                        if (existing && existing.id) {
                            return db.run(
                                `UPDATE user_certificates SET ticket_id = ?, registration_id = ?, template_id = COALESCE(?, template_id), enabled = CASE WHEN ? = 1 THEN 1 ELSE user_certificates.enabled END, scan_verified = 1, event_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
                                [row.ticket_id, row.registration_id, templateId, enabledVal, eventId, existing.id],
                                (eUp) => {
                                    if (eUp) return cb(eUp);
                                    finishCert(existing.id);
                                }
                            );
                        }
                        // Upsert on (user_id, seminar_id): one common participation certificate
                        // per seminar even when multiple day tickets are scanned.
                        db.run(
                            `INSERT INTO user_certificates (user_id, seminar_id, registration_id, ticket_id, template_id, enabled, scan_verified, event_id, updated_at)
                             ...
                             ON CONFLICT (user_id, seminar_id) DO UPDATE SET
                               ...
                               enabled = CASE WHEN excluded.enabled = 1 THEN 1 ELSE user_certificates.enabled END,
                               scan_verified = 1,
                               event_id = COALESCE(excluded.event_id, user_certificates.event_id),
...
/**
 * When a seminar's scheduled certificate release time passes, flip eligible scan-verified
 * certificates to enabled and notify the doctor once (dispatch marks via dispatched_at).
 * Pass userId to limit the sweep to one doctor; pass null to sweep everyone.
 */
function releaseScheduledCertificatesCore(db, userId, deps, cb) {
    const { promoteRegistrationToCertificateIssued } = deps || {};
    const uid = userId == null ? null : parseInt(userId, 10);
    if (userId != null && (!Number.isInteger(uid) || uid < 1)) return cb && cb(null, { released: 0 });
    const whereUser = uid != null ? ' AND uc.user_id = ?' : '';
    const params = uid != null ? [uid] : [];
    db.all(
        `SELECT uc.id AS cert_id, uc.user_id, uc.seminar_id, uc.registration_id,
                COALESCE(uc.dispatched_at, '') AS cert_dispatched_at,
                s.title AS seminar_title, s.certificate_verify_go_live_at,
                s.event_date,
                IFNULL(s.certificate_verify_enabled, 0) AS certificate_verify_enabled,
                IFNULL(s.certificate_verify_manual, 0) AS certificate_verify_manual,
                r.application_no, u.user_id_string,
                o.status AS order_status
         FROM user_certificates uc
         JOIN seminars s ON s.id = uc.seminar_id
         JOIN registrations r ON r.id = uc.registration_id
         ...
         WHERE COALESCE(uc.scan_verified, 0) = 1 AND COALESCE(uc.enabled, 0) = 0${whereUser}`,
        params,
        (err, rows) => {
            if (err) {
                if (/does not exist|no such table|no such column/i.test(String(err.message || ''))) {
                    return cb && cb(null, { released: 0 });
                }
                return cb && cb(err);
            }
            const due = (rows || []).filter(
                (r) =>
                    certificateReleaseLive(r) &&
                    String(r.order_status || '').toLowerCase() === 'success'
            );
            if (!due.length) return cb && cb(null, { released: 0 });
            let done = 0;
            let released = 0;
            const finish = () => cb && cb(null, { released });
            due.forEach((r) => {
                db.run(
                    `UPDATE user_certificates SET enabled = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
                    [r.cert_id],
                    (eUp) => {
                        if (eUp) console.warn('[cert-release]', eUp.message);
                        else released++;
                        const after = () => {
                            if (++done === due.length) finish();
                        };
                        if (!eUp && typeof promoteRegistrationToCertificateIssued === 'function') {
                            promoteRegistrationToCertificateIssued(r.registration_id, () => {});
                        }
                        after();
                        if (eUp || String(r.cert_dispatched_at || '').trim()) return;
                        notifyCertificateAvailable(
                            db,
                            r.seminar_id,
                            {
                                ...
                                event_name: r.seminar_title,
                                seminar_title: r.seminar_title
                            },
                            'participant',
                            (nErr) => {
                                if (nErr) console.warn('[cert-release] notify:', nErr.message);
                            }
                        );
                    }
                );
...
function releaseScheduledVolunteerCertificatesCore(db, userId, cb) {
    const uid = userId == null ? null : parseInt(userId, 10);
    if (userId != null && (!Number.isInteger(uid) || uid < 1)) return cb && cb(null, { released: 0 });
    const whereUser = uid != null ? ' AND vc.user_id = ?' : '';
    const params = uid != null ? [uid] : [];
    db.all(
        `SELECT vc.id AS cert_id, vc.user_id, vc.seminar_id, vc.registration_id,
                COALESCE(vc.dispatched_at, '') AS cert_dispatched_at,
                s.title AS seminar_title, s.certificate_verify_go_live_at,
                s.event_date,
                IFNULL(s.certificate_verify_enabled, 0) AS certificate_verify_enabled,
                IFNULL(s.certificate_verify_manual, 0) AS certificate_verify_manual,
         ...
         JOIN seminars s ON s.id = vc.seminar_id
         LEFT JOIN registrations r ON r.id = vc.registration_id
         LEFT JOIN seminar_volunteers sv ON sv.user_id = vc.user_id AND sv.seminar_id = vc.seminar_id
         JOIN users u ON u.id = vc.user_id
         WHERE COALESCE(vc.scan_verified, 0) = 1 AND COALESCE(vc.enabled, 0) = 0${whereUser}`,
        params,
        (err, rows) => {
            if (err) {
                if (/does not exist|no such table|no such column/i.test(String(err.message || ''))) {
            ...
            const due = (rows || []).filter((r) => certificateReleaseLive(r));
            if (!due.length) return cb && cb(null, { released: 0 });
            let done = 0;
            let released = 0;
            const finish = () => cb && cb(null, { released });
            due.forEach((r) => {
                db.run(
                    `UPDATE volunteer_certificates SET enabled = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
                    [r.cert_id],
                    (eUp) => {
                        if (eUp) console.warn('[cert-release] volunteer:', eUp.message);
                        else released++;
                        const after = () => {
                            if (++done === due.length) finish();
                        };
                        ...
                        notifyCertificateAvailable(
                            ...
                            'volunteer',
                            (nErr) => {
                                if (nErr) console.warn('[cert-release] volunteer notify:', nErr.message);
                            }
                        );
                    }
                );
...
function releaseScheduledCertificatesForUser(db, userId, deps, cb) {
    releaseScheduledCertificatesCore(db, userId, deps, (e, out) => {
        if (e) return cb && cb(e);
        releaseScheduledVolunteerCertificatesCore(db, userId, (e2, out2) => {
            if (e2) return cb && cb(e2);
            cb && cb(null, { released: (out ? out.released : 0) + (out2 ? out2.released : 0) });
        });
    });
}

function releaseScheduledCertificatesForAll(db, deps, cb) {
    releaseScheduledCertificatesForUser(db, null, deps, cb);
}
...
module.exports = {
    ...
    scheduledCertReleasePending,
    certificateReleaseLive,
    releaseScheduledCertificatesForUser,
    releaseScheduledCertificatesForAll,
    doctorCertificateViewState,
    generateVerifyToken,
    normalizeCertId12,
    ensureCertificateIdString,
    qrDataUrlForToken,
    publicVerifyUrl,
    qrImageUrl,
