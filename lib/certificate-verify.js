                ...
                db.get(
                    ...
                    (eFind, existing) => {
                        ...
                        if (existing && existing.id) {
                            return db.run(
                                `UPDATE user_certificates SET ticket_id = ?, registration_id = ?, template_id = COALESCE(?, template_id), enabled = 1, scan_verified = 1, event_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
                                [row.ticket_id, row.registration_id, templateId, eventId, existing.id],
                                (eUp) => {
