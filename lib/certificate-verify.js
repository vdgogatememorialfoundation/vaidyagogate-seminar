    ...
    if (tok) {
        ...
        const volunteerSql = `
            SELECT vc.id AS cert_id, vc.user_id, vc.seminar_id, vc.enabled, vc.verify_token, vc.dispatched_at,
                   ...
                   sv.volunteer_ticket_id_string AS application_no, u.user_id_string, u.email, u.phone,
                   s.id AS sem_id, s.title AS seminar_title, s.event_date, s.certificate_verify_enabled,
                   s.certificate_verify_manual, s.certificate_verify_go_live_at
        ...
        return db.get(participantSql, [tok], (e, row) => {
            if (e) return cb(e);
            const runRow = (r) => {
                if (!r) return cb(null, { ok: false, error: 'No matching issued certificate found.' });
                const sem = {
        ...
        (e, sem) => {
            ...
            afterSeminarGate(sem, (semRow) => {
                ...
                if (app) {
                    ...
                    filterV = ` AND sv.volunteer_ticket_id_string = ?`;
                    extra.push(app);
                } else {
                ...
                const runSearch = (sql, filter, cbRows) => {
                    db.all(sql + filter + ` LIMIT 2`, baseParams.concat(extra), cbRows);
                };
                runSearch(participantSql, filterP, (e2, pRows) => {
                    if (e2) return cb(e2);
                    runSearch(volunteerSql, filterV, (e3, vRows) => {
                        if (e3) return cb(e3);
                        const rows = (pRows || []).concat(vRows || []);
                        if (!rows.length) {
                        ...
                        (eTplV, tplV) => {
                            ...
                            db.all(
                                ...
                                (e2, pRows) => {
                                    if (e2) return cb(e2);
                                    db.all(
                                        `SELECT vc.id AS cert_id, vc.user_id, sv.volunteer_ticket_id_string AS application_no,
