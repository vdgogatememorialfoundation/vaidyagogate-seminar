...
function finalizeParticipantCertificateAfterScan(db, ticketId, deps, cb) {
    ...
    db.get(q, [tid], (err, row) => {
        ...
        const scheduledRelease = scheduledCertReleasePending(row);
    ...
    db.all(
        ...
        (err, rows) => {
            ...
            const due = (rows || []).filter(
                (r) =>
                    r.certificate_verify_go_live_at &&
                    parseGoLiveAt(r.certificate_verify_go_live_at) != null &&
                    parseGoLiveAt(r.certificate_verify_go_live_at) <= Date.now() &&
