...
function finalizeParticipantCertificateAfterScan(db, ticketId, deps, cb) {
    ...
    const q = `
        ...
        LEFT JOIN user_certificates uc ON uc.user_id = t.user_id AND uc.seminar_id = r.seminar_id AND IFNULL(uc.event_id, 0) = IFNULL(t.event_id, 0)
        WHERE t.id = ?
    `;
    db.get(q, [tid], (err, row) => {
        ...
        volunteerCertFlow.ensureBuiltinTemplateIfMissing(
            ...
            (eTpl, templateId) => {
                ...
                db.get(
                    `SELECT id FROM user_certificates WHERE user_id = ? AND seminar_id = ? AND IFNULL(event_id, 0) = ?`,
                    [row.user_id, row.seminar_id, eventId],
                    (eFind, existing) => {
                        ...
                        if (existing && existing.id) {
                            return db.run(
                                `UPDATE user_certificates SET ticket_id = ?, registration_id = ?, template_id = COALESCE(?, template_id), enabled = 1, scan_verified = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
                                [row.ticket_id, row.registration_id, templateId, existing.id],
                                (eUp) => {
