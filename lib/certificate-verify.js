...
function listPublicVerifySeminars(db, cb) {
    db.all(
        `SELECT id, title, event_date, certificate_verify_enabled
         FROM seminars
        ...
        (e, rows) => {
            if (e) return cb(e);
            const out = (rows || []).filter((r) => isSeminarEnded(r.event_date));
            cb(
                null,
                out.map((r) => ({
                    id: r.id,
...
function resolveCertForPublicLookup(db, { seminarId, applicationNo, prn, token }, cb) {
    ...
    function afterSeminarGate(sem, runQuery) {
        ...
        if (!isSeminarEnded(sem.event_date)) {
            return cb(null, {
                ok: false,
                error: 'Certificate verification opens after the seminar has ended.'
            });
        }
        runQuery(sem);
    }
    ...
    db.get(
        `SELECT id, title, event_date, certificate_verify_enabled FROM seminars WHERE id = ?`,
        [sid],
        (e, sem) => {
            if (e) return cb(e);
