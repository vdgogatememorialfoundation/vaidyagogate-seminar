...
function fetchPayment(db, paymentId, cb) {
    loadCredentials(db, (err, creds) => {
        ...
        rzCall((next) => rz.payments.fetch(paymentId, next), (fErr, payment) => {
            if (fErr) {
...
function capturePayment(rz, payment, cb) {
    rzCall((next) => rz.payments.capture(payment.id, payment.amount, payment.currency || 'INR', next), (err, captured) => {
        if (err) {
        ...
        ensureCaptured((cErr, payment, blockMsg) => {
            ...
            findExistingMatch(db, pid, (e2, existing) => {
                ...
                db.get(REG_SELECT + ` WHERE r.id = ?`, [registrationId], (e3, regRow) => {
                    ...
                    if (!regRow.paid_order_id || existing) return proceed();
                    // Already paid (e.g. marked paid manually / e-ticket issued): link this payment to the
                    // existing successful order unless it is already tied to a different Razorpay payment.
                    db.get(
                        `SELECT provider_transaction_id FROM orders WHERE id = ?`,
                        [regRow.paid_order_id],
                        (e4, ord) => {
                            if (e4) return cb(e4);
                            const cur = String((ord && ord.provider_transaction_id) || '').trim();
                            if (cur && cur !== pid) {
                                return cb(null, {
                                    ok: false,
                                    error: `This registration's payment is already linked to Razorpay payment ${cur}. Refund the duplicate in Razorpay instead.`
                                });
                            }
                            proceed();
                        }
