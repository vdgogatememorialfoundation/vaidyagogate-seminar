        ...
        (err, row) => {
            ...
            db.run(
                ...
                (uErr) => {
                    ...
                    const afterSync = () => {
                        const finalizeCert = (next) => {
                            if (typeof deps.finalizeParticipantCertificateAfterScan !== 'function') {
                                return next();
                            }
                            deps.finalizeParticipantCertificateAfterScan(db, row.ticket_id, deps, () => next());
                        };
                        finalizeCert(() => {
                            db.run(
