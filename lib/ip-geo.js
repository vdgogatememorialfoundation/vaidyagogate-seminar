...
function hasCoordinates(geo) {
    return !!(geo && geo.lat != null && geo.lon != null && Number.isFinite(Number(geo.lat)) && Number.isFinite(Number(geo.lon)));
}
...
if (process.env.NODE_ENV === 'production' && !googleMapsApiKey()) {
    console.warn('[ip-geo] GOOGLE_MAPS_API_KEY is not set — Google city/state/country enrichment is disabled');
}
...
function resolveVisitorGeo(opts, cb) {
    ...
    if (!clientGeo && opts && opts.prevGeo) {
        let stored = null;
        try {
            stored = typeof opts.prevGeo === 'string' ? JSON.parse(opts.prevGeo) : opts.prevGeo;
        } catch (_) {}
        if (stored && hasCoordinates(stored) && !hasPlaceNames(stored)) {
            return enrichGeoPlaceNames(normalizeGeo(stored, ip), (err, enriched) => {
                if (enriched && hasPlaceNames(enriched)) return cb(null, enriched);
                lookupIpGeo(ip, cb);
            });
        }
    }
...
function hasIncompleteGeo(geoJson) {
    if (!geoJson) return false;
    let g = geoJson;
    if (typeof geoJson === 'string') {
        try {
    ...
    return hasCoordinates(g) && !hasPlaceNames(g);
}

function needsGeoLookup(prevGeo, clientGeo) {
    if (clientGeo) return true;
    if (!prevGeo) return true;
    if (hasUsableGeo(prevGeo)) return false;
    return true;
}
