...
function lookupIpGeo(ip, cb) {
    ...
    lookupIpWhoIs(clean, (err1, geo1) => {
        if (geo1 && geo1.label) return cb(null, geo1);
        lookupIpApiCo(clean, (err2, geo2) => {
            if (geo2 && geo2.label) return cb(null, geo2);
            lookupIpApiCom(clean, (err3, geo3) => {
                cb(null, geo3 || geo2 || geo1 || null);
            });
        });
    });
}
...
function reverseGeocodeGoogle(lat, lon, cb) {
    ...
    fetchJson(url, { timeout: 6000 }, (err, j) => {
        if (!j || j.status !== 'OK' || !j.results || !j.results[0]) return cb(null, null);
...
function hasUsableGeo(geoJson) {
    ...
    return !!(g && (g.label || g.city || g.country || (g.lat != null && g.lon != null)));
}
