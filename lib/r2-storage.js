...
function formatR2Error(err) {
    ...
    if (/NoSuchBucket|bucket does not exist/i.test(msg + code)) {
        ...
        return (
            ...
            'In Render → Environment, set R2_BUCKET_NAME to the exact bucket name shown in Cloudflare R2 (create the bucket first if needed).'
        );
    }
...
function getR2SetupHint() {
    if (!isR2Configured()) {
        return 'R2 is not configured. Set R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, and R2_BUCKET_NAME on Render.';
    }
    if (_readyCache.error) return _readyCache.error;
    return `Check R2 bucket name "${getBucket()}" in Cloudflare and Render env vars.`;
}
...
async function warmupR2() {
    if (!isR2Configured()) {
        console.log('[r2] not configured — case files use server upload (enable R2 for large files)');
        return false;
    }
