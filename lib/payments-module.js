...
function ensurePaymentsModuleSchema(db, ignoreErr, next) {
    const steps = [
        ...
        `ALTER TABLE cancellation_requests ADD COLUMN action_type TEXT`,
        `ALTER TABLE cancellation_requests ADD COLUMN initiated_by TEXT`,
        ...
        `ALTER TABLE refunds ADD COLUMN updated_at DATETIME`,
        `ALTER TABLE refunds ADD COLUMN provider_payment_id TEXT`,
        `ALTER TABLE refunds ADD COLUMN bank_utr TEXT`,
...
function resolveGatewayFromOrder(order) {
    ...
    if (gw.startsWith('juspay')) return 'juspay';
    return gw || 'mock';
}

function loadGatewayCredentials(db, gatewayName, cb) {
    return pgOpts.loadGatewayCredentials(db, gatewayName, cb);
}
...
function executeGatewayRefundWithDb(db, order, amountRupees, cb) {
    ...
    const isInsufficientRefundBalance = (msg) =>
        /insufficient|low balance|add funds|top[- ]?up|not enough balance|insufficient funds/i.test(
            String(msg || '')
        );
    ...
    loadGatewayCredentials(db, gatewayName, async (err, opt) => {
        ...
        if (gatewayName === 'razorpay') {
            ...
            if (r && !r.ok && isInsufficientRefundBalance(r.error)) {
                return cb(null, {
                    ok: false,
                    manualRequired: true,
                    gateway: 'razorpay',
                    amount: refundAmt,
                    error:
                        'Razorpay refund balance appears insufficient. Refund is queued as manual_pending — add gateway funds and click Sync / Process again.'
                });
            }
...
function recordRefund(db, { orderId, registrationId, amount, percent, gateway, providerRefundId, providerPaymentId, providerStatus, status, failureReason, raw, bankUtr }, cb) {
    const now = new Date().toISOString();
    let utr = String(bankUtr || '').trim();
    if (!utr && raw) {
        try {
            const rzSync = require('./razorpay-refund-sync');
            utr = rzSync.extractBankUtrFromRazorpayRefund(typeof raw === 'string' ? JSON.parse(raw) : raw);
        } catch (_) {}
    }
    db.run(
        `INSERT INTO refunds (order_id, registration_id, amount, percent, gateway, provider_refund_id, provider_payment_id, provider_status, status, failure_reason, raw_response, bank_utr, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
            orderId,
            registrationId,
        ...
        function (insErr) {
            ...
            const orderStatus = String(status || '').toLowerCase() === 'completed' ? 'refunded' : 'refund_pending';
            db.run(
...
function processOrderRefund(db, { orderId, amountRupees, percent, reason, adminUserId }, cb) {
    db.get(`SELECT o.*, r.id AS registration_id FROM orders o JOIN registrations r ON r.id = o.registration_id WHERE o.id = ?`, [orderId], (err, order) => {
        ...
        executeGatewayRefundWithDb(db, order, refundAmt, (gErr, gw) => {
            ...
            const st = gw.manualRequired
                ? 'manual_pending'
                : !gw.ok
                  ? 'failed'
                  : require('./razorpay-refund-sync').deriveRefundStatusFromGatewayResult(gw, refundAmt);
            recordRefund(
                db,
                {
                    orderId: order.id,
                    ...
                    providerPaymentId: gw.paymentId || (gw.raw && gw.raw.payment_id) || order.provider_transaction_id,
                    providerStatus: gw.providerStatus || (gw.raw && gw.raw.status) || null,
                    status: st,
                    failureReason: st === 'failed' ? gw.error || null : null,
                    raw: gw.raw || gw
                },
                (rErr) => {
                    ...
                    cb(null, {
                        ...
                        message: gw.manualRequired
                            ...
                            : st === 'completed'
                              ? `Refund of ₹${refundAmt} completed at Razorpay.`
                              : `Refund of ₹${refundAmt} initiated — tracking Razorpay status.`
                    });
                }
            );
        });
