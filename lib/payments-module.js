...
function recordRefund(db, { orderId, registrationId, amount, percent, gateway, providerRefundId, providerPaymentId, providerStatus, status, failureReason, raw }, cb) {
    const now = new Date().toISOString();
    db.run(
        `INSERT INTO refunds (order_id, registration_id, amount, percent, gateway, provider_refund_id, provider_payment_id, provider_status, status, failure_reason, raw_response, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
            orderId,
            registrationId,
