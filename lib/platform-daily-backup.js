...
async function uploadToGoogleDrive(config, filename, buffer) {
    ...
    if (!String(config.googleDriveFolderId || '').trim()) {
        throw new Error(
            'Google Drive folder ID is required — use a Shared drive folder or a My Drive folder owned by the impersonated user.'
        );
    }
    const impersonate =
        String(config.googleDriveImpersonateEmail || process.env.GOOGLE_DRIVE_IMPERSONATE_EMAIL || '').trim() || '';
