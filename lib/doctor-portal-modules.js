...
/** Always return an explicit tab id list for the client (never null). */
function allowedSetToTabArray(allowedSet) {
    if (allowedSet === null) return TAB_DEFS.map((d) => d[0]);
    const out = Array.from(allowedSet);
    if (allowedSet.has('tab-support') && !allowedSet.has('tab-live-chat')) {
