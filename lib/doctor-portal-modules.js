...
function isFullTabWhitelist(userModulesRaw) {
    const userMap = parseModulesJson(userModulesRaw);
    if (!userMap) return false;
    const san = sanitizeModulesInput(userMap);
    const keys = Object.keys(san);
    if (!keys.length) return false;
    if (keys.some((k) => san[k] === false)) return false;
    return [...KNOWN_TABS].every((tabId) => san[tabId] === true);
}

function userHasCustomModules(userModulesRaw) {
    if (isLegacyVolunteerDefaultModules(userModulesRaw)) return false;
    if (isFullTabWhitelist(userModulesRaw)) return false;
    const userMap = parseModulesJson(userModulesRaw);
    return !!(userMap && Object.keys(userMap).length);
}

function allowedSetToTabArray(allowedSet) {
    const expanded = expandRefundTabAccess(allowedSet);
    if (expanded === null) return TAB_DEFS.map((d) => d[0]);
    const out = Array.from(expanded);
    if (expanded.has('tab-support') && !expanded.has('tab-live-chat')) {
        out.push('tab-live-chat');
    }
    return out;
}

/** Refund tracking sidebar tab — auto-enabled when payments / applications / orders are on the whitelist. */
function expandRefundTabAccess(allowedSet) {
    if (!allowedSet || allowedSet.has('tab-refunds')) return allowedSet;
    if (
        allowedSet.has('tab-payments') ||
        allowedSet.has('tab-applications') ||
        allowedSet.has('tab-orders')
    ) {
        const out = new Set(allowedSet);
        out.add('tab-refunds');
        return out;
    }
    return allowedSet;
...
module.exports = {
    ...
    isFullTabWhitelist,
    allowedSetToTabArray,
    expandRefundTabAccess
};
