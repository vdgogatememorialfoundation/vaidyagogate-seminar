/**
 * Ensure admin logins exist from Render / .env:
...
function parseBootstrapAdmins() {
    ...
    if (!list.length && !require('./hosting').isRender() && process.env.NODE_ENV !== 'production') {
        list.push({
...
function ensureBootstrapAdmin(db, generateId, cb) {
    ...
    if (!admins.length) {
        if (require('./hosting').isProduction()) {
            console.warn(
                '[admin] Set ADMIN_EMAIL + ADMIN_PASSWORD (and optionally ADMIN_EMAIL_2, CO_ADMIN_EMAIL) in Render env.'
            );
        }
        return cb && cb();
    }
