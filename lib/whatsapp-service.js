...
function expandAuthTemplateLanguages(metaLangs) {
    const out = [];
    const add = (c) => {
        const x = String(c || '').trim();
        if (x && !out.includes(x)) out.push(x);
    };
    (metaLangs || []).forEach((lang) => {
        add(lang);
        if (lang === 'en') {
            add('en_US');
            add('en_GB');
        }
        if (lang === 'en_US') add('en');
    });
    return out;
}
...
function buildTemplateLanguage(langCode) {
    const code = String(langCode || 'en').trim();
    return { code, policy: 'deterministic' };
}
...
async function sendWhatsAppOtpTemplate(phone, templateName, code) {
    ...
    const langCandidates = metaLangs.length
        ? expandAuthTemplateLanguages(metaLangs)
        : whatsappLangCandidates(templateLang);
    ...
    for (const langCode of langCandidates) {
        ...
        for (const attempt of attempts) {
            triedMethods.push(langCode + ':' + attempt.label);
            const r = await postWhatsAppMessage({
                ...
                template: {
                    name: tplName,
                    language: buildTemplateLanguage(langCode),
