...
async function sendWhatsAppTemplate(phone, templateName, bodyParams, opts) {
    ...
    const buildPayload = (lang, params) => {
        const components = [];
        if (options.headerComponent) components.push(options.headerComponent);
        ...
        if (Array.isArray(options.extraComponents)) components.push(...options.extraComponents);
        return {
...
/**
 * Media / document message (public https link). mediaType: image | document | video | audio.
 * Only works inside the 24h customer-service window or after a template message.
 */
async function sendWhatsAppMedia(phone, media) {
    const to = normalizePhoneE164(phone);
    if (!to) return { ok: false, error: 'Invalid phone' };
    const m = media || {};
    const type = ['image', 'document', 'video', 'audio'].includes(String(m.type)) ? String(m.type) : 'document';
    const link = String(m.link || '').trim();
    if (!/^https:\/\//i.test(link)) return { ok: false, error: 'Media link must be a public https URL' };
    const obj = { link };
    if (m.caption && type !== 'audio') obj.caption = String(m.caption).slice(0, 1024);
    if (type === 'document' && m.filename) obj.filename = String(m.filename).slice(0, 240);
    const payload = { messaging_product: 'whatsapp', to, type };
    payload[type] = obj;
    return postWhatsAppMessage(payload);
}

/** Interactive Flow message (WhatsApp Flows). */
async function sendWhatsAppFlow(phone, flow) {
    const to = normalizePhoneE164(phone);
    if (!to) return { ok: false, error: 'Invalid phone' };
    const f = flow || {};
    if (!f.flowId) return { ok: false, error: 'flowId required' };
    const params = {
        flow_message_version: '3',
        flow_token: String(f.flowToken || ''),
        flow_id: String(f.flowId),
        flow_cta: String(f.cta || 'Open').slice(0, 20),
        flow_action: f.firstScreen ? 'navigate' : 'data_exchange'
    };
    if (f.firstScreen) {
        params.flow_action_payload = { screen: String(f.firstScreen), data: f.data || undefined };
    }
    return postWhatsAppMessage({
        messaging_product: 'whatsapp',
        to,
        ...
        interactive: {
            type: 'flow',
            header: f.header ? { type: 'text', text: String(f.header).slice(0, 60) } : undefined,
            body: { text: String(f.body || 'Tap to continue').slice(0, 1024) },
            footer: f.footer ? { text: String(f.footer).slice(0, 60) } : undefined,
...
/**
 * WhatsApp Payments India - order details / review and pay.
 *
 * `amount` is supplied in INR and converted to paise.
 * Meta payment configuration name must match the configuration
 * configured in WhatsApp Manager. Current configuration: Razorpay.
 */
async function sendWhatsAppOrderDetails(phone, order) {
    const to = normalizePhoneE164(phone);
    if (!to) {
        return {
            ok: false,
    ...
    const amountRupees = Number(o.amount);
    ...
    if (!referenceId) {
        return {
            ok: false,
            error: 'Payment reference ID required'
        };
    }
...
/** All message templates on the configured WABA (name, language, status, category, components). */
async function listAllMetaTemplates() {
    const wabaId = await resolveWabaId();
    if (!wabaId) return { ok: false, error: 'WABA id not configured', templates: [] };
    try {
        const all = await listMessageTemplatesOnWaba(wabaId);
        return {
            ok: true,
            templates: all.map((t) => ({
                ...
                status: t.status,
                category: t.category,
                bodyVariableCount: countBodyVariablesInMetaTemplate(t),
                headerType: (() => {
                    const h = (t.components || []).find((c) => String(c.type).toUpperCase() === 'HEADER');
                    return h ? String(h.format || 'TEXT').toLowerCase() : '';
                })(),
                bodyText: (() => {
        ...
        return { ok: false, error: formatWhatsAppError(e), templates: [] };
    }
}
