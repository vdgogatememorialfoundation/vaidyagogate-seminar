...
const FULLY_ADMITTED_STATUSES = ['completed', 'checked_in', 'e_ticket_issued', 'certificate_issued'];

function isConfirmedParticipant(row) {
    if (!row) return false;
    const status = String(row.status || '').toLowerCase();
    if (['cancelled', 'rejected', 'submitted', 'waitlisted', 'pending_approval', 'revision_required'].includes(status)) {
        return false;
    }
    const pay = String(row.order_status || row.payment_status || '').toLowerCase();
    const hasTicket = !!(row.ticket_id_string || row.ticket_id);
    if (pay !== 'success' && !hasTicket) return false;

    // Paid / ticketed and fully admitted — include on public delegate lists
    if (FULLY_ADMITTED_STATUSES.includes(status) && (pay === 'success' || hasTicket)) {
        return true;
    }
