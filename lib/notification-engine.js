 ...
 */
const { sendEmail, isEmailConfigured, isConnectionLikeError } = require('./email-service');
const integrationSettings = require('./integration-settings');
const { formatSeminarDateTime, formatSeminarDateForTicketLine, formatSeminarSchedule, formatSeminarDayLine } = require('./seminar-datetime');
const {
...
const { sendMsg91Sms, isMsg91Configured, plainTextForSms } = require('./msg91-service');
const { DEFAULT_TEMPLATES, EVENT_KEYS, buildTransactionDetailsHtml, formatGatewayLabel, finalizeEmailHtml } = require('./notification-defaults');
...
function upgradeStaleAccountCreatedTemplate(db, row) {
    if (!row) return row;
    const html = String(row.email_html || '');
    if (html.includes('{{portal_name}}')) return row;
    const def = DEFAULT_TEMPLATES.find((t) => t.event_key === 'ACCOUNT_CREATED');
    if (!def) return row;
    const upgraded = Object.assign({}, row, {
        email_subject: def.email_subject,
        email_html: def.email_html,
        whatsapp_body: def.whatsapp_body || row.whatsapp_body
    });
    db.run(
        `UPDATE notification_templates SET email_subject = ?, email_html = ?, whatsapp_body = ?, updated_at = CURRENT_TIMESTAMP
         WHERE event_key = ? AND seminar_id IS NULL`,
        [def.email_subject || '', def.email_html || '', def.whatsapp_body || '', 'ACCOUNT_CREATED'],
        () => {}
    );
    return upgraded;
}
...
function loadTemplate(db, eventKey, seminarId, cb) {
    ...
    const deliver = (row) => {
        if (row && eventKey === 'ACCOUNT_CREATED') {
            row = upgradeStaleAccountCreatedTemplate(db, row);
        }
        cb(null, row);
    };
    const finish = (row) => {
        if (row) return deliver(row);
        db.get(
...
function buildVarsFromRows(user, seminar, reg, order, extra, refund, ticket) {
    ...
    let accountPortal = null;
    try {
        accountPortal = require('./portal-urls').resolveAccountPortal(user);
    } catch (_) {
        accountPortal = null;
    }
    const vars = {
        ...
        payment_link: doctorPortalUrl().replace(/#.*$/, '') + '#payments',
        account_portal_url: accountPortal ? accountPortal.url : doctorPortalUrl(),
        portal_name: accountPortal ? accountPortal.name : 'Doctor portal',
        portal_button_label: accountPortal ? accountPortal.buttonLabel : 'Open doctor portal',
        portal_welcome_blurb: accountPortal ? accountPortal.blurb : '',
        doctor_portal_url: doctorPortalUrl(),
        staff_portal_url: (function () {
            try {
                return require('./portal-urls').getPortalUrls().staff;
            } catch (_) {
                return doctorPortalUrl() + '/../staff/login';
            }
        })(),
        admin_portal_url: (function () {
            try {
                return require('./portal-urls').getPortalUrls().admin;
            } catch (_) {
                return doctorPortalUrl() + '/../admin';
            }
        })(),
        judge_portal_url: (function () {
            try {
                return require('./portal-urls').getPortalUrls().judge;
            } catch (_) {
                return doctorPortalUrl() + '/../judge';
            }
        })(),
        scanner_portal_url: (function () {
            try {
                return require('./portal-urls').getPortalUrls().scanner;
            } catch (_) {
                return doctorPortalUrl() + '/../scanner';
            }
        })(),
        seminar_name: (seminar && seminar.title) || 'VGMF National Seminar',
        event_name:
            (extra && extra.event_name) ||
            (extra && extra.scan_event_title) ||
            (seminar && seminar.title) ||
            'VGMF National Seminar',
        seminar_date:
            (extra && extra.seminar_date) ||
            (seminar && seminar.event_date ? formatSeminarSchedule(seminar, seminar.days) : ''),
        seminar_start_date:
            seminar && seminar.event_date ? formatSeminarDateForTicketLine(seminar.event_date) : '',
        seminar_end_date:
            seminar && (seminar.event_end_date || seminar.event_date)
                ? formatSeminarDateForTicketLine(seminar.event_end_date || seminar.event_date)
                : '',
        seminar_venue:
            (seminar && seminar.location_text) ||
            (seminar && seminar.venue_label) ||
            (seminar && seminar.location_url) ||
        ...
        order_id_string: (extra && extra.order_id_string) || (order && order.order_id_string) || '',
        payment_gateway: (extra && extra.payment_gateway) || (order && order.payment_gateway) || '',
        payment_gateway_label:
            (extra && extra.payment_gateway_label) ||
            formatGatewayLabel((extra && extra.payment_gateway) || (order && order.payment_gateway)),
        provider_transaction_id:
            (extra && extra.provider_transaction_id) || (order && order.provider_transaction_id) || '',
        payment_date_formatted:
            (extra && extra.payment_date_formatted) ||
            (order && order.payment_date ? formatNotifyDateTime(order.payment_date) : ''),
        provider_refund_id:
            (extra && extra.provider_refund_id) ||
            (refund && refund.provider_refund_id) ||
            '',
        bank_utr: (extra && extra.bank_utr) || (refund && refund.bank_utr) || '',
        refund_gateway: (extra && extra.refund_gateway) || (refund && refund.gateway) || '',
        refund_gateway_label:
            (extra && extra.refund_gateway_label) ||
            formatGatewayLabel((extra && extra.refund_gateway) || (refund && refund.gateway)),
        refund_status: (extra && extra.refund_status) || (refund && refund.status) || '',
        refund_status_label: (extra && extra.refund_status_label) || '',
        refund_tracking_url: doctorPortalUrl() + '#refunds',
        ...
        cancellation_reason: (extra && extra.cancellation_reason) || '',
        admin_notes: (extra && extra.admin_notes) || '',
        refund_amount: (extra && extra.refund_amount) != null ? extra.refund_amount : '',
        refund_percent: (extra && extra.refund_percent) != null ? extra.refund_percent : '',
    ...
    if (ticket && ticket.ticket_id_string && !vars.ticket_id) {
        vars.ticket_id = ticket.ticket_id_string;
    }
    if (refund && refund.status && !vars.refund_status_label) {
        try {
            const refundTracking = require('./refund-tracking');
            vars.refund_status_label = refundTracking.refundStatusLabel(refund.status);
        } catch (_) {
            vars.refund_status_label = String(refund.status);
        }
    }
    if (extra && extra.otp_code) vars.otp_code = extra.otp_code;
    if (extra && typeof extra === 'object') {
        Object.keys(extra).forEach((k) => {
            if (extra[k] != null && extra[k] !== '') vars[k] = extra[k];
        });
    }
...
function fetchContext(db, { userId, seminarId, registrationId }, cb) {
    const out = { user: null, seminar: null, reg: null, order: null, refund: null, ticket: null };
    db.get(`SELECT * FROM users WHERE id = ?`, [userId], (eu, user) => {
        ...
        const loadSem = (next) => {
            ...
            db.get(`SELECT * FROM seminars WHERE id = ?`, [seminarId], (es, sem) => {
                ...
                if (!sem) return next();
                db.all(
                    `SELECT title, day_date FROM seminar_days WHERE seminar_id = ? AND IFNULL(is_active, 1) = 1 ORDER BY sort_order ASC, day_date ASC`,
                    [seminarId],
                    (ed, days) => {
                        sem.days = !ed && Array.isArray(days) ? days : [];
                        next();
                    }
                );
            });
            ...
            loadSem(() => {
                ...
                db.get(
                    ...
                    (eo, order) => {
                        ...
                        const afterOrder = () => {
                            db.get(
                                `SELECT * FROM refunds WHERE registration_id = ? ORDER BY id DESC LIMIT 1`,
                                [rid],
                                (er, refund) => {
                                    out.refund = refund;
                                    if (!order || !order.id) return cb(null, out);
                                    db.get(
                                        `SELECT ticket_id_string FROM tickets WHERE order_id = ? ORDER BY id DESC LIMIT 1`,
                                        [order.id],
                                        (et, ticket) => {
                                            out.ticket = ticket;
                                            cb(null, out);
...
const SMS_DEDUP_MINUTES = Math.max(2, parseInt(process.env.NOTIF_SMS_DEDUP_MINUTES || '15', 10) || 15);
/** OTP / auth events — SMS is for seminar status updates only, not signup or login codes. */
const SMS_EXCLUDED_EVENT_KEYS = new Set([
    'OTP_VERIFICATION',
    'OTP_SIGNUP',
    'OTP_LOGIN',
    'OTP_REGISTRATION',
    ...
    'EMAIL_VERIFICATION',
    'ACCOUNT_CREATED',
    'ACCOUNT_CREATED_STAFF',
    'ADMIN_COMPOSE',
    'DIRECT_MESSAGE',
    'INTEGRATION_TEST_SMS'
]);
...
const PRIORITY_EMAIL_EVENTS = new Set([
    'ACCOUNT_CREATED',
    'EMAIL_VERIFICATION',
    'FORGOT_PASSWORD',
...
function emailPlainFromHtml(html, subject) {
    const headlineMatch = String(html || '').match(/<h1[^>]*>([\s\S]*?)<\/h1>/i);
    const headline = headlineMatch
        ? headlineMatch[1].replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()
        : '';
    const text = String(html || '')
        ...
        .replace(/\s+/g, ' ')
        .trim();
    return [headline, text].filter(Boolean).join(' — ') || String(subject || '').trim() || 'Notification';
}

function resolveOtpEventKey(purpose, eventKey) {
    if (eventKey) return eventKey;
    const p = String(purpose || '').toLowerCase();
    if (p === 'signup') return 'OTP_SIGNUP';
    if (p === 'login') return 'OTP_LOGIN';
    if (p === 'admin_confirm') return 'OTP_ADMIN';
    if (p === 'certificate_verify') return 'OTP_CERTIFICATE';
    if (p === 'proxy_applicant' || p === 'registration' || p === 'registration_submit' || p === 'registration_field') {
        return 'OTP_REGISTRATION';
    }
    return 'OTP_VERIFICATION';
}

function formatEmailLogError(result) {
    if (!result || result.ok) return null;
    return [result.error, result.provider && 'provider=' + result.provider, result.hint]
        .filter(Boolean)
        .join(' | ');
}

function sendPriorityEmailNow(db, { eventKey, destination, subject, html, userId, seminarId, emailExtras }) {
    return new Promise((resolve) => {
        if (!destination || !isEmailConfigured()) {
            return resolve({ ok: false, skipped: true });
        }
        const text = emailPlainFromHtml(html, subject);
        const sendOpts = { text };
        if (emailExtras && emailExtras.fromDisplay) sendOpts.fromDisplay = emailExtras.fromDisplay;
        if (emailExtras && emailExtras.replyTo) sendOpts.replyTo = emailExtras.replyTo;

        integrationSettings.ensureIntegrationSettingsLoaded(db, () => {
            emailDeliveryPolicy.checkEmailSendAllowed(
                db,
                { hasAttachment: false, priority: true },
                async (eGate, gate) => {
                    if (eGate) {
                        const r = await sendEmail(destination, subject, html, sendOpts);
                        return finish(r);
                    }
                    if (!gate || !gate.allowed) {
                        return resolve({ ok: false, deferred: true, reason: (gate && gate.reason) || 'throttled' });
                    }
                    const r = await sendEmail(destination, subject, html, sendOpts);
                    finish(r);
                }
        ...
        function finish(r) {
            if (db && destination && r && !r.ok && !r.skipped) {
                try {
                    require('./email-delivery-flags').recordEmailDeliveryFailure(
                        db,
                        destination,
                        r.error,
                        r.responseCode,
...
function isWhatsAppBurstThrottled(db, { destination, eventKey }, cb) {
    if (
        eventKey === 'APPLICATION_UNDER_REVIEW' ||
        eventKey === 'WAITLIST_JOINED' ||
        eventKey === 'CERTIFICATE_AVAILABLE' ||
        eventKey === 'TICKET_ISSUED' ||
        eventKey === 'CHECK_IN_CONFIRMED'
    ) {
...
function wasSameWhatsAppBodyRecentlySent(db, destination, body) {
    ...
    const snippet = String(body || '')
        ...
        .trim()
        .slice(0, 72);
    if (!dest || snippet.length < 24) return Promise.resolve(false);
    const since = new Date(Date.now() - WA_DEDUP_MINUTES * 60000).toISOString();
    return new Promise((resolve) => {
        db.get(
            `SELECT 1 AS ok FROM notification_logs
...
function wasSmsEventRecentlySent(db, { eventKey, destination }, cb) {
    const dest = normalizeLogDestination('sms', destination);
    if (!dest || !eventKey) return cb(null, false);
    const since = new Date(Date.now() - SMS_DEDUP_MINUTES * 60000).toISOString();
    db.get(
        `SELECT 1 AS ok FROM notification_logs
         WHERE channel = 'sms' AND status = 'sent' AND event_key = ? AND destination = ? AND created_at >= ?
         LIMIT 1`,
        [eventKey, dest, since],
        (e, row) => cb(e, !!(row && row.ok))
    );
...
function isSmsPendingInQueue(db, { eventKey, destination }, cb) {
    const dest = normalizeLogDestination('sms', destination);
    if (!dest || !eventKey) return cb(null, false);
    const since = new Date(Date.now() - SMS_DEDUP_MINUTES * 60000).toISOString();
    db.get(
        `SELECT 1 AS ok FROM notification_queue
         WHERE channel = 'sms' AND template_key = ? AND destination = ?
         AND status IN ('pending', 'processing') AND scheduled_at >= ?
         LIMIT 1`,
        [eventKey, dest, since],
        (e, row) => cb(e, !!(row && row.ok))
...
function shouldSkipSmsEnqueue(db, { eventKey, destination, body }, cb) {
    wasSmsEventRecentlySent(db, { eventKey, destination }, (e1, dup) => {
        if (e1) return cb(e1, true);
        if (dup) return cb(null, true);
        isSmsPendingInQueue(db, { eventKey, destination }, (e2, queued) => cb(e2, !!queued));
    });
}

function enqueueSmsForStatusUpdate(db, { eventKey, phone, waBody, subject, html, vars, userId, seminarId, tasks, opts, cb }) {
    if (opts.skipSms || !isMsg91Configured() || !integrationSettings.isMsg91SmsEnabled()) {
        return cb(null, false);
    }
    if (SMS_EXCLUDED_EVENT_KEYS.has(String(eventKey || '').trim())) {
        return cb(null, false);
    }
    if (!phone) return cb(null, false);
    const smsBody =
        plainTextForSms(waBody) || plainTextForSms(subject) || plainTextForSms(html);
    if (!smsBody) return cb(null, false);
    shouldSkipSmsEnqueue(db, { eventKey, destination: phone, body: smsBody }, (eSkip, skip) => {
        if (eSkip) return cb(eSkip);
        if (skip) return cb(null, false);
        tasks.push({
            channel: 'sms',
            destination: phone,
            payload: { body: smsBody, text: smsBody, vars, eventKey }
        });
        cb(null, true);
    });
}
...
function logNotification(db, row, cb) {
    ...
    if (row.channel === 'sms' && dest) {
        try {
            const { normalizePhoneE164 } = require('./msg91-service');
            dest = normalizePhoneE164(dest) || dest;
        } catch (_) {}
    }
...
/**
 * Send TICKET_ISSUED email with the participant's printable e-ticket HTML attached.
 */
function sendTicketIssuedEmail(db, opts, cb) {
    opts = opts || {};
    const userId = opts.userId;
    const registrationId = opts.registrationId;
    ...
    const ticketRows =
        Array.isArray(opts.ticketRows) && opts.ticketRows.length
            ? opts.ticketRows.filter((r) => r && r.qr_code_data)
            : ticketRow && ticketRow.qr_code_data
              ? [ticketRow]
              : [];
    const extraVars = opts.vars || {};
    const immediate = opts.immediate !== false;
    if (!userId || !registrationId || !ticketId) return cb && cb(null, { skipped: true });

    const ticketHtml = require('./ticket-html');
    const ticketPdf = require('./certificate-pdf-server');
    const base = publicBaseUrl();

    const ticketUnitLabel = (r) =>
        [r.day_title, r.event_title].filter(Boolean).join(' · ') || '';

    const loadSeminarDays = (seminarId) =>
        new Promise((resolve) => {
            if (!seminarId) return resolve([]);
            db.all(
    ...
    const buildOneAttachment = (r, seminarDays) => {
        const tid = r.ticket_id_string || ticketId;
        const unit = ticketUnitLabel(r);
        const isUnitTicket = !!(r.day_date || r.sub_event_date);
        const fileBase =
            'E-Ticket-' +
            (unit ? unit.replace(/[^\w]+/g, '-').replace(/^-|-$/g, '') + '-' : '') +
            String(tid).replace(/\W/g, '');
        return ticketHtml
            .buildTicketHtmlFromRow(
                {
                    ticket_id_string: tid,
                    application_no: r.application_no,
                    seminar_title: unit ? r.seminar_title + ' — ' + unit : r.seminar_title,
                    event_date: r.day_date || r.sub_event_date || r.event_date,
                    event_end_date: isUnitTicket ? null : r.event_end_date,
                    // A seminar-wide ticket for a multi-day event lists every day (with admin-set timings).
                    event_date_fmt: r.day_date
                        ? formatSeminarDayLine({ day_date: r.day_date, start_time: r.day_start_time, end_time: r.day_end_time })
                        : isUnitTicket
                          ? ''
                          : formatSeminarSchedule(r, seminarDays) || '',
                    ticket_expires_at: r.ticket_expires_at,
                    location_url: r.location_url,
                    portal_year: r.portal_year,
                    display_name: [r.first_name, r.last_name].filter(Boolean).join(' '),
                    qr_code_data: r.qr_code_data,
                    payment_status: r.payment_status || 'success',
            ...
            .then((html) =>
                ticketPdf
                    ...
                    .catch((pdfErr) => {
                        console.warn('[ticket-issue] PDF render failed, attaching HTML:', pdfErr && pdfErr.message);
                        return { filename: fileBase + '.html', content: html, contentType: 'text/html' };
                    })
            );
    };

    const buildAttachments = () => {
        if (!ticketRows.length) return Promise.resolve(null);
        const semId = (ticketRow && ticketRow.seminar_id) || (ticketRows[0] && ticketRows[0].seminar_id);
        return loadSeminarDays(semId)
            .then((seminarDays) => Promise.all(ticketRows.map((r) => buildOneAttachment(r, seminarDays))))
            .then((list) => list.filter(Boolean))
            .catch((err) => {
                console.warn('[ticket-issue] attachment build failed:', err && err.message);
                return null;
            });
    };

    const ticketListHtml = () => {
        if (ticketRows.length < 2) return '';
        const items = ticketRows
            .map((r) => {
                const unit = ticketUnitLabel(r);
                const when = formatSeminarDateForTicketLine(r.day_date || r.sub_event_date || r.event_date);
                return (
                    '<li style="margin:4px 0;">' +
                    (unit ? '<strong>' + escapeHtml(unit) + '</strong> — ' : '') +
                    (when ? escapeHtml(when) + ' — ' : '') +
                    'Ticket <code>' +
                    escapeHtml(r.ticket_id_string || '') +
                    '</code></li>'
                );
            })
            .join('');
        return (
            '<p style="margin-top:14px;"><strong>This registration includes ' +
            ticketRows.length +
            ' e-tickets</strong> (one QR per day / session). Carry the matching ticket on each day:</p><ul style="padding-left:18px;">' +
            items +
            '</ul>'
        );
    };

    buildAttachments().then((attachments) => {
        const seminarId = ticketRow && ticketRow.seminar_id;
        loadTemplate(db, 'TICKET_ISSUED', seminarId, (te, tpl) => {
            if (te) return cb && cb(te);
            fetchContext(db, { userId, seminarId, registrationId }, (ce, ctx) => {
                if (ce) return cb && cb(ce);
                if (!ctx.user || !ctx.user.email) {
                    return cb && cb(null, { skipped: true, reason: 'no email' });
                }
                const vars = buildVarsFromRows(
                    ctx.user,
                    ctx.seminar,
                    ctx.reg,
                    ctx.order,
                    Object.assign(
                        {
                            ticket_id: ticketId,
                            qr_code_url: extraVars.qr_code_url || base + '/doctor#tab-tickets',
                            ticket_pdf_url: extraVars.ticket_pdf_url || base + '/doctor#tab-tickets',
                            payment_status: 'PAID'
                        },
                        extraVars
                    ),
                ...
                const subject =
                    tpl && tpl.email_subject
                        ? renderTemplate(tpl.email_subject, vars)
                        : 'Your E-Ticket is Ready – National Seminar 2026';
                let html =
                    tpl && tpl.enabled && tpl.email_html
                        ? renderTemplate(tpl.email_html, vars)
                        : '<p>Dear ' +
                          escapeHtml(vars.full_name || 'Doctor') +
                          ',</p><p>Your e-ticket is ready. The printable ticket is attached to this email.</p>';
                html += ticketListHtml();
                if (attachments && attachments.length) {
                    html +=
                        '<p style="font-size:0.9rem;color:#64748b;margin-top:14px;">' +
                        (attachments.length > 1
                            ? 'All ' + attachments.length + ' e-ticket files are <strong>attached</strong> to this email.'
                            : 'Your personal e-ticket file is <strong>attached</strong> to this email.') +
                        ' You can also open them anytime from the doctor portal under <em>Participant tickets</em>.</p>';
                }
                html = finalizeEmailHtml(html, subject);
                const text = emailPlainFromHtml(html, subject);
                enqueueDirectMessage(
                    ...
                    (qErr, qOut) => {
                        if (qErr) return cb && cb(qErr);
                        cb && cb(null, Object.assign({ queued: true, hasAttachment: !!(attachments && attachments.length) }, qOut || {}));
                    }
                );
            });
        });
...
function isValidUserEmail(email) {
    ...
    return !!(e && e.includes('@') && !/@pending\.local$/i.test(e));
}

function mapEmailDeliveryResult(r) {
    if (!r) return { ok: false, message: 'Unknown email result.' };
    if (r.ok) return { ok: true, sent: true, message: 'Email sent successfully.' };
    if (r.skipped) {
        const reason = r.reason || 'skipped';
        let message = 'Email skipped: ' + reason + '.';
        if (reason === 'no_valid_email') message = 'No valid email on this account.';
        if (reason === 'email_not_configured') message = 'Email provider is not configured on the server.';
    ...
    if (r.deferred) {
        return {
            ...
            deferred: true,
            reason: r.reason,
            message: 'Email deferred (rate limit): ' + (r.reason || 'try again later') + '.'
        };
    }
    const err = formatEmailLogError(r);
    return { ok: false, error: err, message: 'Email failed: ' + (err || 'send error') + '.' };
}

/**
 * Send ACCOUNT_CREATED welcome email (and optional WhatsApp) with delivery status.
 */
function sendAccountWelcomeEmail(db, opts, cb) {
    opts = opts || {};
    const userId = opts.userId;
    const sendWhatsapp = opts.sendWhatsapp !== false;
    const sendEmail = opts.skipEmail !== true;
    if (!userId) return cb && cb(null, { skipped: true, reason: 'no_user_id' });

    fetchContext(db, { userId }, (ce, ctx) => {
        if (ce) return cb && cb(ce);
        if (!ctx.user) return cb && cb(null, { skipped: true, reason: 'user_not_found' });

        const password =
            opts.temporary_password != null && String(opts.temporary_password) !== ''
                ? String(opts.temporary_password)
                : String(ctx.user.password || '');

        loadTemplate(db, 'ACCOUNT_CREATED', null, (te, tpl) => {
            if (te) return cb && cb(te);
            if (!tpl || !tpl.enabled) {
                return cb(null, {
                    skipped: true,
                    reason: 'template_disabled',
                    message: 'Welcome email template is disabled.'
                });
            }
            ...
            const html = finalizeEmailHtml(renderTemplate(tpl.email_html, vars), subject);
            const email = String(ctx.user.email || '').trim();

            const finish = (emailOut, waOut) => {
                cb(null, {
                    email: emailOut,
                    whatsapp: waOut,
            ...
            const runWhatsapp = (emailOut) => {
                if (!sendWhatsapp || !ctx.user.phone) {
                    return finish(emailOut, {
                        skipped: true,
                        reason: !ctx.user.phone ? 'no_phone' : 'disabled',
                        message: !ctx.user.phone
                            ? 'No WhatsApp number on account.'
                ...
                notify(
                    ...
                    {
                        userId,
                        vars: { temporary_password: password },
                        immediate: true,
                        skipEmail: true
                    },
                    (wErr, wOut) => {
                        if (wErr) {
                            return finish(emailOut, {
                                ok: false,
                                message: 'WhatsApp error: ' + wErr.message
                            });
                        }
                        const queued = !!(wOut && wOut.queued);
                        const skipped = !!(wOut && wOut.skipped);
                        finish(emailOut, {
                            ok: queued,
                            queued,
                            skipped,
            ...
            if (!sendEmail) {
                return runWhatsapp({ skipped: true, reason: 'disabled', message: 'Email not requested.' });
            }
            if (!isValidUserEmail(email)) {
                return runWhatsapp({
                    ...
                    message: 'No valid email on this account — add an email address first.'
                });
            }
            ...
            sendPriorityEmailNow(db, {
                ...
                runWhatsapp(mapEmailDeliveryResult(r));
            });
        });
    });
}
...
function enqueueDirectMessage(db, { channel, destination, subject, html, text, body, event_key, immediate, replyTo, fromDisplay, userId, attachments, priority }, cb) {
    ...
    const payload =
        channel === 'email'
            ? {
                  ...
                  html: finalizeEmailHtml(html || '', subject || 'Notification'),
                  text: text || ''
              }
            : { body: body || text || '', text: body || text || '' };
    if (replyTo && channel === 'email') payload.replyTo = replyTo;
    if (fromDisplay && channel === 'email') payload.fromDisplay = fromDisplay;
    if (attachments && channel === 'email' && Array.isArray(attachments) && attachments.length) {
        payload.attachments = attachments;
        payload.priority = true;
    }
    if (priority && channel === 'email') payload.priority = true;
    enqueueNotification(
        db,
        {
            channel,
...
function notify(db, eventKey, opts, cb) {
    ...
    loadTemplate(db, eventKey, seminarId, (te, tpl) => {
        ...
        fetchContext(db, { userId, seminarId, registrationId }, (ce, ctx) => {
            ...
            vars.transaction_details_html = buildTransactionDetailsHtml(vars, eventKey);
            const channel = tpl.channel || 'both';
            const subject = renderTemplate(tpl.email_subject, vars);
            const html = finalizeEmailHtml(renderTemplate(tpl.email_html, vars), subject);
            const waBody = renderTemplate(tpl.whatsapp_body, vars);
            const tasks = [];
            const sidResolved = seminarId || (ctx.seminar && ctx.seminar.id);
            const emailText = emailPlainFromHtml(html, subject);
            const emailPayload = { subject, html, text: emailText };
            if (opts.emailExtras && opts.emailExtras.replyTo) emailPayload.replyTo = opts.emailExtras.replyTo;
            if (opts.emailExtras && opts.emailExtras.fromDisplay) {
                emailPayload.fromDisplay = opts.emailExtras.fromDisplay;
            }
            ...
            const queueTasks = (list) => {
                if (!list.length) return cb && cb(null, { queued: false });
                ...
                enqueueNotification(
                    db,
                    {
                        ...
                        event_key: eventKey,
                        payload: { ...t.payload, userId, seminarId: sidResolved },
            ...
            const continueNotify = () => {
            const finishPhoneChannels = () => {
                enqueueSmsForStatusUpdate(
                    db,
                    {
                        eventKey,
                        ...
                        vars,
                        userId,
                        seminarId: sidResolved,
                        tasks,
                        opts,
                        cb: (eSms) => {
                            if (eSms) return cb && cb(eSms);
                            queueTasks(tasks);
                        }
                    }
                );
            ...
            if ((channel === 'email' || channel === 'both') && ctx.user.email && !opts.skipEmail) {
                if (PRIORITY_EMAIL_EVENTS.has(eventKey)) {
                    return sendPriorityEmailNow(db, {
                        eventKey,
                        destination: ctx.user.email,
                        subject,
                        html,
                        userId,
                        seminarId: sidResolved,
                        emailExtras: opts.emailExtras
                    }).then((r) => {
                        if (!r.ok && !r.skipped) addEmailTask();
                        continueNotify();
                    });
                }
                addEmailTask();
...
async function deliverQueueRow(db, row) {
    ...
    if (row.channel === 'email') {
        ...
        const priority =
            eventKey === 'ADMIN_COMPOSE' ||
            eventKey === 'DIRECT_MESSAGE' ||
            !!(payload && payload.priority);
        const gate = await new Promise((resolve) => {
            emailDeliveryPolicy.checkEmailSendAllowed(
                db,
                { hasAttachment: !!(payload.attachments && payload.attachments.length), priority },
                (eGate, allowed) => {
        ...
        const outboundHtml = finalizeEmailHtml(payload.html || '', payload.subject || 'Notification');
        const r = await sendEmail(row.destination, payload.subject || 'Notification', outboundHtml || payload.text || '', {
            text: payload.text || emailPlainFromHtml(outboundHtml, payload.subject),
            attachments: payload.attachments,
            replyTo: payload.replyTo,
            fromDisplay: payload.fromDisplay
        });
        ok = !!r.ok;
        lastErr = formatEmailLogError(r) || r.error || '';
        ...
        if (!ok && isConnectionLikeError(lastErr)) {
            const nextAttempt = Number(row.attempts || 0) + 1;
            if (nextAttempt < 6) {
                const deferMin = Math.min(45, 5 * nextAttempt);
                await new Promise((res) => emailDeliveryPolicy.deferQueueRow(db, row.id, deferMin, lastErr, res));
                db.run(`UPDATE notification_queue SET attempts = ? WHERE id = ?`, [nextAttempt, row.id]);
                logNotification(db, {
                    event_key: eventKey,
                    channel: row.channel,
                    ...
                    body_preview: emailPlainFromHtml(
                        payload.html || payload.text || payload.body || '',
                        payload.subject
                    ).slice(0, 500),
                    error: lastErr + ' (queued to retry in ~' + deferMin + ' min)'
                });
                return { ok: false, deferred: true, reason: 'connection_timeout' };
            }
        }
        ...
        await new Promise((resolve) => integrationSettings.ensureIntegrationSettingsLoaded(db, () => resolve()));
        const r = await sendMsg91Sms(
            row.destination,
            payload.body || payload.text || payload.sms || '',
            {
                eventKey: eventKey || payload.eventKey,
                vars: payload.vars,
    ...
    const logDest =
        row.channel === 'whatsapp' || row.channel === 'sms'
            ? normalizeLogDestination(row.channel, row.destination)
            : row.destination;
    logNotification(db, {
        event_key: eventKey,
        channel: row.channel,
        ...
        status,
        subject: payload.subject,
        body_preview: emailPlainFromHtml(payload.html || payload.text || payload.body || '', payload.subject).slice(
            0,
            500
        ),
...
async function sendOtpMessages({ email, phone, code, db, eventKey, purpose }) {
    const vars = {
        otp_code: code,
    ...
    const key = resolveOtpEventKey(purpose, eventKey);
    const tpl = db ? await loadTemplateAsync(db, key, null).catch(() => null) : null;
    const fallbackTpl = !tpl && db ? await loadTemplateAsync(db, 'OTP_VERIFICATION', null).catch(() => null) : null;
    const activeTpl = tpl || fallbackTpl;
    const results = { email: { ok: false, skipped: true }, whatsapp: { ok: false, skipped: true } };

    if (email && isEmailConfigured()) {
        const subject = activeTpl ? renderTemplate(activeTpl.email_subject, vars) : 'Your verification code';
        const html = activeTpl
            ? finalizeEmailHtml(renderTemplate(activeTpl.email_html, vars), subject)
            : finalizeEmailHtml(
                  '<p>Dear Participant,</p><p>Your verification code is:</p><p style="font-size:28px;font-weight:bold;letter-spacing:6px;">' +
                      code +
                      '</p><p>Valid for 10 minutes. Do not share this code.</p>',
                  'Your verification code'
              );
        const text = emailPlainFromHtml(html, subject);
        results.email = await sendEmail(email, subject, html, { text: text || 'Your code is ' + code });
        if (db && results.email && !results.email.ok && !results.email.skipped) {
            try {
                require('./email-delivery-flags').recordEmailDeliveryFailure(
                    db,
                    email,
                    results.email.error,
                    results.email.responseCode,
                    () => {}
                );
            } catch (_) {}
        }
    ...
    if (phone && isWhatsAppConfigured()) {
        const waBody = activeTpl
            ? renderTemplate(activeTpl.whatsapp_body, vars)
