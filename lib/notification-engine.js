...
async function getOtpWhatsAppTemplateName(db) {
    try {
        ...
        if (rt.whatsapp_otp_template_name && String(rt.whatsapp_otp_template_name).trim()) {
            return String(rt.whatsapp_otp_template_name).trim();
        }
    } catch (_) {}
    const env = process.env.WHATSAPP_OTP_TEMPLATE_NAME && String(process.env.WHATSAPP_OTP_TEMPLATE_NAME).trim();
    if (env) return env;
    if (!db) return '';
    const tpl = await loadTemplateAsync(db, 'OTP_VERIFICATION', null).catch(() => null);
    return tpl && tpl.whatsapp_template_name ? String(tpl.whatsapp_template_name).trim() : '';
}
...
function syncOtpNotificationDefaults(db, payload, cb) {
    ...
    const waTpl =
        payload && payload.whatsapp_otp_template_name
            ? String(payload.whatsapp_otp_template_name).trim()
            : '';
    const emailSub =
