...
const PRIORITY_EMAIL_EVENTS = new Set(['ACCOUNT_CREATED', 'EMAIL_VERIFICATION', 'FORGOT_PASSWORD']);
...
function sendPriorityEmailNow(db, { eventKey, destination, subject, html, userId, seminarId }) {
    return new Promise((resolve) => {
        ...
        emailDeliveryPolicy.checkEmailSendAllowed(
            ...
            async (eGate, gate) => {
                if (eGate) {
                    const r = await sendEmail(destination, subject, html, { text });
                    return finish(r);
                }
