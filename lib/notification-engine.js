...
function sendTicketIssuedEmail(db, opts, cb) {
    ...
    const base = publicBaseUrl();
    const ticketRows = (
        Array.isArray(opts.ticketRows) && opts.ticketRows.length ? opts.ticketRows : ticketRow ? [ticketRow] : []
    ).filter((tr) => tr && tr.ticket_id_string);

    const buildOneAttachment = (tr) => {
        if (!tr || !tr.qr_code_data) return Promise.resolve(null);
        const tid = tr.ticket_id_string || ticketId;
        return ticketHtml
            .buildTicketHtmlFromRow(
                {
                    ticket_id_string: tid,
                    application_no: tr.application_no,
                    seminar_title: tr.day_title
                        ? tr.seminar_title + ' — ' + tr.day_title
                        : tr.seminar_title,
                    event_date: tr.day_date || tr.event_date,
                    location_url: tr.location_url,
                    portal_year: tr.portal_year,
                    display_name: [tr.first_name, tr.last_name].filter(Boolean).join(' '),
                    qr_code_data: tr.qr_code_data,
                    payment_status: tr.payment_status || 'success',
                    is_scanned: tr.is_scanned,
                    scan_time: tr.scan_time,
                    is_valid: tr.is_valid
                },
                db
            )
            .then((html) => ({
                filename: 'E-Ticket-' + String(tid).replace(/\W/g, '') + '.html',
                content: html,
    ...
    const buildAttachments = () => {
        ...
        return Promise.all(ticketRows.map(buildOneAttachment)).then((list) => {
            const ok = list.filter(Boolean);
            return ok.length ? ok : null;
        });
    };

    const multiTicketListHtml = () => {
        if (ticketRows.length < 2) return '';
        const items = ticketRows
            .map((tr) => {
                const label =
                    String(tr.seminar_title || 'Seminar') + (tr.day_title ? ' — ' + tr.day_title : '');
                const when = tr.day_date || tr.event_date || '';
                const link = tr.view_url
                    ? ' <a href="' + escapeHtml(tr.view_url) + '">Open / download</a>'
                    : '';
                return (
                    '<li><strong>' +
                    escapeHtml(label) +
                    ...
                    ' — E-ticket ID: <code>' +
                    escapeHtml(String(tr.ticket_id_string)) +
                    '</code>.' +
                    link +
                    '</li>'
                );
        ...
        return (
            ...
            ticketRows.length +
            ' e-tickets (one per day / event). Each ticket file is attached to this email:</strong></p><ul style="margin:6px 0 0;padding-left:20px;">' +
            items +
            '</ul>'
        );
    };
        ...
        loadTemplate(db, 'TICKET_ISSUED', seminarId, (te, tpl) => {
            ...
            fetchContext(db, { userId, seminarId, registrationId }, (ce, ctx) => {
                ...
                if (attachments && attachments.length) {
                    html +=
                        '<p style="font-size:0.9rem;color:#64748b;margin-top:14px;">Your personal e-ticket file' +
                        (attachments.length > 1 ? 's are' : ' is') +
                        ' <strong>attached</strong> to this email. You can also open it anytime from the doctor portal under <em>Participant tickets</em>.</p>';
                }
                html += multiTicketListHtml();
