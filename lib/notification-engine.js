...
function buildVarsFromRows(user, seminar, reg, order, extra) {
    ...
    const vars = {
        ...
        seminar_date: seminar && seminar.event_date ? String(seminar.event_date).slice(0, 10) : '',
            ...
            if ((channel === 'whatsapp' || channel === 'both') && ctx.user.phone) {
                tasks.push({
                    ...
                    payload: {
                        ...
                        body: waBody,
                        bodyParams: [vars.first_name, vars.seminar_name, vars.ticket_id].filter(Boolean)
                    }
                });
            }
...
async function deliverQueueRow(db, row) {
    ...
    if (row.channel === 'email') {
        ...
        if (payload.templateName) {
            r = await sendWhatsAppTemplate(row.destination, payload.templateName, payload.bodyParams || []);
        } else {
...
async function sendOtpMessages({ email, phone, code, db, eventKey }) {
    ...
    if (phone && isWhatsAppConfigured()) {
        ...
        if (tpl && tpl.whatsapp_template_name) {
            results.whatsapp = await sendWhatsAppTemplate(phone, tpl.whatsapp_template_name, [code]);
        } else {
