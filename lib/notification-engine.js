...
function enqueueDirectMessage(db, { channel, destination, subject, html, text, body, event_key, immediate, replyTo }, cb) {
    ...
    enqueueNotification(
        ...
        () => {
            ...
            if (immediate) {
                drainNotificationQueue(db, 4).then(done).catch(() => done());
                return;
            }
            try {
                processQueueOnce(db);
                ...
                enqueueNotification(
                    ...
                    () => {
                        ...
                        if (left === 0) {
                            ...
                            if (opts.immediate) {
                                drainNotificationQueue(db, 4).then(finishNotify).catch(finishNotify);
                            } else {
                                processQueueOnce(db);
                                finishNotify();
                            }
