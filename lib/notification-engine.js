...
function sendTicketIssuedEmail(db, opts, cb) {
    ...
    const buildAttachments = () => {
        if (!ticketRow || !ticketRow.qr_code_data) return Promise.resolve(null);
        return ticketHtml
            .buildTicketHtmlFromRow(
                {
                    ticket_id_string: ticketRow.ticket_id_string || ticketId,
                    application_no: ticketRow.application_no,
                    seminar_title: ticketRow.day_title
                        ? ticketRow.seminar_title + ' — ' + ticketRow.day_title
                        : ticketRow.seminar_title,
                    event_date: ticketRow.day_date || ticketRow.event_date,
                    location_url: ticketRow.location_url,
                    portal_year: ticketRow.portal_year,
                    display_name: [ticketRow.first_name, ticketRow.last_name].filter(Boolean).join(' '),
                    qr_code_data: ticketRow.qr_code_data,
                    payment_status: ticketRow.payment_status || 'success',
                    is_scanned: ticketRow.is_scanned,
                    scan_time: ticketRow.scan_time,
                    is_valid: ticketRow.is_valid
                },
                db
            )
            .then((html) => [
        ...
        loadTemplate(db, 'TICKET_ISSUED', seminarId, (te, tpl) => {
            ...
            fetchContext(db, { userId, seminarId, registrationId }, (ce, ctx) => {
                ...
                if (attachments && attachments.length) {
                    html +=
                        '<p style="font-size:0.9rem;color:#64748b;margin-top:14px;">Your personal e-ticket file is <strong>attached</strong> to this email. You can also open it anytime from the doctor portal under <em>Participant tickets</em>.</p>';
                }
