...
const { DEFAULT_TEMPLATES, EVENT_KEYS, buildTransactionDetailsHtml, formatGatewayLabel } = require('./notification-defaults');
...
function emailPlainFromHtml(html, subject) {
    const text = String(html || '')
        ...
        .replace(/\s+/g, ' ')
        .trim();
    return text || String(subject || '').trim() || 'Notification';
}
...
async function sendOtpMessages({ email, phone, code, db, eventKey, purpose }) {
    ...
    if (email && isEmailConfigured()) {
        ...
        const html = activeTpl
            ? renderTemplate(activeTpl.email_html, vars)
