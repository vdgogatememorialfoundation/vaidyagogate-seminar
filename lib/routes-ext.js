                    ...
                    db.get(
                        ...
                        (e0, regRow) => {
                            ...
                            if (type === 'volunteer') {
                                if (en === 1) {
                                    db.get(
                                        `SELECT id FROM seminar_volunteers WHERE user_id = ? AND seminar_id = ? AND status = 'approved'`,
                                        [uid, sid],
                                        (eVol, volRow) => {
                                                            ...
                                                            (e3, rrow) => {
                                                                ...
                                                                db.run(
                                                                    `INSERT INTO volunteer_certificates (user_id, seminar_id, registration_id, display_name, enabled, scan_verified)
                                                                     VALUES (?, ?, ?, ?, 0, 0)
                                                                     ON CONFLICT(user_id, seminar_id) DO UPDATE SET display_name = excluded.display_name`,
                                                                    [vol.user_id, vol.seminar_id, regId, dn],
                                                                    () => {
                                                                        db.run(
                                                                            `INSERT INTO user_certificates (user_id, seminar_id, registration_id, display_name, enabled, scan_verified)
                                                                             VALUES (?, ?, ?, ?, 0, 0)
                                                                             ON CONFLICT(user_id, seminar_id) DO UPDATE SET display_name = COALESCE(excluded.display_name, user_certificates.display_name)`,
                                                                            [vol.user_id, vol.seminar_id, regId, dn],
                                                                            () => {
                                                                                db.get(
                                                                                    `SELECT id FROM tickets WHERE order_id = ? ORDER BY id DESC LIMIT 1`,
                                                                                    [orderId],
                                                                                    (eTid, trow) => {
                                                                                        if (!eTid && trow && trow.id) {
                                                                                            syncCertificateEligibilityForTicket(
                                                                                                trow.id,
                                                                                                () => {}
                                                                                            );
                                                                                        ...
                                                                                        res.json({
                                                                                            ...
                                                                                            ticketId: ticketStr,
                                                                                            message:
                                                                                                'Volunteer approved. Free registration and e-ticket created. Issue participation certificate (after venue scan) and volunteer certificate separately in Certificate Management.'
                                                                                        });
                                                                                    }
                                                                                );
                                                                            }
