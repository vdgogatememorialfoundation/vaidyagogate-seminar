...
module.exports = function registerExtendedRoutes(app, deps) {
    ...
    app.get('/api/branding/logo', (req, res) => {
        db.get(`SELECT value FROM global_settings WHERE key = 'site_logo_path'`, [], (e, row) => {
            if (e) return res.status(500).json({ error: e.message });
            res.json({ logoPath: row && row.value ? row.value : '' });
        });
    });

    app.post('/api/admin/branding/logo', upload.single('logo'), (req, res) => {
        if (!req.file) return res.status(400).json({ error: 'logo file required' });
        const rel = '/uploads/' + req.file.filename;
        db.run(
            `INSERT INTO global_settings (key, value) VALUES ('site_logo_path', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value`,
            [rel],
            (err) => {
                if (err) return res.status(500).json({ error: err.message });
                res.json({ success: true, logoPath: rel });
            }
        );
    });
    ...
    app.get('/api/admin/certificates/candidates', (req, res) => {
        ...
        const sql = `
            SELECT r.id AS registration_id, r.user_id, r.application_no, r.status AS reg_status,
                   ...
                   t.id AS ticket_id, t.is_scanned, t.scan_time, t.ticket_id_string,
    ...
    app.post('/api/admin/certificates/bulk-toggle', (req, res) => {
        ...
        db.get(
            ...
            (eTpl, tplRow) => {
                ...
                const templateMissing = !tplRow;
                let done = 0;
                let errMsg = null;
                ids.forEach((uid) => {
                    db.get(
                        `SELECT r.id AS registration_id, r.form_data, t.id AS ticket_id
                         FROM registrations r
                         LEFT JOIN orders o ON o.registration_id = r.id AND o.status = 'success'
                         LEFT JOIN tickets t ON t.order_id = o.id
            ...
            db.all(
                ...
                (e2, existing) => {
                    ...
                    const next = () => {
                        if (i >= files.length) {
                            db.run(
                                `UPDATE case_submissions SET status = 'resubmitted', updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
                                [submissionId],
                        ...
                        db.run(
                            ...
                            ['/uploads/' + f.filename, f.originalname, fid, submissionId],
                            () => {
                                i++;
                                next();
                            }
  ...
  function sendAdminReport(req, res) {
        ...
        db.all(spec.sql, [sid], (e, rows) => {
            ...
            if (format === 'xlsx' || format === 'excel') {
                const buf = exportReports.toXlsxBuffer(rows || [], spec.title);
                res.setHeader(
                    'Content-Type',
            ...
            if (format === 'pdf' || format === 'html') {
                const html = exportReports.toHtmlTable(rows || [], spec.title);
                res.setHeader('Content-Type', 'text/html; charset=utf-8');
                res.setHeader('Content-Disposition', `attachment; filename="${base}.html"`);
                return res.send(html);
            }
            const csv = exportReports.toCsv(rows || []);
