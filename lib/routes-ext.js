...
const caseMarkingDeadline = require('./case-marking-deadline');
const extModules = require('./extended-modules');
const volunteerCertFlow = require('./volunteer-cert-flow');
const volunteerTicketFlow = require('./volunteer-ticket-flow');
const adminUserLookup = require('./admin-user-lookup');
const publicParticipantList = require('./public-participant-list');
...
module.exports = function registerExtendedRoutes(app, deps) {
    ...
    app.post('/api/admin/registration-overrides', (req, res) => {
        const body = req.body || {};
        const { userId, userIdString, seminarId, enabled, note, adminUserId } = body;
        const registerUntil = body.registerUntil ?? body.register_until ?? body.registerBy ?? '';
        const sid = parseInt(seminarId, 10);
        if (!Number.isInteger(sid)) {
            return res.status(400).json({ error: 'seminarId required' });
        }
        const isEnabled = enabled !== false;
        let registerUntilStored = null;
        if (isEnabled) {
            const rawUntil = registerUntil != null ? String(registerUntil).trim() : '';
            if (!rawUntil) {
                const hint =
                    body.userIdString || body.seminarId
                        ? ' Hard-refresh admin (Ctrl+F5) if you already entered a deadline — your browser may be using an old admin script.'
                        : '';
                return res.status(400).json({
                    error:
                        'Register-by deadline is required when enabling late registration for a doctor.' + hint
                });
            }
            registerUntilStored = seminarDt.normalizeSeminarRegistrationEndForStorage(rawUntil);
            const untilMs = seminarDt.parseSeminarMs(registerUntilStored);
            if (untilMs == null || Number.isNaN(untilMs)) {
                return res.status(400).json({
                    error:
                        'Invalid register-by deadline. Use DD-MM-YYYY HH:mm (e.g. 30-05-2026 19:07) or 2026-05-30T19:07 (IST).'
                });
            }
            if (untilMs < Date.now()) {
                return res.status(400).json({ error: 'Register-by deadline must be in the future.' });
            }
        }
        resolveInternalUserId(db, userId, userIdString || userId, (e, uid) => {
            ...
            db.run(
                `INSERT INTO registration_overrides (user_id, seminar_id, enabled, register_until, note, created_by)
                 VALUES (?, ?, ?, ?, ?, ?)
                 ON CONFLICT(user_id, seminar_id) DO UPDATE SET
                    enabled = excluded.enabled,
                    register_until = excluded.register_until,
                    note = excluded.note`,
                [uid, sid, isEnabled ? 1 : 0, registerUntilStored, note || null, adminUserId || null],
                function (err) {
                    if (err) return res.status(500).json({ error: err.message });
                    res.json({ success: true, userId: uid, registerUntil: registerUntilStored });
                }
            );
        });
    });
    ...
    app.get('/api/doctor/registration-overrides/:userId', (req, res) => {
        const portalHint =
            ...
            req.params.userId;
        resolveInternalUserId(db, req.params.userId, portalHint, (eRes, uid) => {
            if (eRes) return res.status(500).json({ error: eRes.message });
            if (!uid) return res.status(400).json({ error: 'Invalid user' });
            db.all(
                `SELECT seminar_id, register_until, enabled FROM registration_overrides
                ...
                (e, rows) => {
                    ...
                    const ids = new Set();
                    const overrides = [];
                    const overrideSeminarIds = [];
                    (rows || []).forEach((r) => {
                        if (!extModules.registrationOverrideIsEnabled(r.enabled)) return;
                        const sid = Number(r.seminar_id);
                        if (sid <= 0) return;
                        const isActive = extModules.registrationOverrideIsActive(r.register_until);
                        const registerUntil =
                            r.register_until != null && r.register_until !== ''
                                ? seminarDt.normalizeSeminarDateTimeForStorage(r.register_until) ||
                        ...
                        overrideSeminarIds.push(sid);
                        if (isActive) ids.add(sid);
                    });
                    db.all(
                        ...
                        (e2, vrows) => {
                            if (e2) return res.status(500).json({ error: e2.message });
                            const volunteerSeminarIds = [];
                            (vrows || []).forEach((r) => {
                                const sid = Number(r.seminar_id);
                                if (sid > 0) {
                                    volunteerSeminarIds.push(sid);
                                    ids.add(sid);
                                }
                            });
                            const finish = (seminarRows) => {
                                res.json({
                                    resolvedUserId: uid,
                                    seminarIds: Array.from(ids),
                                    overrides,
                                    volunteerSeminarIds,
                                    seminars: seminarRows || []
                                });
                            };
                            if (!overrideSeminarIds.length) return finish([]);
                            const ph = overrideSeminarIds.map(() => '?').join(',');
                            db.all(
                                `SELECT * FROM seminars WHERE id IN (${ph}) AND IFNULL(is_active, 1) = 1`,
                                overrideSeminarIds,
                                (e3, semRows) => {
                                    if (e3) return res.status(500).json({ error: e3.message });
                                    finish(semRows || []);
        ...
        return {
            sql:
                `SELECT sv.id, sv.id AS assignment_id, sv.seminar_id, sv.user_id, sv.status, sv.notes, sv.duties,
                        ...
                        s.title AS seminar_title, s.event_date, s.registration_start, s.registration_end,
    ...
    app.get('/api/admin/volunteers/user-preview', (req, res) => {
        const sid = parseInt(req.query.seminarId, 10);
        const userId = parseInt(req.query.userId, 10);
        const userIdString = String(req.query.userIdString || req.query.userId || '').trim();
        if (!Number.isInteger(sid) || sid < 1) {
            return res.status(400).json({ error: 'seminarId required' });
        }

        function sendPreview(uid) {
            if (!uid) return res.status(404).json({ error: 'Doctor not found' });
            buildVolunteerUserPreview(db, uid, sid, (err, payload) => {
                if (err) return res.status(500).json({ error: err.message });
        ...
        if (Number.isInteger(userId) && userId > 0) {
            return sendPreview(userId);
        }

        const q = String(req.query.q || userIdString || '').trim();
        if (!q) {
            return res.status(400).json({ error: 'Enter name, phone, email, portal ID, or application number.' });
        }

        adminUserLookup.searchVolunteerDoctorsForSeminar(db, sid, q, (sErr, matches) => {
            if (sErr) return res.status(500).json({ error: sErr.message });
            const list = matches || [];
            if (!list.length) {
                return res.status(404).json({ error: 'No doctor found for “' + q + '”. Try another spelling or application number.' });
            }
            if (list.length === 1) {
                return sendPreview(list[0].id);
            }
            res.json({ success: true, multiple: true, matches: list });
        });
    });

    app.get('/api/admin/volunteers/search', (req, res) => {
        const sid = parseInt(req.query.seminarId, 10);
        ...
        adminUserLookup.searchVolunteerDoctorsForSeminar(db, sid, q, (err, matches) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true, matches: matches || [] });
        });
    });

    function buildVolunteerUserPreview(db, uid, sid, cb) {
        db.get(
            `SELECT u.id, u.user_id_string, u.first_name, u.middle_name, u.last_name, u.email, u.phone,
                    u.qualification, u.doctor_category
            ...
            (uErr, user) => {
                ...
                db.get(
                    ...
                    (rErr, reg) => {
                        ...
                        db.get(
                            `SELECT sv.* FROM seminar_volunteers sv WHERE sv.user_id = ? AND sv.seminar_id = ?`,
                            [uid, sid],
                            (vErr, vol) => {
                                if (vErr) return cb(vErr);
                                db.get(`SELECT title, price, event_date FROM seminars WHERE id = ?`, [sid], (sErr, sem) => {
                                    if (sErr) return cb(sErr);
                                    const regSummary = reg ? volunteerTicketFlow.summarizeRegistrationForm(reg) : null;
                                    cb(null, {
                                        success: true,
                                        user: {
                                            id: user.id,
                                            ...
                                            doctorCategory: user.doctor_category,
                                            city: (regSummary && regSummary.city) || '',
                                            state: (regSummary && regSummary.state) || '',
                                            pincode: (regSummary && regSummary.pin) || '',
                                            address: (regSummary && regSummary.address) || ''
                                        },
                                        seminar: sem
                                            ? {
                                                  id: sid,
                                                  title: sem.title,
                                                  price: sem.price,
                                                  eventDate: sem.event_date
                                              }
                                            : { id: sid },
                                        registration: regSummary,
                                        registrationId: reg ? reg.id : null,
                                        volunteerAssignment: vol
                                            ? {
                                                  id: vol.id,
                                                  status: vol.status,
                                                  duties: vol.duties,
                                                  notes: vol.notes,
                                                  ticketId: vol.volunteer_ticket_id_string
                                              }
                                            : null,
                                        canAutoConvert:
                                            !!(
                                                regSummary &&
                                                regSummary.registrationComplete &&
                                                !(vol && vol.volunteer_ticket_id_string)
                                            ),
                                        isVolunteerCategory: String(user.doctor_category || '').toLowerCase() === 'volunteer'
                                    });
                                });
                            }
                        );
        ...
        resolveInternalUserId(db, userId, userIdString || userId, (e, uid) => {
            ...
            volunteerTicketFlow.assignVolunteerFromAdmin(
                db,
                {
                    ...
                    duties,
                    adminUserId: adminId,
                    setVolunteerRole: setVolunteerRole !== false && setVolunteerRole !== 0,
                    tryAutoTicket: true,
                    volunteerTicketDeps
                },
                (aErr, result) => {
                    if (aErr) return res.status(500).json({ error: aErr.message });
                    if (result && result.ticketIssued) {
                        return res.json({
                            success: true,
                            ...
                            message:
                                'Volunteer assigned and converted. Registration was complete — free e-ticket (₹0) and certificates issued.'
                        });
                    }
                    res.json({
                        success: true,
                        id: result && result.volunteerId,
                        userId: uid,
                        message:
                            'Volunteer assigned and portal role updated. Free ticket (₹0) issues automatically after registration is submitted.'
                    });
                }
            );
        });
                ...
                (fErr, result) => {
                    ...
                    if (result && result.issued) {
                        return res.json({
                            success: true,
                            ticketId: result.ticketId,
                            dualCertificatesIssued: !result.toppedUp,
                            message: result.message
                        });
                    }
                    if (result && result.reason === 'already_issued' || (result && result.reason === 'order_exists')) {
                        return res.json({
                            success: true,
                            message: result.message || 'Ticket already issued (all allotted day tickets present).',
                            ticketId: result.ticketId || vol.volunteer_ticket_id_string
                        });
                    }
                    return res.status(400).json({
                        error:
        ...
        db.get(`SELECT * FROM seminar_volunteers WHERE id = ?`, [vid], (e, vol) => {
            ...
            db.all(
                `SELECT o.id FROM orders o
                 ...
                 WHERE r.user_id = ? AND r.seminar_id = ? AND o.payment_gateway = 'volunteer_waiver'`,
                ...
                (e2, orders) => {
                    ...
                    orderIds.forEach((oid) => {
                        steps.push([
                            `UPDATE user_certificates SET ticket_id = NULL WHERE ticket_id IN (SELECT id FROM tickets WHERE order_id = ?)`,
                            [oid]
                        ]);
                        steps.push([`DELETE FROM tickets WHERE order_id = ?`, [oid]]);
                        steps.push([`DELETE FROM orders WHERE id = ?`, [oid]]);
                    });
                    steps.push([
                        `DELETE FROM volunteer_certificates WHERE user_id = ? AND seminar_id = ?`,
                        [vol.user_id, vol.seminar_id]
                    ]);
                    steps.push([`DELETE FROM seminar_volunteers WHERE id = ?`, [vid]]);
                    const runNext = (i) => {
                        if (i >= steps.length) {
                            return res.json({
                                success: true,
                                message:
                                    'Volunteer assignment removed' +
                                    (orderIds.length ? ' (free volunteer ticket cancelled).' : '.')
                            });
                        }
                        db.run(steps[i][0], steps[i][1], (err) => {
                            if (err && !/no such table|does not exist|no such column/i.test(err.message)) {
    ...
    app.get('/api/doctor/volunteer-assignments/:userId', (req, res) => {
        const uid = parseInt(req.params.userId, 10);
        db.all(
            `SELECT sv.*, s.title, s.event_date, s.registration_start, s.registration_end
             FROM seminar_volunteers sv
             JOIN seminars s ON s.id = sv.seminar_id
             WHERE sv.user_id = ?
             ORDER BY s.event_date DESC`,
    ...
    app.post('/api/case/resubmit', upload.array('files', 5), (req, res) => {
        ...
        if (files.length !== replaceFileIds.length) {
            return res.status(400).json({
                error:
                    'Each uploaded file must map to one replaceFileId (' +
                    files.length +
                    ' file(s) for ' +
                    replaceFileIds.length +
                    ' slot(s)).'
            });
        }
    ...
    app.get('/api/doctor/volunteer-certificates/:userId', (req, res) => {
        ...
        resolveInternalUserId(db, req.params.userId, portalHint, (eRes, uid) => {
            ...
            let sql = `SELECT vc.*, s.title AS seminar_title, s.portal_year, s.event_date, ct.file_path AS template_path, ct.mime_type
            ...
            sql += ` ORDER BY COALESCE(s.portal_year, 0) DESC, vc.seminar_id DESC`;
            db.all(sql, params, (e, rows) => {
                if (e) return res.status(500).json({ error: e.message });
                res.json(rows || []);
            });
    ...
    app.get('/api/admin/case/submissions/:id', (req, res) => {
        ...
        db.get(
            ...
            (e, sub) => {
                if (e) return res.status(500).json({ error: e.message });
                if (!sub) return res.status(404).json({ error: 'Not found' });
                db.all(`SELECT * FROM case_files WHERE submission_id = ? ORDER BY sort_order`, [id], async (e2, files) => {
                    if (e2) return res.status(500).json({ error: e2.message });
                    db.all(
                        ...
                        async (e3, judges) => {
                            if (e3) return res.status(500).json({ error: e3.message });
                            // Enrich files with R2 signed URLs (2 hour expiry for judges)
                            const caseFileAccess = require('./case-file-access');
                            try {
                                const enrichedFiles = await caseFileAccess.enrichCaseFiles(files || [], { expiresSec: 7200 });
                                res.json({ submission: sub, files: enrichedFiles, assignedJudges: judges || [] });
                            } catch (enrichErr) {
                                console.error('[admin/case-submission] enrich error:', enrichErr.message);
    ...
    app.post('/api/admin/case/submissions/:id/assign-judges', (req, res) => {
        ...
        const body = req.body || {};
        const markingDeadlineRaw = body.markingDeadline || body.marking_deadline;
        const markingDeadlineNorm = caseMarkingDeadline.normalizeMarkingDeadlineInput(markingDeadlineRaw);
        if (!markingDeadlineNorm) {
            return res.status(400).json({
                error: 'Marking deadline is required (IST date/time until judges can submit marks).'
            });
        }
        if (caseMarkingDeadline.isMarkingDeadlinePassed(markingDeadlineNorm)) {
            return res.status(400).json({ error: 'Marking deadline must be in the future.' });
        }
        const caseJudgeReminders = require('./case-judge-marking-reminders');
                ...
                db.run(
                    ...
                    () => {
                        ...
                        if (done === unique.length) {
                            db.run(
                                `UPDATE case_submissions SET status = 'judging', marking_deadline = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
                                [markingDeadlineNorm, sid],
                                () => {
                                    portalTracking.logCaseEvent(
                                        db,
                                        sid,
                                        'judging',
                                        'Judging in progress',
                                        'Your case is with the judging panel.',
                                        () => {
                                            const caseNotify = require('./case-presentation-notify');
                                            caseNotify.notifyCaseDoctor(
                                                db,
                                                ...
                                                {
                                                    marking_deadline:
                                                        caseMarkingDeadline.formatMarkingDeadlineDisplay(
                                                            markingDeadlineNorm
                                                        )
                                                },
                                                () => {
                                                    res.json({
                                                        success: true,
                                                        judgeIds: unique,
                                                        markingDeadline: markingDeadlineNorm
                                                    });
                                                }
                                            );
                                        }
    ...
    app.post('/api/admin/case/submissions/:id/marking-deadline', (req, res) => {
        const sid = parseInt(req.params.id, 10);
        if (!Number.isInteger(sid)) return res.status(400).json({ error: 'Invalid submission id' });
        const raw = (req.body && (req.body.markingDeadline || req.body.marking_deadline)) || '';
        const norm = caseMarkingDeadline.normalizeMarkingDeadlineInput(raw);
        if (!norm) return res.status(400).json({ error: 'Valid marking deadline required' });
        if (caseMarkingDeadline.isMarkingDeadlinePassed(norm)) {
            return res.status(400).json({ error: 'Marking deadline must be in the future.' });
        }
        db.run(
            `UPDATE case_submissions SET marking_deadline = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
            [norm, sid],
            function (err) {
                if (err) return res.status(500).json({ error: err.message });
                const respondOk = () => {
                    let display = norm;
                    try {
                        display = caseMarkingDeadline.formatMarkingDeadlineDisplay(norm);
                    } catch (fmtErr) {
                        console.warn('[case] marking deadline display:', fmtErr.message);
                    }
                    res.json({
                        success: true,
                        markingDeadline: norm,
                        markingDeadlineDisplay: display
                    });
                };
                if (this.changes === 0) {
                    return db.get(`SELECT id FROM case_submissions WHERE id = ?`, [sid], (e2, row) => {
                        if (e2) return res.status(500).json({ error: e2.message });
  ...
  function sendAdminReport(req, res) {
    ...
    function resolveRegistrationForVerifyList(dbConn, seminarId, body, cb) {
        const rid = parseInt(body && body.registrationId, 10);
        if (Number.isInteger(rid) && rid > 0) {
            return dbConn.get(
                `SELECT r.id, r.seminar_id, r.application_no, r.status
                 FROM registrations r WHERE r.id = ? AND r.seminar_id = ?`,
                [rid, seminarId],
                (e, row) => cb(e, row)
            );
        }
        const appNo = String((body && body.applicationNo) || '').trim();
        if (appNo) {
            return dbConn.get(
                `SELECT r.id, r.seminar_id, r.application_no, r.status
                 FROM registrations r WHERE r.application_no = ? AND r.seminar_id = ?`,
                [appNo, seminarId],
                (e, row) => cb(e, row)
            );
        }
        const userIdString = String((body && body.userIdString) || '').trim();
        if (userIdString) {
            return dbConn.get(
                `SELECT r.id, r.seminar_id, r.application_no, r.status
                 FROM registrations r
                 JOIN users u ON u.id = r.user_id
                 WHERE u.user_id_string = ? AND r.seminar_id = ?
                 ORDER BY r.id DESC LIMIT 1`,
                [userIdString, seminarId],
                (e, row) => cb(e, row)
            );
        }
        cb(null, null);
    ...
    app.get('/api/admin/seminars/:seminarId/verify-delegate-list', (req, res) => {
        const sid = parseInt(req.params.seminarId, 10);
        const q = String((req.query && req.query.q) || '').trim();
        if (!Number.isInteger(sid) || sid < 1) {
            return res.status(400).json({ error: 'Invalid seminar id' });
        }
        db.get(`SELECT id, title, public_list_enabled FROM seminars WHERE id = ?`, [sid], (err, sem) => {
            if (err) return res.status(500).json({ error: err.message });
            ...
            db.all(publicParticipantList.VERIFY_DELEGATE_LIST_SQL, [sid], (e2, rows) => {
                if (e2) return res.status(500).json({ error: e2.message });
                const payload = publicParticipantList.buildAdminVerifyDelegatePayload(rows, sem);
                if (q) {
                    payload.listed = publicParticipantList.filterParticipantsByQuery(payload.listed, q);
                    payload.missingPaid = publicParticipantList.filterParticipantsByQuery(payload.missingPaid, q);
                }
                res.json(payload);
            });
        });
    ...
    app.post('/api/admin/seminars/:seminarId/verify-delegate-list/include', (req, res) => {
        const sid = parseInt(req.params.seminarId, 10);
        const body = req.body || {};
        if (!Number.isInteger(sid) || sid < 1) {
            return res.status(400).json({ error: 'Invalid seminar id' });
        }
        resolveRegistrationForVerifyList(db, sid, body, (eFind, reg) => {
            if (eFind) return res.status(500).json({ error: eFind.message });
            if (!reg) {
                return res.status(404).json({
                    error: 'Registration not found. Provide registrationId, applicationNo, or userIdString for this seminar.'
                });
            }
            db.all(publicParticipantList.VERIFY_DELEGATE_LIST_SQL, [sid], (eRows, rows) => {
                if (eRows) return res.status(500).json({ error: eRows.message });
                const match = publicParticipantList
                    .dedupeRegistrationRows(rows)
                    .find((r) => Number(r.registration_id || r.id) === Number(reg.id));
                if (!match) {
                    return res.status(400).json({ error: 'Registration has no successful payment for this seminar.' });
                }
                if (!publicParticipantList.isPaidOrderRow(match)) {
                    return res.status(400).json({ error: 'Registration must have a successful order for this seminar.' });
                }
                db.run(
                    `UPDATE registrations SET public_list_include = 1, public_list_exclude = 0 WHERE id = ?`,
                    [reg.id],
                    (eUp) => {
                        if (eUp) return res.status(500).json({ error: eUp.message });
                        res.json({
                            success: true,
                            message: 'Added to public verify list.',
                            participant: publicParticipantList.toPublicParticipant(
                                { ...match, public_list_include: 1, public_list_exclude: 0 },
                                'manual'
                            )
                        });
                    }
    ...
    app.post('/api/admin/seminars/:seminarId/verify-delegate-list/remove', (req, res) => {
        const sid = parseInt(req.params.seminarId, 10);
        const body = req.body || {};
        const rid = parseInt(body.registrationId, 10);
        if (!Number.isInteger(sid) || sid < 1 || !Number.isInteger(rid) || rid < 1) {
            return res.status(400).json({ error: 'seminarId and registrationId required' });
        }
        db.run(
            `UPDATE registrations SET public_list_include = 0 WHERE id = ? AND seminar_id = ?`,
            [rid, sid],
            function (eUp) {
                if (eUp) return res.status(500).json({ error: eUp.message });
                if (!this.changes) return res.status(404).json({ error: 'Registration not found' });
                res.json({ success: true, message: 'Manual include removed.' });
            }
        );
    });

    app.post('/api/admin/seminars/:seminarId/verify-delegate-list/exclude', (req, res) => {
        const sid = parseInt(req.params.seminarId, 10);
        const body = req.body || {};
        const rid = parseInt(body.registrationId, 10);
        if (!Number.isInteger(sid) || sid < 1 || !Number.isInteger(rid) || rid < 1) {
        ...
        db.run(
            `UPDATE registrations SET public_list_exclude = 1, public_list_include = 0 WHERE id = ? AND seminar_id = ?`,
            [rid, sid],
            function (eUp) {
                if (eUp) return res.status(500).json({ error: eUp.message });
                if (!this.changes) return res.status(404).json({ error: 'Registration not found' });
                res.json({ success: true, message: 'Removed from public verify list.' });
            }
        );
    });
};
