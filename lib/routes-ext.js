...
module.exports = function registerExtendedRoutes(app, deps) {
    ...
    app.get('/api/admin/volunteers', (req, res) => {
        ...
        db.all(
            `SELECT sv.*, u.user_id_string, u.first_name, u.last_name, u.email, u.phone, u.doctor_category,
                    r.id AS registration_id, r.status AS registration_status, r.application_no
             FROM seminar_volunteers sv
             JOIN users u ON u.id = sv.user_id
    ...
    app.post('/api/admin/volunteers', (req, res) => {
        const { seminarId, userId, userIdString, notes, setVolunteerRole, actingAdminId } = req.body || {};
        const sid = parseInt(seminarId, 10);
            ...
            const finishAssign = () => {
                db.run(
                    `INSERT INTO seminar_volunteers (seminar_id, user_id, status, notes) VALUES (?, ?, 'pending', ?)
                     ON CONFLICT(seminar_id, user_id) DO UPDATE SET notes = COALESCE(excluded.notes, seminar_volunteers.notes), status = CASE
                            ...
                            () => {
                                if (!volunteerTicketDeps) {
                                    return res.json({
                                        ...
                                        userId: uid,
                                        message:
                                            'Volunteer assigned. Doctor must complete seminar registration in the portal; free ticket (₹0) and messages send automatically after that.'
                                    });
                                }
                                volunteerTicketFlow.tryFulfillVolunteerAfterRegistration(
                                    ...
                                    (fErr, fRes) => {
                                        const base = {
                                            ...
                                            userId: uid,
                                            message:
                                                'Volunteer assigned. Ask them to complete seminar registration in the doctor portal. Free ticket (₹0) and notifications are sent only after registration is submitted.'
                                        };
                                        if (fRes && fRes.issued) {
                                            base.ticketId = fRes.ticketId;
                                            base.message = fRes.message;
                                            base.ticketIssued = true;
                                        }
                                        res.json(base);
                                    }
                                );
