        ...
        app.put('/api/admin/seminars/:seminarId/certificate-verify', (req, res) => {
            ...
            db.get(`SELECT event_date FROM seminars WHERE id = ?`, [sid], (e, row) => {
                ...
                db.run(
                    ...
                    (e2) => {
                        if (e2) return res.status(500).json({ error: e2.message });
                        // "Make live" -- release held scan-verified certificates and send
                        // the certificate emails that were withheld at scan time.
                        const nowLive = certVerify.certificateReleaseLive({
                            certificate_verify_enabled: enabled ? 1 : 0,
                            certificate_verify_manual: manualOverride ? 1 : 0,
                            certificate_verify_go_live_at: goLiveAt,
                        ...
                        if (nowLive) {
                            try {
                                certVerify.releaseScheduledCertificatesForAll(
                                    ...
                                    { promoteRegistrationToCertificateIssued },
                                    (relErr, relOut) => {
                                    if (relErr) console.warn('[cert-release] after make-live:', relErr.message);
                                    else if (relOut && relOut.released)
                                        console.log('[cert-release] released after make-live:', relOut.released);
                                    }
                                );
                            } catch (_) {}
                        }
