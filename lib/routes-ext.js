        ...
        resolveRegistrationForVerifyList(db, sid, body, (eFind, reg) => {
            ...
            db.all(publicParticipantList.VERIFY_DELEGATE_LIST_SQL, [sid], (eRows, rows) => {
                ...
                if (!publicParticipantList.isPaidOrderRow(match)) {
                    return res.status(400).json({ error: 'Only paid registrations can be added to the verify list.' });
                }
                if (publicParticipantList.isVolunteerOrderRow(match)) {
                    return res.status(400).json({
                        error: 'Volunteer (₹0 waiver) registrations are not shown on the public verify list.'
                    });
                }
                db.run(
