...
module.exports = function registerExtendedRoutes(app, deps) {
    ...
    app.get('/api/admin/volunteers/user-preview', (req, res) => {
        ...
        if (!userIdString) {
            return res.status(400).json({ error: 'Doctor portal User ID required' });
        }
        resolveInternalUserId(db, null, userIdString, (e, uid) => {
            if (e) return res.status(500).json({ error: e.message });
            if (!uid) {
                return res.status(404).json({ error: doctorNotFoundMessage(userIdString) });
            }
            db.get(
                                    ...
                                    db.get(`SELECT title, price, event_date FROM seminars WHERE id = ?`, [sid], (sErr, sem) => {
                                        ...
                                        res.json({
                                            ...
                                            volunteerAssignment: vol
                                                ...
                                                : null,
                                            canAutoConvert:
                                                !!(regSummary && regSummary.registrationComplete && !(vol && vol.volunteer_ticket_id_string)),
