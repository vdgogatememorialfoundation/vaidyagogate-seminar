...
const REG_READY_STATUSES = new Set([
    'submitted',
    'pending_approval',
...
function isRegistrationCompleteForVolunteer(regRow) {
    ...
    if (!REG_READY_STATUSES.has(st)) return false;
    let fd = regRow.form_data;
    if (typeof fd === 'string') {
        try {
            fd = JSON.parse(fd);
        } catch (_) {
                        ...
                        insertParticipantTicket(
                            ...
                            (tErr, _etk, qrData) => {
                                ...
                                db.run(
                                    ...
                                    () => {
                                        db.run(
                                            `UPDATE registrations SET status = 'completed' WHERE id = ? AND status NOT IN ('rejected','cancelled')`,
                                            [reg.id],
                                            () => {
                                                db.run(
...
function applyVolunteerDoctorPortalRole(db, userId, cb) {
    ...
    db.run(
        `UPDATE users SET doctor_category = 'volunteer', doctor_modules = ? WHERE id = ? AND lower(trim(role)) = 'doctor'`,
        [modulesJson, userId],
        cb
    );
}
