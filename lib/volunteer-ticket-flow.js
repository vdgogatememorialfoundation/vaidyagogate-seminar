...
function findVolunteerTicketOrder(db, registrationId, cb) {
    db.get(
        `SELECT o.id, o.order_id_string FROM orders o
...
/**
 * An already-issued volunteer order must hold one ticket per allotted seminar day.
 * Issues the missing day tickets (older orders were issued with a single ticket).
 */
function topUpVolunteerDayTickets(db, deps, order, reg, uid, cb) {
    const { generateId, insertParticipantTicket, notifyTicketIssued } = deps;
    if (!order || !insertParticipantTicket) return cb(null, { added: 0 });
    seminarDays.getDayIdsForTicketIssue(db, reg.id, (dErr, dayIds) => {
        if (dErr) return cb(dErr);
        if (!dayIds || dayIds.length < 2) return cb(null, { added: 0 });
        db.all(`SELECT id, day_id, ticket_id_string FROM tickets WHERE order_id = ? ORDER BY id ASC`, [order.id], (tErr, rows) => {
            if (tErr) return cb(tErr);
            const have = new Set((rows || []).map((t) => Number(t.day_id)).filter((n) => Number.isInteger(n) && n > 0));
            let missing = dayIds.filter((d) => !have.has(Number(d)));
            if (!missing.length) return cb(null, { added: 0 });
            const runNext = () => {
                if (!missing.length) {
                    const first = (rows && rows[0] && rows[0].ticket_id_string) || null;
                    relabelVolunteerTickets(db, generateId, order.id, first, () => {
                        if (notifyTicketIssued && first) {
                            notifyTicketIssued(uid, reg.id, first, { email: true, whatsapp: false, immediate: false, source: 'volunteer' });
                        }
                        cb(null, { added: dayIds.length - have.size });
                    });
                    return;
                }
                const dayId = missing.shift();
                // A legacy single ticket without a day is assigned to the first missing day.
                const legacy = (rows || []).find((t) => t.day_id == null && !t.__used);
                if (legacy) {
                    legacy.__used = true;
                    return db.run(`UPDATE tickets SET day_id = ? WHERE id = ?`, [dayId, legacy.id], () => runNext());
                }
 ...
 */
function relabelVolunteerTickets(db, generateId, orderId, firstTicketStr, cb) {
    db.all(`SELECT id, qr_code_data, ticket_id_string FROM tickets WHERE order_id = ? ORDER BY id ASC`, [orderId], (err, rows) => {
        if (err || !rows || !rows.length) return cb();
        let left = rows.length;
        rows.forEach((t, i) => {
            const cur = String(t.ticket_id_string || '');
            const tid = i === 0 ? firstTicketStr || cur : cur.startsWith('VOL_') ? cur : 'VOL_' + generateId();
            let qrJson = t.qr_code_data;
            try {
                const qr = JSON.parse(t.qr_code_data);
                qr.ticketId = tid;
                qr.volunteer = true;
                qr.dualCertificates = true;
                qrJson = JSON.stringify(qr);
            } catch (_) {}
            db.run(`UPDATE tickets SET ticket_id_string = ?, qr_code_data = ? WHERE id = ?`, [tid, qrJson, t.id], () => {
                if (--left === 0) cb();
            });
        });
    });
...
function fulfillVolunteerAfterRegistration(db, deps, params, cb) {
    ...
    loadVolunteerAssignment(db, uid, sid, (eVol, vol) => {
        ...
        const marked = !!(vol.volunteer_ticket_id_string && String(vol.volunteer_ticket_id_string).trim());
        ...
        loadRegistration(db, uid, sid, regIdParam, (eReg, reg) => {
            ...
            if (!isRegistrationCompleteForVolunteer(reg)) {
                if (marked) {
                    return cb(null, {
                        ...
                        message:
                            'Marked as issued but no completed registration exists for this doctor any more — remove the assignment and re-add, or fill the application again.'
                    });
                }
                return cb(null, {
                    skipped: true,
            ...
            findVolunteerTicketOrder(db, reg.id, (eOrd, order) => {
                if (eOrd) return cb(eOrd);
                if (order) {
                    return topUpVolunteerDayTickets(db, deps, order, reg, uid, (tuErr, out) => {
                        if (tuErr) return cb(tuErr);
                        if (out && out.added > 0) {
                            return cb(null, {
                                issued: true,
                                toppedUp: true,
                                ...
                                message: `Issued ${out.added} missing day ticket(s) for this volunteer.`
                            });
                        }
                        cb(null, { skipped: true, reason: marked ? 'already_issued' : 'order_exists', ticketId: vol.volunteer_ticket_id_string });
                    });
                }
                // Marked issued but the ticket/order no longer exists (registration was deleted and
                // re-made): issue afresh.
                ...
                db.run(
                    ...
                    function (oErr) {
                        ...
                        const issueAll = insertParticipantTicketsForRegistration || insertParticipantTicket;
                        issueAll(
                            orderId,
                            uid,
                            orderStr,
                            ...
                            (tErr) => {
                                if (tErr) return cb(tErr);
                                // One VOL_ id per ticket (multi-day seminars issue one ticket per day);
                                // the first one is recorded on the volunteer assignment.
                                relabelVolunteerTickets(db, generateId, orderId, ticketStr, () => {
                                        ...
                                        db.run(
                                            ...
                                            (uErr) => {
                                                ...
                                                db.get(
                                                    `SELECT id FROM tickets WHERE order_id = ? ORDER BY id ASC LIMIT 1`,
                                                    [orderId],
                                                    (eTid, trow) => {
                                                        const ticketDbId =
