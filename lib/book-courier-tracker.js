...
const { trackShiprocket, trackNimbuspost } = require('./logistics-aggregators');
...
async function trackIndianPost(trackingNo) {
    ...
    return resultFrom(
        ...
        'Dispatched — India Post scans appear here when available (refresh)',
        false,
        [],
        'indian_post_pending'
    );
...
async function fetchLiveCourierTracking(provider, trackingNo, options) {
    ...
    // Prefer explicit provider adapter first so Delhivery AWB does not get swallowed
    // by aggregator "ok but empty" responses.
    pushDirect();
    ...
    if (integration === 'shiprocket') {
        ...
        // auto / direct: try configured aggregators after provider-specific adapter
        tryOrder.push(() => trackShiprocket(t, logistics));
        tryOrder.push(() => trackNimbuspost(t, logistics));
    }
    ...
    for (const fn of tryOrder) {
        try {
            const r = await fn();
            const hasEvents = !!(r && Array.isArray(r.events) && r.events.length);
            const meaningfulLabel =
                !!(r && r.statusLabel && !/update|unavailable|in transit/i.test(String(r.statusLabel).toLowerCase()));
            if (r && r.ok && (hasEvents || r.delivered || meaningfulLabel)) {
                r.externalLink = ext;
                r.externalUrl = ext.portalOnly ? null : ext.url;
                return r;
            }
        } catch (_) {
            /* next adapter */
