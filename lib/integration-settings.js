...
function getEmailApiProvider() {
    ...
    const p = String(rt.email_api_provider || process.env.EMAIL_API_PROVIDER || '')
        .trim()
        .toLowerCase();
...
function getEmailConfigStatus() {
    const rt = getRuntimeIntegrations();
    const httpProvider = getEmailApiProvider();
    const httpKey =
        rt.email_api_key ||
    ...
    const fallbackProvider = getEmailApiFallbackProvider();
    const fallbackKey =
    ...
    if (httpProvider && httpKey && from) {
        const smtpCfg = getMailConfig();
        return {
            configured: true,
            missing: [],
            via: 'http',
            provider: httpProvider,
            fallbackProvider: fallbackProvider && fallbackKey ? fallbackProvider : null,
            fallbackConfigured: !!(fallbackProvider && fallbackKey),
