...
function getEmailApiProvider() {
    ...
    const p = String(rt.email_api_provider || process.env.EMAIL_API_PROVIDER || '')
        .trim()
        .toLowerCase();
    return ['brevo', 'resend', 'zeptomail'].includes(p) ? p : '';
}
...
function getEmailConfigStatus() {
    ...
    const httpKey = rt.email_api_key || process.env.BREVO_API_KEY || process.env.RESEND_API_KEY || process.env.ZEPTOMAIL_TOKEN;
    const from =
    ...
    if (httpProvider && httpKey && from) {
        return { configured: true, missing: [], via: 'http', provider: httpProvider };
    }
