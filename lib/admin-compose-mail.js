...
function resolveRecipients(db, opts, cb) {
    ...
    if (audience === 'seminar_paid' || audience === 'seminar_all' || audience === 'approved_payment_due' || audience === 'document_reupload') {
        ...
        if (audience === 'approved_payment_due') {
            sql = sql.replace("r.status NOT IN ('rejected', 'cancelled')", "r.status = 'approved_pending_payment'");
        }
        if (audience === 'document_reupload') {
            sql = sql.replace("r.status NOT IN ('rejected', 'cancelled')", "r.status = 'revision_required'");
        }
...
/**
 * Send one email immediately (legacy direct SMTP).
 */
async function sendSingleMail({ to, name, subject, body, replyTo, fromDisplay }) {
    ...
    const text = b;
    return sendEmail(em, sub, html, { text, replyTo: replyTo || undefined, fromDisplay: fromDisplay || undefined });
}

function queueSingleMail(db, { to, name, subject, body, replyTo, fromDisplay }, cb) {
    const em = normalizeEmail(to);
    if (!EMAIL_RE.test(em)) return cb(new Error('Invalid email address'));
...
/**
 * Queue first for a fast admin response, then try delivery in the background.
 */
async function sendSingleMailReliable(db, opts) {
    const em = normalizeEmail(opts.to);
    if (!EMAIL_RE.test(em)) return { ok: false, error: 'Invalid email address' };
    if (!isEmailConfigured()) return { ok: false, error: 'Email (SMTP) is not configured in integrations' };
    const sub = String(subjectOr(opts, 'subject'));
    const b = String(subjectOr(opts, 'body')).trim();
    if (!sub || !b) return { ok: false, error: 'Subject and message are required' };

    if (!db) {
        return sendSingleMail(opts);
    }

    await new Promise((resolve, reject) => {
        queueSingleMail(db, opts, (err) => (err ? reject(err) : resolve()));
    });

    setImmediate(() => {
        notifEngine.drainNotificationQueue(db, 6).catch((e) => {
            console.warn('[admin-compose-mail] background drain', e.message);
        });
    });
    ...
    return {
        ...
        queued: true,
        message:
            'Email queued for delivery. It usually sends within a minute. Check Admin → Notifications → Delivery logs if needed.'
    };
}

function subjectOr(opts, key) {
    return String((opts && opts[key]) || '').trim();
}
 ...
 */
function sendBulkMail(db, { recipients, subject, body, useQueue = true, eventKey = 'ADMIN_COMPOSE', fromDisplay }, cb) {
    ...
    list.forEach((r) => {
        ...
        if (useQueue) {
            ...
            sendEmail(r.email, sub, html, { text, fromDisplay: fromDisplay || undefined })
