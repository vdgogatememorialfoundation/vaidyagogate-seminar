...
function computeSeminarAnalytics(rows, orders) {
    ...
    // One row per registration (multi-day tickets produce several joined rows): scanned if any ticket scanned.
    const regMap = new Map();
    (rows || []).forEach((r) => {
        const prev = regMap.get(r.id);
        if (!prev) regMap.set(r.id, { ...r, is_scanned: Number(r.is_scanned) === 1 || r.is_scanned === true ? 1 : 0 });
        else if (Number(r.is_scanned) === 1 || r.is_scanned === true) prev.is_scanned = 1;
    });

    Array.from(regMap.values()).forEach((r) => {
        ...
        if (pendingOrder && !successOrder && ['approved_pending_payment', 'submitted', 'waitlisted', 'pending_approval'].includes(st)) {
            paymentPending++;
        }

        if (['submitted', 'waitlisted', 'pending_approval', 'revision_required'].includes(st)) {
            verificationPending++;
    ...
    const paidRegs = new Set((orders || []).filter((o) => o.status === 'success').map((o) => o.registration_id));
    const pendingSeen = new Set();
    (orders || []).forEach((o) => {
        if (o.status === 'success') {
            totalRevenue += (Number(o.amount) || 0) - (Number(o.refunded_amount) || 0);
        } else if (o.status === 'pending' && !paidRegs.has(o.registration_id) && !pendingSeen.has(o.registration_id)) {
            pendingSeen.add(o.registration_id);
            pendingRevenue += Number(o.amount) || 0;
        }
    });
...
function loadSeminarAnalytics(db, seminarId, cb) {
    ...
    db.all(
        ...
        (e, rows) => {
            if (e) return cb(e);
            db.all(
                `SELECT o.registration_id, o.status, o.amount, o.refunded_amount
                 FROM orders o
