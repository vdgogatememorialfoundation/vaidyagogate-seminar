...
/** Ticket expires at 00:00 IST on the calendar day after seminar event_date (valid through event day). */
function isTicketExpiredForSeminar(eventDateStr, nowMs) {
    const eventYmd = normalizeCheckinDateYmd(eventDateStr);
    if (!eventYmd) return false;
    const todayYmd = localDateYmd(nowMs != null ? new Date(nowMs) : new Date());
    return dayDiffYmd(eventYmd, todayYmd) < 0;
}

/** First calendar date (IST) when ticket is no longer valid for entry (day after event_date). */
function ticketExpiryStartsYmd(eventDateStr) {
    const eventYmd = normalizeCheckinDateYmd(eventDateStr);
    if (!eventYmd) return '';
    const p = eventYmd.match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if (!p) return '';
...
/**
 * Absolute ms when a QR ticket stops being valid.
 * Explicit `ticket_expires_at` (admin-set IST date+time) wins; otherwise 00:00 IST the day after
 * `event_end_date` (or `event_date` / per-day date) — the historical behaviour.
 * Returns null when no date is known.
 */
function ticketExpiryMs(row) {
    if (!row) return null;
    const seminarDt = require('./seminar-datetime');
    if (row.ticket_expires_at != null && String(row.ticket_expires_at).trim() !== '') {
        const ms = seminarDt.parseSeminarMs(row.ticket_expires_at);
        if (ms != null) return ms;
    }
    const nextYmd = ticketExpiryStartsYmd(row.event_end_date || row.event_date);
    if (!nextYmd) return null;
    return seminarDt.parseSeminarMs(nextYmd + 'T00:00:00');
}

/** True when the ticket QR is past its expiry (see ticketExpiryMs). */
function isTicketExpired(row, nowMs) {
    const ms = ticketExpiryMs(row);
    if (ms == null) return false;
    return (nowMs != null ? nowMs : Date.now()) >= ms;
}

/** Human label for the ticket expiry moment, e.g. "5 Apr 2026, 12:00 am IST". */
function formatTicketExpiry(row) {
    const ms = ticketExpiryMs(row);
    if (ms == null) return '';
    const seminarDt = require('./seminar-datetime');
    return seminarDt.formatSeminarDateTime(new Date(ms).toISOString()) + ' IST';
}
