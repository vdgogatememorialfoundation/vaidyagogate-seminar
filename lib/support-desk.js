...
function computeVisibleAtForStaffReply(ticket, cfg) {
    const c = cfg || getConfig();
...
function departmentIdForCategory(db, category, cb) {
    ...
    const slug =
        cat === 'registration' || cat === 'payment' || cat === 'technical' || cat === 'case' ? cat : 'general';
...
function pickAgentForDepartment(db, departmentId, cb) {
    db.all(
        `SELECT u.id, u.first_name, u.last_name, p.max_open_tickets, p.is_available
         FROM users u
...
function autoAssignTicket(db, ticketRow, cb) {
    ...
    departmentIdForCategory(db, ticketRow.category, (eDept, deptId) => {
        ...
        pickAgentForDepartment(db, deptId, (ePick, agentId) => {
