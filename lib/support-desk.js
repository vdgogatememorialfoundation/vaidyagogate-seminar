...
const DEFAULTS = {
    ...
    websiteLiveChatEnabled: true,
    doctorPortalLiveChatEnabled: true,
...
function normalizePortalKey(portal) {
    const p = String(portal || 'website').toLowerCase();
    return p === 'doctor' || p === 'doctor_portal' ? 'doctor' : 'website';
}

function isPortalLiveChatEnabled(cfg, portal) {
    const c = cfg || getConfig();
    const key = normalizePortalKey(portal);
    const legacy = c.liveChatEnabled !== false;
    if (key === 'doctor') {
        if (c.doctorPortalLiveChatEnabled != null) return c.doctorPortalLiveChatEnabled !== false;
        return legacy;
    }
    if (c.websiteLiveChatEnabled != null) return c.websiteLiveChatEnabled !== false;
    return legacy;
}
...
function mergeConfig(raw) {
    ...
    const legacyLive = o.liveChatEnabled !== false;
    const websiteLiveChatEnabled =
        o.websiteLiveChatEnabled != null ? o.websiteLiveChatEnabled !== false : legacyLive;
    const doctorPortalLiveChatEnabled =
        o.doctorPortalLiveChatEnabled != null ? o.doctorPortalLiveChatEnabled !== false : legacyLive;
    return {
        holdEarlyStaffReplies: o.holdEarlyStaffReplies !== false,
        autoAssignEnabled: o.autoAssignEnabled !== false,
        websiteLiveChatEnabled,
        doctorPortalLiveChatEnabled,
        liveChatEnabled: websiteLiveChatEnabled,
