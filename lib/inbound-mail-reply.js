...
function handleInboundMail(db, req, res) {
    ...
    if (!ref) {
        return res.status(400).json({
            error: 'Could not find thread reference. Include [VGMF-ADM-id] or [VGMF-CASE-id-judgeId] in the email.',
            provider: norm.provider
        });
    }
    if (ref.type === 'ticket') {
        return res.status(400).json({
            error: 'Email replies to support tickets are disabled. Sign in to the doctor portal to reply.',
            provider: norm.provider
        });
    }
