...
function extractSenderName(from) {
    ...
    return raw || 'Website visitor';
}

function isToParserInbox(norm) {
    const parser = messageReplyAddress.parserInboxAddress();
    if (!parser) return false;
    const want = parser.toLowerCase();
    return (norm.toList || []).some((addr) => String(addr || '').toLowerCase().includes(want));
}
...
function ingestContactReply(db, ref, senderEmail, message, cb) {
    const inquiryId = parseInt(ref.inquiryId, 10);
    if (!Number.isInteger(inquiryId) || inquiryId < 1) return cb(new Error('Invalid contact inquiry id'));
    db.get(`SELECT * FROM contact_inquiries WHERE id = ?`, [inquiryId], (err, row) => {
        if (err) return cb(err);
        if (!row) return cb(new Error('Contact inquiry not found'));
        const ownerEmail = String(row.email || '').trim().toLowerCase();
        if (ownerEmail && senderEmail !== ownerEmail) {
            return cb(new Error('Sender email does not match the contact inquiry on file'));
        }
        const noteLine =
            '[Visitor replied by email ' +
            new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }) +
            ']\n' +
            message;
        const mergedNotes = row.admin_notes ? row.admin_notes + '\n\n' + noteLine : noteLine;
        db.run(
            `UPDATE contact_inquiries SET admin_notes = ?, status = 'follow_up', updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
            [mergedNotes, inquiryId],
            (uErr) => {
                ...
                threadReplyNotify.notifyStaffInbox(
                    db,
                    {
                        threadLabel: 'Website contact — ' + (row.subject || row.name || row.email),
                        messagePreview: message,
                        dashboardUrl: threadReplyNotify.dashboardUrl('admin_contact_inquiries'),
                        intro: 'A website contact visitor replied by email.'
                    },
                    () => cb(null, { type: 'contact', inquiryId, via: 'email' })
                );
            }
        );
    });
...
function ingestNewContactInquiry(db, norm, senderEmail, message, cb) {
    const { ensureContactInquiriesSchema } = require('./contact-inquiries-schema');
    const name = extractSenderName(norm.from) || senderEmail.split('@')[0] || 'Website visitor';
    const subject = String(norm.subject || '').trim() || 'Email enquiry';
    const msg = String(message || '').trim();
    if (!msg) return cb(new Error('Empty message body'));
    ensureContactInquiriesSchema(db, () => {}, () => {
        db.run(
            `INSERT INTO contact_inquiries (name, email, phone, subject, message, status, admin_notes, created_at, updated_at)
            ...
            [
                ...
                subject.slice(0, 300),
                msg,
                '[Created from inbound email via Mailparser]'
            ],
            function (insErr) {
                ...
                threadReplyNotify.notifyStaffInbox(
                    db,
                    {
                        threadLabel: 'Website contact — ' + subject,
                        messagePreview: msg,
                        dashboardUrl: threadReplyNotify.dashboardUrl('admin_contact_inquiries'),
                        intro: 'A new message arrived at your Mailparser inbox and was added to Website contact.'
                    },
                    () => cb(null, { type: 'contact', inquiryId: this.lastID, created: true, via: 'email' })
                );
            }
        );
    });
...
function handleInboundMail(db, req, res) {
    ...
    if (!ref && isToParserInbox(norm)) {
        ref = { type: 'contact_new' };
    }
    if (!ref) {
        return res.status(400).json({
            error:
                'Could not find thread reference. Include [VGMF-ADM-id], [VGMF-CTC-id], [VGMF-CASE-id-judgeId], or [VGMF-TKT-…] in the email.',
    ...
    if (ref.type === 'contact') return ingestContactReply(db, ref, senderEmail, message, done);
    if (ref.type === 'contact_new') return ingestNewContactInquiry(db, norm, senderEmail, message, done);
