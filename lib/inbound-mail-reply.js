/**
 * Ingest inbound email into portal threads (support tickets, case judge, admin compose).
 * Sources: built-in Zoho IMAP poller (free), or optional webhook POST /api/webhooks/inbound-email
 */
...
function isClientError(err) {
    ...
    return (
        ...
        msg.includes('could not find thread reference')
    );
}

function isIgnorableInboundError(err) {
...
function processInboundEmail(db, norm, cb) {
    const ref = resolveThreadRef(norm);
    if (!ref) {
        return cb(
            new Error(
                'Could not find thread reference. Include [VGMF-TKT-…], [VGMF-CASE-…], or [VGMF-ADM-…] in the email.'
            )
        );
    }
    ...
    if (ref.type === 'ticket') return ingestTicketReply(db, ref, senderEmail, message, cb);
    if (ref.type === 'case') return ingestCaseReply(db, ref, senderEmail, message, cb);
    if (ref.type === 'admin') return ingestAdminReply(db, ref, senderEmail, message, cb);
...
function handleInboundMail(db, req, res) {
    ...
    processInboundEmail(db, norm, (err, result) => {
        if (err) {
            console.warn('[inbound-mail]', err.message);
            const code = isClientError(err) ? 400 : 500;
            return res.status(code).json({ error: err.message, provider: norm.provider });
        }
