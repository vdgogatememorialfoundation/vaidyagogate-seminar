/**
 * Public website verify-delegate list: paid participants + volunteers (*name*).
 * Admins can force-include registrations that are missing from the auto list.
 */
...
function isConfirmedForPublicList(row) {
...
function wouldAutoListOnPublicSite(row) {
    ...
    return isConfirmedForPublicList(row);
}
...
function toPublicParticipant(row, source) {
    ...
    const isVolunteer = isVolunteerOrderRow(row);
    const plainName = displayNameFromRow(row);
    return {
        registrationId: row.registration_id || row.id,
        applicationNo: row.application_no,
        name: plainName,
        displayName: isVolunteer ? formatVolunteerDisplayName(plainName) : plainName,
        city: loc.city,
        state: loc.state,
        phone: loc.phone,
        status: row.status,
...
function buildAdminVerifyDelegatePayload(rows, seminar) {
    ...
    unique.forEach((row) => {
        ...
        if (onList) {
            ...
            listed.push(entry);
            if (isVolunteer) volunteersListed += 1;
            else paidListed += 1;
            return;
        }
