...
function courierExternalLink(provider, trackingNo) {
    ...
    if (p === 'indian_post' || p === 'speed_post') {
        return {
            url: 'https://www.indiapost.gov.in/track-result/article-tracking/' + t.toLowerCase().replace(/[^a-z0-9]/g, ''),
            manualAwb: t,
            portalOnly: false,
            note: 'India Post — if the link does not open, use AWB ' + t + ' at indiapost.gov.in/track'
        };
    }
    return {
...
function courierTrackingUrl(provider, trackingNo) {
    ...
    if (p === 'indian_post' || p === 'speed_post') {
        return 'https://www.indiapost.gov.in/track-result/article-tracking/' + t.toLowerCase().replace(/[^a-z0-9]/g, '');
    }
    if (p === 'dtdc') {
