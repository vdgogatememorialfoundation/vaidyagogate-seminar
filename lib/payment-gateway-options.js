...
function expandGatewayRow(row) {
    ...
    if (name === 'razorpay') {
        ...
        const test = c.test || {};
        const live = c.live || {};
        const testKeys = hasRazorpayKeys(test);
        const testOn = test.enabled !== false && testKeys;
        if (testKeys && testOn) {
            options.push({
                id: 'razorpay:test',
                gateway: 'razorpay',
        ...
        const liveKeys = hasRazorpayKeys(live);
        const liveOn =
            live.enabled !== false &&
            (String(live.key_id || '').startsWith('rzp_live_') || liveKeys);
        if (liveKeys && liveOn) {
            options.push({
                id: 'razorpay:live',
                gateway: 'razorpay',
...
function mergeRazorpayConfig(existing, incoming) {
    ...
    if (hasRazorpayKeys(inc.live) && inc.live.enabled !== false) inc.live.enabled = true;
    if (hasRazorpayKeys(inc.test) && inc.test.enabled !== false) inc.test.enabled = true;
    return inc;
}
...
function normalizeGatewaySave(name, config, isActive) {
    ...
    if (n === 'razorpay' && hasRazorpayKeys(c.test || {}) && c.test.enabled !== false) {
        if (!c.test) c.test = {};
        c.test.enabled = true;
    }
