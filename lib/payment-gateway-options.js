...
const LIVE_CHECKOUT_PROVIDERS = new Set(['razorpay', 'cashfree', 'juspay']);
...
const MANUAL_PROVIDER_NAMES = new Set(['payu', 'easebuzz', 'paytm', 'phonepe', 'cashfree', 'juspay']);
...
function pickDefaultLiveGateway(rows) {
    ...
    const hasJus = expanded.some((o) => o.gateway === 'juspay' && o.mode === 'live');
    if (hasJus) return 'juspay';
    return null;
}
