...
function ticketReplyTo(ticketRow) {
    const canonical = ticketRow.ticket_id || ticketRow.tracking_id || String(ticketRow.id || '');
    return messageReplyAddress.buildTicketReplyAddress(canonical);
}
    ...
    notifEngine.notify(
        ...
        eventKey,
        {
            ...
            vars: {
                ...
                ticket_message: msg + messageReplyAddress.replyFooterNote(replyTo),
...
function notifyStaffSupportReply(db, ticketRow, message, cb) {
    ...
    designatedNotify.loadConfig(db, (eCfg, cfg) => {
        ...
        const replyTo = ticketReplyTo(ticketRow);
        const bodyWithFooter = body + messageReplyAddress.replyFooterNote(replyTo);
