...
function notifyDoctorSupportEvent(db, ticketRow, eventKey, extra, cb) {
    ...
    const portalUrl = notifEngine.publicBaseUrl() + '/doctor.html#tab-support';
    const msg = (extra && extra.message) || '';
    const emailExtras = { replyTo: careReplyToEmail() };
    ...
    notifEngine.notify(
        ...
        eventKey,
        {
            ...
            vars: {
                ...
                ticket_message: msg + messageReplyAddress.supportTicketPortalFooter(portalUrl),
...
function notifyStaffSupportReply(db, ticketRow, message, cb) {
    ...
    designatedNotify.loadConfig(db, (eCfg, cfg) => {
        ...
        const portalUrl = notifEngine.publicBaseUrl() + '/support.html';
        const bodyWithFooter =
            body +
            '\n\n—\nReply in the support desk portal (email replies to tickets are disabled):\n' +
            portalUrl;
