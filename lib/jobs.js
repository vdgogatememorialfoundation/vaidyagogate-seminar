...
const caseJudgeMarkingReminders = require('./case-judge-marking-reminders');
const seminarAlumniNotify = require('./seminar-alumni-notify');
...
function scheduleCaseJudgeMarkingReminders(db) {
    const cronExpr = process.env.CASE_JUDGE_REMINDER_CRON || '0 9 * * *';
    cron.schedule(cronExpr, () => {
        caseJudgeMarkingReminders.runCaseJudgeMarkingReminders(db, (err, r) => {
            if (err) console.error('[case-judge-reminders]', err.message);
            else if (r && r.sent) console.log('[case-judge-reminders] sent', r.sent);
        });
    });
}

function scheduleRazorpayRefundReconcile(db) {
    const cronExpr = process.env.REFUND_RECONCILE_CRON || '*/5 * * * *';
    let running = false;
    cron.schedule(cronExpr, () => {
        if (running) return;
        running = true;
        try {
            ...
            razorpayRefundSync.reconcilePendingRazorpayRefunds(db, (err, summary) => {
                running = false;
                if (err) console.error('[refund-reconcile]', err.message);
                else if (summary && summary.updated) {
                    console.log('[refund-reconcile] updated', summary.updated, 'of', summary.checked);
                }
            });
        } catch (e) {
            running = false;
            console.warn('[refund-reconcile] failed:', e.message);
        }
    });
}

function scheduleSeminarAlumniNotifications(db) {
    const cronExpr = process.env.ALUMNI_NOTIFY_CRON || '0 * * * *';
    cron.schedule(cronExpr, () => {
        seminarAlumniNotify.runAutoAlumniNotifications(db, notif, (err, r) => {
            if (err) console.error('[alumni-notify]', err.message);
            else if (r && r.totalSent) console.log('[alumni-notify] sent', r.totalSent, 'across', r.processed, 'seminars');
        });
    });
}

function scheduleTicketExpiry(db) {
    const cronExpr = process.env.TICKET_EXPIRY_CRON || '*/10 * * * *';
    let running = false;
    const runOnce = () => {
        if (running) return;
        running = true;
        require('./ticket-expiry').runTicketExpiry(db, (err, r) => {
            running = false;
            if (err) console.error('[ticket-expiry]', err.message);
            else if (r && r.expired) console.log('[ticket-expiry] expired', r.expired, 'registrations across', r.seminars, 'seminars');
        });
    };
...
function startWorkers(db) {
    ...
    scheduleTicketExpiry(db);
    scheduleDailyReminders(db);
    schedulePendingRegistrationReminders(db);
    scheduleCaseJudgeMarkingReminders(db);
    scheduleRazorpayRefundReconcile(db);
    scheduleSeminarAlumniNotifications(db);
    caseJudgeMarkingReminders.ensureSchema(db, () => {});
    pendingRegReminders.ensureSchema(db, () => {});
    seminarAlumniNotify.ensureSchema(db, () => {});
    try {
        require('./platform-daily-backup').scheduleDailyBackup(db);
    } catch (e) {
        console.warn('[platform-backup] schedule failed:', e.message);
    }
    console.log('[jobs] Notification queue + daily + pending registration + case judge + refund reconcile + alumni notify started.');
}
