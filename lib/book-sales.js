...
const { bookShiprocketShipment, bookNimbuspostShipment } = require('./logistics-aggregators');
...
function normalizeConfig(raw) {
    ...
    c.books = books
        .map((b) => ({
            ...
            stock: normalizeBookStock(b && b.stock)
        }))
    ...
    return {
        ...
        books: config.enabled
            ? config.books.filter((b) => b.active !== false).map((b) => ({
                  ...
                  author: b.author,
                  price: b.price,
                  stock: b.stock || normalizeBookStock(null)
              }))
            : [],
...
function ensureBookSalesSchema(db, cb) {
    ...
    const colAlters = isPg
        ? [
            ...
            'ALTER TABLE book_order_items ADD COLUMN IF NOT EXISTS line_note TEXT'
          ]
        : [
            ...
            'ALTER TABLE book_order_items ADD COLUMN line_note TEXT'
          ];
...
/** Per-language stock: null = unlimited, number = units available. */
function normalizeBookStock(raw) {
    const stock = {};
    LANGUAGE_DEFS.forEach((l) => {
        const v = raw && raw[l.id];
        if (v === null || v === undefined || v === '') stock[l.id] = null;
        else {
...
function bookStockRemaining(book, lang) {
...
function applyStockDeltaInMemory(config, bookId, language, delta) {
    const book = (config.books || []).find((b) => b.id === bookId);
    if (!book) return { ok: false, error: 'Unknown book: ' + bookId };
    if (!book.stock) book.stock = normalizeBookStock(null);
    const rem = bookStockRemaining(book, language);
    if (rem === null) return { ok: true };
    const next = rem + delta;
    if (next < 0) {
        return {
            ok: false,
            error:
    ...
    book.stock[language] = next;
    return { ok: true };
}

function reserveStockForLines(db, config, lines, cb) {
    const cfg = cloneConfig(config);
    for (const line of lines) {
        const r = applyStockDeltaInMemory(cfg, line.bookId, line.language, -(line.qty || 0));
        if (!r.ok) return cb(null, r.error);
    }
    saveBookSalesConfig(db, cfg, null, (err) => cb(err, cfg));
}

function releaseStockForLines(db, config, lines, cb) {
    const cfg = cloneConfig(config);
    for (const line of lines) {
        applyStockDeltaInMemory(cfg, line.bookId, line.language, line.qty || 0);
    }
    saveBookSalesConfig(db, cfg, null, (err) => cb(err, cfg));
}

function mapItemRow(ir, config) {
    const book = config ? bookById(config, ir.book_id) : null;
    return {
        id: ir.id,
        bookId: ir.book_id,
        bookTitle: book ? book.title : ir.book_id,
        language: ir.language,
        languageLabel: languageLabel(ir.language),
        qty: ir.qty,
        unitPrice: ir.unit_price,
        lineTotal: ir.line_total,
        lineNote: ir.line_note || null
    };
}
...
function validateLineItems(config, items, opts) {
    opts = opts || {};
    const lines = [];
    let total = 0;
    ...
    for (const raw of items) {
        ...
        const lineNote = String((raw && raw.lineNote) || '').trim() || null;
        ...
        if (!opts.skipStock) {
            const rem = bookStockRemaining(book, lang);
            if (rem !== null && rem === 0) {
                return { error: book.title + ' (' + languageLabel(lang) + ') is out of stock.' };
            }
            if (rem !== null && qty > rem) {
                return {
                    error: book.title + ' (' + languageLabel(lang) + '): only ' + rem + ' available in stock.'
                };
            }
        }
        const unit = Number(book.price) || 0;
        if (unit <= 0 && !opts.allowZeroPrice) {
            return { error: 'Book price not configured by admin yet (' + book.title + ').' };
        }
        const lineTotal = Math.round(unit * qty * 100) / 100;
        total += lineTotal;
        lines.push({ bookId, language: lang, qty, unitPrice: unit, lineTotal, bookTitle: book.title, lineNote });
    }
    return { lines, total: Math.round(total * 100) / 100 };
}

function cancelBookOrderWithStockRestore(db, bookOrderId, cb) {
    db.all(
        `SELECT book_id, language, qty FROM book_order_items WHERE book_order_id = ?`,
        [bookOrderId],
        (e0, itemRows) => {
            if (e0) return cb(e0);
            loadBookSalesConfig(db, (e1, config) => {
                if (e1) return cb(e1);
                const lines = (itemRows || []).map((ir) => ({
                    bookId: ir.book_id,
                    language: ir.language,
                    qty: ir.qty
                }));
                const doCancel = () => {
                    db.run(
                        ...
                        function (err) {
                            if (err) return cb(err);
                            if (!this.changes) return cb(null, { error: 'Cannot cancel' });
                            logBookOrderEvent(
                                db,
                                bookOrderId,
                                'cancelled',
                ...
                if (!lines.length) return doCancel();
                releaseStockForLines(db, config, lines, (relErr) => {
                    if (relErr) return cb(relErr);
                    doCancel();
                });
            });
        }
...
function updateBookOrderItems(db, bookOrderId, payload, adminId, cb) {
    const rawItems = payload && payload.items;
    const adminNote = String((payload && payload.adminNote) || '').trim();
    if (!Array.isArray(rawItems) || !rawItems.length) {
        return cb(null, { error: 'At least one line item is required.' });
    }
    fetchBookOrderFull(db, bookOrderId, (err, order) => {
        if (err) return cb(err);
        if (!order) return cb(null, { error: 'Order not found' });
        const editable = ['awaiting_confirmation', 'confirmed', 'pending_payment'];
        if (!editable.includes(order.status)) {
            return cb(null, { error: 'Cannot change items after shipping or completion.' });
        }
        loadBookSalesConfig(db, (cfgErr, config) => {
            if (cfgErr) return cb(cfgErr);
            const oldLines = (order.items || []).map((it) => ({
                bookId: it.bookId,
                language: it.language,
                qty: it.qty
            }));
            const cfgWork = cloneConfig(config);
            oldLines.forEach((line) => applyStockDeltaInMemory(cfgWork, line.bookId, line.language, line.qty));
            const validated = validateLineItems(cfgWork, rawItems, { skipStock: false, allowZeroPrice: true });
            if (validated.error) return cb(null, { error: validated.error });
            for (const line of validated.lines) {
                const r = applyStockDeltaInMemory(cfgWork, line.bookId, line.language, -line.qty);
                if (!r.ok) return cb(null, { error: r.error });
            }
            saveBookSalesConfig(db, cfgWork, null, (saveErr) => {
                if (saveErr) return cb(saveErr);
                db.run(`DELETE FROM book_order_items WHERE book_order_id = ?`, [bookOrderId], (delErr) => {
                    if (delErr) return cb(delErr);
                    let left = validated.lines.length;
                    if (!left) return cb(null, { error: 'No items' });
                    validated.lines.forEach((line) => {
                        db.run(
                            `INSERT INTO book_order_items (book_order_id, book_id, language, qty, unit_price, line_total, line_note)
                             VALUES (?, ?, ?, ?, ?, ?, ?)`,
                            [
                                bookOrderId,
                                line.bookId,
                            ...
                            (insErr) => {
                                ...
                                left--;
                                if (left > 0) return;
                                const notes = adminNote || null;
                                db.run(
                                    `UPDATE book_orders SET total_amount = ?, notes = COALESCE(?, notes), updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
                                    [validated.total, notes, bookOrderId],
                                    (uErr) => {
                                        ...
                                        logBookOrderEvent(
                                            ...
                                            { adminId: adminId || null, total: validated.total },
                                            () =>
                                                fetchBookOrderFull(db, bookOrderId, (e2, fresh) =>
                                                    cb(e2, { success: true, order: fresh })
                                                )
                                        );
                                    }
                                );
        ...
        (err, row) => {
            ...
            db.all(
                ...
                (e2, itemRows) => {
                    ...
                    loadBookSalesConfig(db, (cfgErr, config) => {
                        if (cfgErr) return cb(cfgErr);
                        const items = (itemRows || []).map((ir) => mapItemRow(ir, config));
...
function registerBookSalesRoutes(app, db, deps) {
    ...
    app.get('/api/admin/book-sales/logistics-config', (req, res) => {
    ...
    app.get('/api/admin/book-sales/orders', (req, res) => {
        ...
        db.all(sql, params, (err, rows) => {
            ...
            db.all(
                `SELECT * FROM book_order_items WHERE book_order_id IN (${placeholders}) ORDER BY book_order_id, id`,
                ids,
                (e2, itemRows) => {
                    if (e2) return res.status(500).json({ error: e2.message });
                    loadBookSalesConfig(db, (e3, config) => {
                        if (e3) return res.status(500).json({ error: e3.message });
                        const byOrder = {};
                        (itemRows || []).forEach((ir) => {
                            if (!byOrder[ir.book_order_id]) byOrder[ir.book_order_id] = [];
                            byOrder[ir.book_order_id].push(mapItemRow(ir, config));
                        });
    ...
    app.post('/api/admin/book-sales/orders/:id/items', (req, res) => {
        ...
        updateBookOrderItems(db, id, req.body || {}, adminId, (err, result) => {
            if (err) return res.status(500).json({ error: err.message });
    ...
    app.post('/api/admin/book-sales/orders/:id/cancel', (req, res) => {
        const id = parseInt(req.params.id, 10);
        if (!Number.isInteger(id) || id < 1) return res.status(400).json({ error: 'Invalid order id' });
        cancelBookOrderWithStockRestore(db, id, (err, result) => {
            if (err) return res.status(500).json({ error: err.message });
            if (result && result.error) return res.status(400).json({ error: result.error });
            res.json(result);
        });
    ...
    app.post('/api/admin/book-sales/orders/:id/status', (req, res) => {
        ...
        if (status === 'cancelled') {
            return cancelBookOrderWithStockRestore(db, id, (err, result) => {
                if (err) return res.status(500).json({ error: err.message });
                if (result && result.error) return res.status(400).json({ error: result.error });
                res.json(result);
            });
                        ...
                        db.run(
                            ...
                            () => {
                                ...
                                if (!left) {
                                    ...
                                    reserveStockForLines(db, config, validated.lines, (stockErr) => {
                                        if (stockErr) console.warn('[book-sales] stock reserve:', stockErr.message);
                                        finishPlaced();
                                    });
                                }
                            }
    ...
    app.get('/api/admin/book-sales/logistics-status', (req, res) => {
    ...
    app.post('/api/admin/book-sales/orders/:id/book-aggregator', (req, res) => {
        const id = parseInt(req.params.id, 10);
        const body = req.body || {};
