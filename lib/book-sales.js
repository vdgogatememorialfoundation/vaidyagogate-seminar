...
function courierTrackingUrl(provider, trackingNo) {
    ...
    if (p === 'indian_post') {
        return 'https://www.indiapost.gov.in/_layouts/15/DOP.Portal.Tracking/TrackConsignment.aspx?TrackingNumber=' + enc;
    }
    if (p === 'dtdc') return 'https://www.dtdc.in/tracking/tracking_results.asp?Ttype=awb_no&strCnno=' + enc;
    if (p === 'bluedart') return 'https://www.bluedart.com/web/guest/trackdartresultthirdparty?trackFor=0&trackNo=' + enc;
    if (p === 'delhivery') return 'https://www.delhivery.com/track/package/' + enc;
    if (p === 'ecom_express') return 'https://ecomexpress.in/tracking/?awb_field=' + enc;
    return 'https://www.google.com/search?q=' + encodeURIComponent('courier track ' + t);
}
...
function buildBookOrderTimeline(order) {
    ...
    if (online) {
        const payDone =
            st !== 'pending_payment' &&
            (st === 'confirmed' || st === 'fulfilled' || order.paymentStatus === 'success');
    ...
    if (st === 'confirmed') {
        ...
        const prov = providerLabels[order.courierProvider] || order.courierProvider || 'Courier';
        steps.push(
            mk(
                'Shipped',
                order.courierTrackingNo
                    ? prov + ' · Tracking ' + order.courierTrackingNo
                    : 'Dispatched via ' + prov,
                'completed',
                order.courierDispatchedAt || order.fulfilledAt,
                'fa-truck',
        ...
        steps.push(
            mk(
                'Delivered',
                'Marked as delivered via courier.',
                'completed',
                order.fulfilledAt,
                'fa-check-circle'
            )
        ...
        db.run(
            ...
            function (err) {
                ...
                logBookOrderEvent(
                    ...
                    'shipped_courier',
                    'Shipped by courier',
                    (provider || 'Courier') + ' · ' + trackingNo,
                    { courierProvider: provider, trackingNo, courierCharge: charge }
                );
    ...
    app.post('/api/scanner/volunteer-book-fulfill', (req, res) => {
        ...
        lookupBookOrderFromQr(db, qrData, (err, bookOrderId) => {
            ...
            fetchOrderWithCheckinStatus(db, bookOrderId, (e2, result) => {
                ...
                if (previewOnly || order.status !== 'confirmed') {
                    const payload = {
                    ...
                    const code =
                        order.status === 'fulfilled' ? 400 : order.status === 'confirmed' ? 200 : 403;
