...
const { rzCall } = require('./razorpay-call');
...
function normalizeConfig(raw) {
    ...
    c.defaultParcelWeightKg = Math.max(0.1, Number(c.defaultParcelWeightKg) || 0.5);
    c.defaultParcelLengthCm = Math.max(1, Number(c.defaultParcelLengthCm) || 22);
    c.defaultParcelBreadthCm = Math.max(1, Number(c.defaultParcelBreadthCm) || 15);
    c.defaultParcelHeightCm = Math.max(1, Number(c.defaultParcelHeightCm) || 5);
...
function publicBookSalesPayload(config, logisticsExtras) {
    ...
    return {
        ...
        defaultParcelWeightKg: config.defaultParcelWeightKg,
        defaultParcelLengthCm: config.defaultParcelLengthCm,
        defaultParcelBreadthCm: config.defaultParcelBreadthCm,
        defaultParcelHeightCm: config.defaultParcelHeightCm,
...
function parseDoctorCourierCheckout(body, salesConfig) {
    ...
    const lineQty = Array.isArray(body.items)
        ? body.items.reduce((sum, line) => sum + (parseInt(line.qty, 10) || 0), 0)
        : 0;
    const qtyFactor = Math.max(1, lineQty || 1);
    const cfgWeight = Number(salesConfig.defaultParcelWeightKg) || 0.5;
    const cfgLength = Number(salesConfig.defaultParcelLengthCm) || 22;
    const cfgBreadth = Number(salesConfig.defaultParcelBreadthCm) || 15;
    const cfgHeight = Number(salesConfig.defaultParcelHeightCm) || 5;
    const weightKg =
        body.weightKg != null
            ? Number(body.weightKg)
            : body.parcelWeightKg != null
              ? Number(body.parcelWeightKg)
              : cfgWeight * qtyFactor;
    const lengthCm =
        body.lengthCm != null ? Number(body.lengthCm) : body.length != null ? Number(body.length) : cfgLength;
    const breadthCm =
        body.breadthCm != null ? Number(body.breadthCm) : body.breadth != null ? Number(body.breadth) : cfgBreadth;
    const heightCm =
        body.heightCm != null ? Number(body.heightCm) : body.height != null ? Number(body.height) : cfgHeight;
    ...
    return {
        ...
        charge,
        shiprocketCourierId,
        weightKg: Number.isFinite(weightKg) && weightKg > 0 ? weightKg : cfgWeight * qtyFactor,
        lengthCm: Number.isFinite(lengthCm) && lengthCm > 0 ? lengthCm : cfgLength,
        breadthCm: Number.isFinite(breadthCm) && breadthCm > 0 ? breadthCm : cfgBreadth,
        heightCm: Number.isFinite(heightCm) && heightCm > 0 ? heightCm : cfgHeight
    };
}
...
function fulfillBookOrderPayment(db, { bookOrderId, orderDbId, gateway, txnId }, cb) {
    db.run(
        ...
        (uErr) => {
            ...
            confirmBookOrder(db, bookOrderId, null, (cErr, result) => {
                ...
                if (result && result.error) return cb(new Error(result.error));
                cb(null, result);
            });
        }
    );
...
function pollPendingBookOrderPayment(db, payRow, user, cb) {
    const gw = String(payRow.payment_gateway || '');
    if (gw.includes('razorpay') && payRow.provider_order_id) {
        return db.all(`SELECT * FROM payment_gateways WHERE is_active = 1`, [], (eG, gwRows) => {
            ...
            const opt = resolved || adminPaymentFlow.pickRazorpayGateway(gwRows);
            if (!opt) return cb(null, false);
            const rz = new Razorpay({
                key_id: opt.config.key_id,
                key_secret: opt.config.key_secret
            });
            rzCall((next) => rz.orders.fetchPayments(payRow.provider_order_id, next), (payErr, payments) => {
                if (payErr) return cb(null, false);
                const items = (payments && payments.items) || [];
                const captured = items.find((p) => String(p.status).toLowerCase() === 'captured');
                if (!captured) return cb(null, false);
                cb(null, { gateway: gw, txnId: captured.id });
            });
        });
    }
    const gateway = [...hostedCheckout.HOSTED_GATEWAYS].find((g) => gw.startsWith(g));
    if (!gateway || !payRow.provider_order_id) return cb(null, false);
    db.all(`SELECT * FROM payment_gateways WHERE is_active = 1`, [], (eG, gwRows) => {
        ...
        if (!resolved) return cb(null, false);
        hostedCheckout.pollGateway(gateway, resolved.config, payRow, user, (pErr, poll) => {
            if (pErr || !poll || !poll.paid) return cb(null, false);
            const ref = poll.easepayId || poll.paymentId || poll.providerRef || 'PG_' + payRow.provider_order_id;
            cb(null, { gateway: gw, txnId: ref });
        });
    });
}

function initiateBookOrderPayment(db, listDoctorPaymentOptions, { bookOrderId, bookOrder, orderDbId, orderIdStr, amount, uid, mid }, res) {
    const respondPaid = (gateway, txnId, message) => {
        fulfillBookOrderPayment(db, { bookOrderId, orderDbId, gateway, txnId }, (fErr, result) => {
            if (fErr) return res.status(500).json({ success: false, error: fErr.message });
            res.json({
                success: true,
                paid: true,
                message: message || 'Payment received. Your book order is confirmed.',
    ...
    listDoctorPaymentOptions((eList, options) => {
        ...
        if (!opt) return res.status(400).json({ success: false, error: 'Choose a payment method.' });
        db.all(`SELECT * FROM payment_gateways`, [], (eGw, gwRows) => {
            ...
            if (mid === 'dqr') {
                ...
                return adminPaymentFlow.createRazorpayDqr(
                    razorpay,
                    { amountRupee: amount, orderIdStr, applicationNo: bookOrder.order_code },
                    (dErr, dqr) => {
                        if (dErr) return res.status(400).json({ success: false, error: dErr.message });
                        db.run(
            ...
            const orderRow = { id: orderDbId, order_id_string: orderIdStr };
            const finishInit = (body) => {
                res.json({
                    success: true,
                    paid: false,
                    bookOrderId,
            ...
            if (hostedCheckout.isHosted(resolved.gateway)) {
                return hostedCheckout.initiateBookOrder(
                    db,
                    {
                        orderRow,
                        bookOrderId,
                        bookOrderCode: bookOrder.order_code,
                        userId: uid,
                        amount,
                        resolved,
                        finishInit
                    ...
                    (hErr, hOut) => {
                        if (hErr) return res.status(500).json({ success: false, error: hErr.message });
                        if (hOut && hOut.error) return res.status(400).json({ success: false, error: hOut.error });
                    }
                );
            }
            if (resolved.gateway === 'razorpay') {
                ...
                const receipt =
                    orderIdStr.length > 40 ? orderIdStr.slice(0, 40) : orderIdStr;
                ...
                return adminPaymentFlow.createRazorpayOrder(
                    rz,
                    ...
                    (rzErr, rzOrder) => {
                        if (rzErr) {
                            return res.status(400).json({
                                success: false,
                                error: adminPaymentFlow.razorpayErrorMessage(rzErr, 'Razorpay order failed')
                            });
                        }
                        db.run(
                            `UPDATE orders SET amount = ?, payment_gateway = ?, provider_order_id = ? WHERE id = ?`,
                            [amount, gwTag, rzOrder.id, orderDbId],
                            (uErr) => {
                                ...
                                finishInit({
                                    ...
                                    message: 'Razorpay checkout ready. Complete payment to confirm your book order.'
                                });
                            }
                        );
                    }
            ...
            res.status(400).json({
                success: false,
                error: `${resolved.label} is not available. Check gateway keys in Admin → Payment Gateways.`
            });
        });
    });
}
...
function registerBookSalesRoutes(app, db, deps) {
    ...
    app.get('/api/payments/book-order-status', (req, res) => {
        ...
        fetchBookOrderFull(db, bookOrderId, (err, order) => {
            ...
            if (order.status === 'pending_payment' && order.orderId) {
                db.get(`SELECT * FROM orders WHERE id = ?`, [order.orderId], (e2, pay) => {
                    ...
                    const user = { email: order.buyerEmail, phone: order.buyerPhone };
                    pollPendingBookOrderPayment(db, pay || {}, user, (pErr, polled) => {
                        ...
                        if (polled) {
                            return fulfillBookOrderPayment(
                                ...
                                {
                                    bookOrderId,
                                    orderDbId: order.orderId,
                                    gateway: polled.gateway,
                                    txnId: polled.txnId
                                },
                                (fErr, result) => {
    ...
    app.post('/api/admin/book-sales/pos', guard({ orders: true }, (req, res) => {
        ...
        const finishPosInsert = (userId, name, phone) => {
            loadBookSalesConfig(db, (err, config) => {
                ...
                const isOnlineGateway = paymentMode === 'online' && !!methodId;
                if (isOnlineGateway && !userId) {
                    return res.status(400).json({
                        error: 'Link a doctor account (search above) for online gateway checkout at POS.'
                    });
                }
                const initialStatus = isOnlineGateway ? 'pending_payment' : 'confirmed';
                ...
                db.run(
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
                    [
                        ...
                        config.seminarId,
                        initialStatus,
                        isOnlineGateway ? 'online' : paymentMode,
                        total,
                        name || null,
                        phone || null,
                        qr,
                        isOnlineGateway ? null : adminId || null,
                        isOnlineGateway ? null : new Date().toISOString()
                    ],
                function (insErr) {
                    if (insErr) return res.status(500).json({ error: insErr.message });
                    const bookOrderId = this.lastID;
                                    ...
                                    db.run(
                                        ...
                                        () => {
                                            logBookOrderEvent(
                                                ...
                                                'POS order placed',
                                                isOnlineGateway
                                                    ? 'Awaiting online gateway payment.'
                                                    : 'Walk-in sale at book desk.',
                                                { orderCode, paymentMode: isOnlineGateway ? 'online' : paymentMode }
                                            );
                                            const finishConfirmedPos = () => {
                                                logBookOrderEvent(
                                                    db,
                                                    bookOrderId,
                                                    'confirmed',
                                            ...
                                            if (isOnlineGateway) {
                                                const orderStr =
                                                    'BKP' + Date.now().toString(36).toUpperCase();
                                                return db.run(
                                                    `INSERT INTO orders (order_id_string, registration_id, amount, status) VALUES (?, NULL, ?, 'pending')`,
                                                    [orderStr, total],
                                                    function (oErr) {
                                                        ...
                                                        db.run(
                                                            ...
                                                            () => {
                                                                initiateBookOrderPayment(
                                                                    ...
                                                                    {
                                                                        bookOrderId,
                                                                        bookOrder: { order_code: orderCode },
                                                                        orderDbId,
                                                                        orderIdStr: orderStr,
                                                                        amount: total,
                                                                        uid: userId,
