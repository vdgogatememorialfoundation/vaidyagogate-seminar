...
function buildBookOrderTimeline(order) {
    ...
    } else if (courier && shipSt === 'ready_to_ship' && st !== 'fulfilled') {
        steps.push(mk('Courier delivery', 'Preparing shipment.', 'active', order.courierDetailsSavedAt, 'fa-truck'));
    } else if (st === 'fulfilled' && courier) {
        steps.push(
...
function registerBookSalesRoutes(app, db, deps) {
    ...
    app.post('/api/admin/book-sales/orders/:id/status', (req, res) => {
        ...
        return res.status(400).json({ error: 'Unsupported status. Use confirmed, fulfilled, or cancelled.' });
    });
    ...
    app.post('/api/admin/book-sales/orders/:id/dispatch-courier', (req, res) => {
        ...
        fetchBookOrderFull(db, id, (err, order) => {
            ...
            db.run(
             ...
             courier_dispatched_at = CURRENT_TIMESTAMP, fulfilled_at = CURRENT_TIMESTAMP, fulfilled_by = ?,
                ...
                function (uErr) {
                    ...
                    fetchBookOrderFull(db, id, (e2, fresh) =>
                        res.json({
                            ...
                            order: fresh,
                            courierTrackingUrl: trackUrl,
                            message: 'Order dispatched. Doctor can track via ' + courierProviderLabel(provider) + '.'
                        })
                    );
                }
            );
