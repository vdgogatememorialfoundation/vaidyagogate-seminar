                    ...
                    db.run(
                        ...
                        function (uErr) {
                            ...
                            logBookOrderEvent(
                                ...
                                () =>
                                    notifyBookOrderById(db, bookOrderId, 'BOOK_ORDER_SHIPPED', () =>
...
function recalcBookOrderTotal(db, bookOrderId, cb) {
    db.get(
        `SELECT COALESCE(SUM(line_total), 0) AS total FROM book_order_items WHERE book_order_id = ? AND COALESCE(line_status, 'active') = 'active'`,
        [bookOrderId],
        (err, row) => {
            if (err) return cb(err);
            const total = Math.round((Number(row && row.total) || 0) * 100) / 100;
            db.run(
...
function registerBookSalesRoutes(app, db, deps) {
    ...
    app.get('/api/staff/book-sales/session', guard({}, (req, res) => {
        res.json({
            ...
            modules: {
                inventory: bookAuth.canAccessBookInventory(req.bookSalesActor),
                orders: bookAuth.canAccessBookOrders(req.bookSalesActor)
            }
        });
    }));
        ...
        fetchBookOrderFull(db, id, (err, order) => {
            ...
            db.run(
                ...
                function (uErr) {
                    ...
                    logBookOrderEvent(db, id, 'cancelled', 'Order cancelled', 'Cancelled by staff.', { actorId: req.bookSalesActorId }, () =>
