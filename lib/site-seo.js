...
function registerSiteSeoRoutes(app, deps) {
    ...
    app.get(['/favicon.ico', '/favicon.svg'], (req, res) => {
        loadPublicSiteCms((e, cms) => {
            const fav = normalizeSeo((cms && cms.seo) || {}).faviconUrl || DEFAULT_SEO.faviconUrl;
            if (fav.startsWith('http://') || fav.startsWith('https://')) {
                return res.redirect(302, fav);
            }
            if (fav.startsWith('/api/branding/logo/file')) {
                return serveBrandingFavicon(req, res, db);
            }
            if (String(req.path || '').endsWith('.svg') && fav.endsWith('.svg')) {
