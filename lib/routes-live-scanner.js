...
/** Effective check-in date (admin-set seminar check-in date only) and the seminar day it maps to. */
        ...
        (err, sem) => {
            ...
            db.all(
                ...
                (e2, days) => {
                    ...
                    const effective = semEnabled ? override : null;
    ...
    app.get('/api/admin/live-scanner/seminars', (req, res) => {
        ...
        db.all(
            ...
            (err, rows) => {
                ...
                res.json(
                    (rows || []).map((r) => {
                        const enabled = r.checkin_enabled === true || Number(r.checkin_enabled) === 1;
                        return {
                            id: r.id,
                            title: r.title,
                            checkin_enabled: enabled,
                            checkin_date: enabled ? normalizeCheckinDateYmd(r.checkin_date) || null : null
                        };
                    })
                );
            }
        ...
        db.all(
            ...
            (err, rows) => {
                ...
                loadCheckinContext(db, seminarId, (e3, checkin) => {
                    ...
                    } else if (!checkin.effectiveDate) {
                        roster = [];
                        checkin.days = [];
                        checkin.lockedToDay = true;
                        checkin.noDateSet = true;
