...
function upsertTelemetryRow(db, uid, ip, locationLabel, geoJson, diagnosticsJson, cb) {
    ...
    const upsertSql = isPg()
        ...
        : `INSERT OR REPLACE INTO user_client_telemetry (user_id, client_ip, location_label, geo_json, diagnostics_json, updated_at)
           VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`;

    db.run(upsertSql, [uid, ip, locationLabel, geoJson, diagnosticsJson], (err) => finish(err));
}
