...
function attachPaymentAmountsToRegistrations(db, rows, cb) {
    ...
    const attach = (orderAmounts) => {
        let left = list.length;
        list.forEach((row, i) => {
            computeRegistrationPaymentAmount(db, row.id, (err, pay) => {
                if (!err && pay) {
                    const orderAmt = orderAmounts[row.id];
                    out[i].payment_amount = orderAmt != null ? orderAmt : pay.amount;
    ...
    const ids = list.map((r) => parseInt(r.id, 10)).filter((id) => Number.isInteger(id) && id > 0);
    if (!ids.length) return attach({});
    // An order amount (e.g. an admin-set custom fee on the pending order) is
    // authoritative over the computed seminar/sessions total. Paid orders win.
    db.all(
        `SELECT registration_id, amount FROM orders
         WHERE registration_id IN (${ids.map(() => '?').join(',')}) AND status IN ('success', 'pending')
        ...
        (eOrd, orderRows) => {
            ...
            if (!eOrd && orderRows) {
                orderRows.forEach((o) => {
                    if (orderAmounts[o.registration_id] == null && o.amount != null) {
                        orderAmounts[o.registration_id] = Number(o.amount);
                    }
                });
            }
            attach(orderAmounts);
