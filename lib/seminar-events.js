...
function replaceForSeminar(db, seminarId, events, cb) {
    ...
    db.run(`DELETE FROM seminar_events WHERE seminar_id = ?`, [sid], (delErr) => {
        ...
        list.forEach((ev, idx) => {
            ...
            db.run(
                `INSERT INTO seminar_events (seminar_id, title, event_date, checkin_date, price, sort_order, checkin_enabled, cert_scans_required, is_active)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [sid, title, eventDate, checkinDate, price, sortOrder, checkinEnabled, certScans, isActive],
                function (insErr) {
    ...
    db.get(
        ...
        (err, reg) => {
            ...
            listForSeminar(db, reg.seminar_id, true, (e2, events) => {
                if (e2) return cb(e2);
                if (!events.length) {
                    const amt = Number(reg.seminar_price) || 0;
                    return cb(null, {
                        amount: amt,
                ...
                const picked = events.filter((ev) => selected.includes(ev.id));
                if (!picked.length) {
                    return cb(null, {
                        amount: 0,
                ...
                const breakdown = picked.map((ev) => ({
                    eventId: ev.id,
                    label: ev.title,
                    amount: ev.price
                }));
                const amount = breakdown.reduce((s, b) => s + (Number(b.amount) || 0), 0);
                cb(null, {
