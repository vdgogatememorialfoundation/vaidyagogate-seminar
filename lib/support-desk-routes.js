...
function registerSupportDeskRoutes(app, deps) {
    const { db, loadPublicSiteCms, getSupportTicketPayload, resolveSupportTicketByRef, canonicalTicketMessageId } =
        deps;
    ...
    app.post('/api/public/support/chat', async (req, res) => {
        try {
            ...
            const knowledge = await chatbotKnowledge.buildChatbotKnowledge(db, loadPublicSiteCms);
                ...
                trackApplication(db, body.applicationNo, (eTr, tr) => {
                    if (!eTr && tr && !tr.error) {
                        if (tr.type === 'seminar') {
                            ...
                            reply = `Support ticket ${tr.ticketRef}: ${tr.status}. Replies appear in the doctor portal under Feedback & Support.`;
                        }
                    }
    ...
    app.post('/api/support-desk/tickets/:ticketRef/reply', requireSupportAgent(db, (req, res) => {
        ...
        resolveSupportTicketByRef(req.params.ticketRef, (err, ticket) => {
            ...
            const visibleAt = supportDesk.computeVisibleAtForStaffReply(ticket);
            db.run(
    ...
    app.get('/api/support-desk/departments', requireSupportAgent(db, (_req, res) => {
        db.all(
            `SELECT id, slug, name, sort_order FROM support_departments WHERE IFNULL(is_active,1)=1 ORDER BY sort_order, name`,
            [],
            (err, rows) => res.json(err ? { error: err.message } : rows || [])
        );
    }));
    ...
    app.get('/api/public/support/live/:sessionId', (req, res) => {
        ...
        db.get(`SELECT id, status, assigned_agent_id, visitor_key FROM support_live_sessions WHERE id = ?`, [sid], (err, row) => {
            if (err) return res.status(500).json({ error: err.message });
    ...
    app.get('/api/public/support/live/:sessionId/messages', (req, res) => {
        supportLiveChat.listMessages(db, req.params.sessionId, req.query.since, (err, rows) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json(rows || []);
        });
    });
    ...
    app.post('/api/public/support/live/:sessionId/message', (req, res) => {
        const msg = String((req.body && req.body.message) || '').trim();
        supportLiveChat.addMessage(db, req.params.sessionId, 'visitor', null, msg, (err, out) => {
    ...
    app.get('/api/support-desk/live/sessions', requireSupportAgent(db, (req, res) => {
        ...
        db.all(
            `SELECT s.id, s.visitor_key, s.user_id, s.assigned_agent_id, s.status, s.started_at, s.last_message_at,
                    u.first_name, u.last_name, u.email, u.user_id_string
    ...
    app.post('/api/support-desk/live/:sessionId/claim', requireSupportAgent(db, (req, res) => {
        supportLiveChat.claimSession(db, req.params.sessionId, req.supportAgentId, (err, out) => {
            if (err) return res.status(400).json({ error: err.message });
            res.json(out);
        });
    }));
    ...
    app.post('/api/support-desk/live/:sessionId/message', requireSupportAgent(db, (req, res) => {
        const msg = String((req.body && req.body.message) || '').trim();
        supportLiveChat.addMessage(db, req.params.sessionId, 'agent', req.supportAgentId, msg, (err, out) => {
            if (err) return res.status(400).json({ error: err.message });
            res.json({ success: true, id: out.id });
        });
    }));
    ...
    app.post('/api/support-desk/live/:sessionId/close', requireSupportAgent(db, (req, res) => {
        supportLiveChat.closeSession(db, req.params.sessionId, (err) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true });
        });
    }));
