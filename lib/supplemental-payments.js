...
function registerSupplementalPaymentRoutes(app, db, deps) {
    ...
    app.get('/api/doctor/supplemental-payments', (req, res) => {
        const uid = parsePositiveUserId(req.query.userId);
    ...
    app.post('/api/admin/supplemental-payments', (req, res) => {
        ...
        if (!Number.isInteger(userId) || userId < 1) return res.status(400).json({ error: 'userId required' });
        ...
        const adminId = parseInt(body.actingAdminId, 10) || null;
        db.run(
            ...
            [
                userId,
                Number.isInteger(seminarId) ? seminarId : null,
                Number.isInteger(regId) ? regId : null,
                title,
